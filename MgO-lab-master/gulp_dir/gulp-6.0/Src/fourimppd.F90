  subroutine fourimppd(xkv,ykv,zkv)
!
!  Subroutine for four-body improper torsion phased second derivatives
!  Distributed memory version.
!
!  12/16 Created from fourimpp
!   1/17 Parallel changes added
...
!
  ...
  if (status/=0) call outofmemory('fourimppd','natmiddle')
  allocate(ntypmiddle(2*nfor),stat=status)
  if (status/=0) call outofmemory('fourimppd','ntypmiddle')
  allocate(nptrnforimp(nfor),stat=status)
  if (status/=0) call outofmemory('fourimppd','nptrnforimp')
  allocate(nuniqueiptr(numat),stat=status)
  if (status/=0) call outofmemory('fourimppd','nuniqueiptr')
  allocate(nuniquekptr(numat),stat=status)
  if (status/=0) call outofmemory('fourimppd','nuniquekptr')
  allocate(nuniquelptr(numat),stat=status)
  if (status/=0) call outofmemory('fourimppd','nuniquelptr')
!
...
!
        if (lanydreiding) then
          call isumall(niltor,niltortot,1_i4,"fourimppd","niltor")
        else
          niltortot = niltor
        endif
!*******************************
...
!
  ...
  enddo ljloop2
!*********************************************************
!  Loop over middle site 2 / j - second middle atom case *
!*********************************************************
...
!
  ...
  if (status/=0) call deallocate_error('fourimppd','nuniquelptr')
  deallocate(nuniquekptr,stat=status)
  if (status/=0) call deallocate_error('fourimppd','nuniquekptr')
  deallocate(nuniqueiptr,stat=status)
  if (status/=0) call deallocate_error('fourimppd','nuniqueiptr')
  deallocate(nptrnforimp,stat=status)
  if (status/=0) call deallocate_error('fourimppd','nptrnforimp')
  deallocate(ntypmiddle,stat=status)
  if (status/=0) call deallocate_error('fourimppd','ntypmiddle')
  deallocate(natmiddle,stat=status)
  if (status/=0) call deallocate_error('fourimppd','natmiddle')
#ifdef TRACE
  call trace_out('fourimppd')
#endif
!
  return
  end

...
!
    if (iflag.eq.-2) then
      if (lrigid) then
!
!  For rigid molecule case try reseting the quaternions
!
        call checkquaternions(lgeometryOK,.true.)
!
!  Call funct again to see if this has fixed the problem
!
        iflag = 1_i4
        call funct(iflag,N,X,F,G)
        if (iflag.eq.-2) then
          call outerror('geometry has become unphysical during LMBFGS relaxation',0_i4)
          call stopnow('lmbfgs')
        endif
      else
        call outerror('geometry has become unphysical during LMBFGS relaxation',0_i4)
...
!
    call lbfgsstep(STX,FXM,DGXM,STY,FYM,DGYM,stp,FM,DGM,BRACKT,STMIN,STMAX,INFOC)
!
...
!
!  Call lbfgsstep to update the interval of uncertainty and to compute the new step.
!
    call lbfgsstep(STX,FX,DGX,STY,FY,DGY,stp,F,DG,BRACKT,STMIN,STMAX,INFOC)
  endif
!
...
!
  ...
  subroutine lbfgsstep(STX,FX,DX,STY,FY,DY,stp,FP,DMP,BRACKT,stpMIN,stpMAX,INFO)
!
  use datatypes
#ifdef TRACE
  use trace,         only : trace_in, trace_out
...
!
!  The purpose of lbfgsstep is to compute a safeguarded step for
...
!  THE SUBROUTINE STATEMENT IS
!
!  subroutine lbfgsstep(STX,FX,DX,STY,FY,DY,stp,FP,DMP,BRACKT,
!                      stpMIN,stpMAX,INFO)
!
!   WHERE
!
...
!
  if ((BRACKT .AND. (stp .LE. MIN(STX,STY) .OR. stp .GE. MAX(STX,STY))) .OR. &
      DX*(stp-STX) .GE. 0.0 .OR. stpMAX .LT. stpMIN) then
#ifdef TRACE
    call trace_out('lbfgsstep')
#endif
    return
  endif
!
...
!
  ...
  return
!
!  Last line of subroutine lbfgsstep.
!
  end

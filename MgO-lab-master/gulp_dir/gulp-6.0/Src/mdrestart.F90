...
!  12/15 Handling of xyz files added for restart
!   9/16 Core/shell mass split corrected
!   6/12 Anisotropic pressure enabled for default integrator
!   3/17 Modifications made to allow for new variable order in iopt
!  10/17 Set up of absolute coordinates moved to setup
!  10/17 Dipole output file added
!   1/18 Trace added
!   1/19 Xcell set for all ensembles
!   1/19 maxwordlength changes added
!   3/19 iopt replaced by ioptindex and iopttype
!   3/20 Check on list based methods for 3-body potentials updated
!
!  Conditions of use:
!
...
!
  use g_constants,       only : pi, kjmtoev
  use control
  use current
...
!
  if (llist3) then
    lwarn = .false.
    do i = 1,nthb
      if (nthrty(i).eq.2.or.nthrty(i).eq.3.or.nthrty(i).eq.4.or.nthrty(i).eq.5.or.nthrty(i).eq.8.or. &
          nthrty(i).eq.14.or.nthrty(i).eq.25.or.nthrty(i).eq.27) then
        if (.not.lwarn) then
          llist3 = .false.
          nwarn = nwarn + 1
          if (ioproc) then
...
!
  if (lplumed) then
    if (ioproc) then
      write(ioout,'(/,''  PLUMED run with input = '',a50)') plumedinput(1:50)
      write(ioout,'(/,''         and log file   = '',a50,/)') plumedlog(1:50)
    endif
  endif
!
...
!****************************************************
  if (nmdintegrator.eq.4) then
    ...
    if (lanisotropicpress) then
      pr_target_press_tens(1,1) = anisotropicpress(1)
      pr_target_press_tens(2,2) = anisotropicpress(2)
      pr_target_press_tens(3,3) = anisotropicpress(3)
      pr_target_press_tens(3,2) = anisotropicpress(4)
      pr_target_press_tens(2,3) = anisotropicpress(4)
      pr_target_press_tens(3,1) = anisotropicpress(5)
      pr_target_press_tens(1,3) = anisotropicpress(5)
      pr_target_press_tens(2,1) = anisotropicpress(6)
      pr_target_press_tens(1,2) = anisotropicpress(6)
    else
      do i = 1,3
...
!
!  Set pointer to end of the xyz file if one exists on channel 33
!
  if (lxyz.and.lxyzmovie.and.ioproc) then
    if (xyzfile.ne.' ') then
      open(33,file=xyzfile,status='old',position='append',form='formatted',err=106)
      goto 108
    else
      open(33,status='old',position='append',form='formatted',err=106)
      goto 108
    endif
106 continue
!
...
!
    if (xyzfile(1:1).ne.' ') then
      open(33,file=xyzfile,status='new',form='formatted')
    else
      open(33)
    endif
108 continue
...
!
  if (lhis.and.ioproc) then
    if (hisfile(1:1).ne.' ') then
      open(34,file=hisfile,status='old',position='append',form='formatted',err=110)
      goto 112
    else
      open(34,status='old',position='append',form='formatted',err=110)
      goto 112
    endif
110 continue
!
...
!
    ...
    if (ntitle.gt.0) then
      write(34,'(a)') trim(titleword(1))
    else
      write(34,'(''History file written from GULP'')')
    endif
    keytrj = 1
...
!
!  Set pointer to end of the dipole file if one exists on channel 37
!
  if (ldip.and.ioproc) then
    if (dipfile(1:1).ne.' ') then
      open(37,file=dipfile,status='old',position='append',form='formatted',err=137)
      goto 140
    else
      open(37,status='old',position='append',form='formatted',err=137)
      goto 140
    endif
137 continue
    if (dipfile(1:1).ne.' ') then
      open(37,file=dipfile,status='new',form='formatted')
    else
      open(37)
    endif
140 continue

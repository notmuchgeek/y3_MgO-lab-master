...
!
  ...
  elseif (ndcentyp(ncf).eq.2) then
    iptr = nint(xdcent(ncf))
    inat = nat(nrela2f(iptr))
    itype = nftype(nrela2f(iptr))
      ...
      if (ndt.ge.10.and.ndt.lt.20) then
        ...
        if (inat.gt.2*maxele) then
          ...
          elseif (ndt.eq.12) then
            iptr = nrela2f(nint(xdef(ndefst+i)))
            inat = nat(iptr)
            itype = nftype(iptr)
...
!
  if (ndcon.gt.0) then
    ...
    ncfst = 0
    do i = 1,ndcon
      nvv1 = ncdvarind(i+ncfst)
      if (ncdvartyp(i+ncfst).eq.idopt_dx) then
        ncrv = 1
      elseif (ncdvartyp(i+ncfst).eq.idopt_dy) then
        ncrv = 2
      elseif (ncdvartyp(i+ncfst).eq.idopt_dz) then
        ncrv = 3
      endif
      nfv1 = ncdfixind(i+ncfst)
      if (ncdfixtyp(i+ncfst).eq.idopt_dx) then
        ncrf = 1
      elseif (ncdfixtyp(i+ncfst).eq.idopt_dy) then
        ncrf = 2
      elseif (ncdfixtyp(i+ncfst).eq.idopt_dz) then
        ncrf = 3
      endif
      write(ioout,'(8x,i4,14x,i4,1x,a1,10x,i4,1x,a1,6x,f10.5,5x,f7.4)') &
        i,nvv1,crd(ncrv),nfv1,crd(ncrf),dconco(i+ncfst)
    enddo
    write(ioout,'(''--------------------------------------------------------------------------------'',/)')
  endif
!

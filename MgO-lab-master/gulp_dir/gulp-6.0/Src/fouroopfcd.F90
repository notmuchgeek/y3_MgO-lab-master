...
#ifdef TRACE
  call trace_in('fouroopfcd')
#endif
!
!  Allocate local memory
!
  allocate(nuniqueptr(numat),stat=status)
  if (status/=0) call outofmemory('fouroopfcd','nuniqueptr')
  allocate(nuniquelocptr(numat),stat=status)
  if (status/=0) call outofmemory('fouroopfcd','nuniquelocptr')
!*************************
...
!
  ...
  if (status/=0) call deallocate_error('fouroopfcd','nuniquelocptr')
  deallocate(nuniqueptr,stat=status)
  if (status/=0) call deallocate_error('fouroopfcd','nuniqueptr')
!
!  All tidying up of derivatives is handled by four so we can just return here
!
#ifdef TRACE
  call trace_out('fouroopfcd')
#endif
  return
  end

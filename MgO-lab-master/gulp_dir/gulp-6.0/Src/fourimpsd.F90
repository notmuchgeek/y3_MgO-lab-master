...
!   6/17 Looping changed to avoid an error in the derivatives
!        of some atoms
!   2/18 Trace added
...
!
              ...
              lrooted1 = .false.
!
!  Flag as to whether any of the other atoms are in the asymmetric unit
!
              lasu123 = (lasu1.or.lasu2.or.lasu3)
!
              ...
              if (lbtyp) then
                ...
                if (nbonds(j).gt.0) then
                  if (lasu123) then
                    ...
                    nuniquel = 0
                    do lloop = 1,nbonds(j)
                      nrell = nrelf2a(nbonded(lloop,j))
                      if (nrela2f(nrell).eq.nbonded(lloop,j)) then
                        lunique = .true.
                        if (nuniquel.gt.0) then
                          do lu = 1,nuniquel
                            if (nbonded(lloop,j).eq.nuniquelptr(lu)) lunique = .false.
                          enddo
                        endif
...
!
!  If no atom is in the asymmetric then skip
!
                if (.not.lasu123.and..not.lasu4) cycle l4loop
!
                nl = nat(l)
                ntypl = nftype(l)
                ocl = occuf(l)

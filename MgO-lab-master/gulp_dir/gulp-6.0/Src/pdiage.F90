...
!
  ...
  if (status/=0) call outofmemory('pdiage','w1')
  allocate(w2(mcv),stat=status)
  if (status/=0) call outofmemory('pdiage','w2')
  allocate(w3(2*mcv),stat=status)
  if (status/=0) call outofmemory('pdiage','w3')
  call ch(maxd2,mcv,derv2,dervi,freq,matz,eigr,eigi,w1,w2,w3,ifail)
  deallocate(w3,stat=status)
  if (status/=0) call deallocate_error('pdiage','w3')
  deallocate(w2,stat=status)
  if (status/=0) call deallocate_error('pdiage','w2')
  deallocate(w1,stat=status)
  if (status/=0) call deallocate_error('pdiage','w1')
  t2 = g_cpu_time()
  tdiag = tdiag + t2 - t1
!

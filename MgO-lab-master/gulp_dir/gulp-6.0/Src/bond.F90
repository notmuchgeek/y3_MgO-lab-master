...
!   3/15 Format of output changed to be one distance per line followed
!        by bond type information
!   1/18 Trace added
...
!
  ...
  integer(i4)                                  :: ntype
  integer(i4)                                  :: numt
  integer(i4), dimension(:,:), allocatable     :: nbtype
...
#ifdef TRACE
  call trace_in('GULP_bond')
#endif
!
  allocate(nbtype(2_i4,maxbond),stat=status)
  if (status/=0) call outofmemory('GULP_bond','nbtype')
...
!
  ...
  write(ioout,'(''--------------------------------------------------------------------------------'')')
  write(ioout,'(''  Asymmetric unit site  Full lattice sites    No.  Distance      Bond Type      '')')
  write(ioout,'(''                                                    (Angs)                      '')')
  write(ioout,'(''--------------------------------------------------------------------------------'')')
!
!  Loop over sites
...
!
          ...
          r = sqrt(r)
!
!  Check to see if distance is equivalent to any that have already been characterised
!
          if (ntype.eq.0) then
            ntype = ntype + 1
            numtype(ntype) = 1
            rtype(ntype) = r
            nbtype(1,ntype) = nbondedtype(1,ib,ifull)
            nbtype(2,ntype) = nbondedtype(2,ib,ifull)
          else
            do n = 1,ntype
              diff = abs(r-rtype(n))
              if (diff.lt.1d-4.and.nbondedtype(1,ib,ifull).eq.nbtype(1,n)) then
...
!
      do k = 1,ntype
        ...
        if (nptr.ne.k) then
          ...
          numtype(nptr) = numt
          numt = nbtype(1,k)
          nbtype(1,k) = nbtype(1,nptr)
          nbtype(1,nptr) = numt
          numt = nbtype(2,k)
          nbtype(2,k) = nbtype(2,nptr)
          nbtype(2,nptr) = numt
        endif
      enddo
      nttype = nttype + ntype
...
!  First line
!
          write(ioout,'(1x,i6,4x,a5,1x,a5,7x,a5,1x,a5,1x,i7,2x,f8.4,4x,a9,1x,a3)') &
                i,labi,stype(nsptr1),labj,stype(nsptr2),numtype(1),rtype(1), &
                botyword(nbtype(1,1)+1),botyword2(nbtype(2,1))
          if (ntype.gt.1) then
            do m = 2,ntype
              write(ioout,'(41x,2(i7,2x,f8.4,4x,a9,1x,a3))') &
                    numtype(m),rtype(m),botyword(nbtype(1,m)+1),botyword2(nbtype(2,m))
            enddo
          endif
        else
          write(ioout,'(29x,a5,1x,a5,1x,i7,2x,f8.4,4x,a9,1x,a3)') &
                labj,stype(nsptr2),numtype(1),rtype(1),botyword(nbtype(1,1)+1),botyword2(nbtype(2,1))
          if (ntype.gt.1) then
            do m = 2,ntype
              write(ioout,'(41x,2(i7,2x,f8.4,4x,a9,1x,a3))') &
                    numtype(m),rtype(m),botyword(nbtype(1,m)+1),botyword2(nbtype(2,m))
...
!
  ...
  deallocate(nbtype,stat=status)
  if (status/=0) call deallocate_error('GULP_bond','nbtype')
#ifdef TRACE
  call trace_out('GULP_bond')
#endif
!
  return
  end

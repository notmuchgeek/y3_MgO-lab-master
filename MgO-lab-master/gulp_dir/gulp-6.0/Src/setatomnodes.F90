...
!
    ...
    if (status/=0) call outofmemory('setatomnodes','ncellpernodes')
!
    maxxy = nspcell(1)*nspcell(2)
    maxx  = nspcell(1)
!
...
!
    ...
    if (status/=0) call deallocate_error('setatomnodes','ncellpernodes')
    deallocate(natompernodes,stat=status)
    if (status/=0) call deallocate_error('setatomnodes','natompernodes')
  endif
!

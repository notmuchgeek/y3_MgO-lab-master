...
!  11/06 Created
!   2/18 Trace added
!   6/19 Modified so that all species quantities are shifted when species list is compacted
...
!
!  Additional quantities move for consistency 
!
      symspec(i) = symspec(nptr(i))
      libspec(i) = libspec(nptr(i))
      lgastinlibspec(i) = lgastinlibspec(nptr(i))
      lgastinspec(i) = lgastinspec(nptr(i))
      linspec(i) = linspec(nptr(i))
      lmassinspec(i) = lmassinspec(nptr(i))
      lnmrinspec(i) = lnmrinspec(nptr(i))
      lmask(i) = lmask(nptr(i))
      lmm3se(i) = lmm3se(nptr(i))
      lspininspec(i) = lspininspec(nptr(i))
      lvdwinspec(i) = lvdwinspec(nptr(i))
      gastspec(1:4,i) = gastspec(1:4,nptr(i))
      nmrspec(1:3,i) = nmrspec(1:3,nptr(i))
      spinspec(i) = spinspec(nptr(i))
      vdwspec(i) = vdwspec(nptr(i))
      natpolspec(i) = natpolspec(nptr(i))
      natratiomspec(i) = natratiomspec(nptr(i))
      dpolmaxspec(i) = dpolmaxspec(nptr(i))
      dpolspec(i) = dpolspec(nptr(i))
      qpolspec(i) = qpolspec(nptr(i))
      ratiomspec(i) = ratiomspec(nptr(i))
    enddo
    nspec = j
  endif
!

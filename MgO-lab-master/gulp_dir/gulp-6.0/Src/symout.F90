...
!
!     hmssg   = Hermann-Mauguin Symbol of Space Group (internal form)
!     hmssgio = Hermann-Mauguin Symbol of Space Group (external form)
...
!
    ...
    if (ncs.eq.0) write(ioout,'(/''  Space group (centrosymmetric)    :  '',16a1)') (hmssgio(i,ncf),i=1,16)
    if (ncs.eq.1) write(ioout,'(/''  Space group (noncentrosymmetric) :  '',16a1)') (hmssgio(i,ncf),i=1,16)

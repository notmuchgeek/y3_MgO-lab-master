  subroutine oscillatorstrengthg(mtv,mtvloc,mtvptr,nphonatc,nphonatptr,ncfoc,iocptr,eigr,maxd2,oscstrength)
!
...
!
          reig(1:3) = 0.0_dp
          do l = 1,3
            reig(l) = reig(l) + drQ(l,1)*eigr(ind+4,m)*rfmass(ind+4)
            reig(l) = reig(l) + drQ(l,2)*eigr(ind+5,m)*rfmass(ind+5)
            reig(l) = reig(l) + drQ(l,3)*eigr(ind+6,m)*rfmass(ind+6)
          enddo
!
...
!
    call sumall(oscstrength,sum3,9_i4*mtv,"oscillatorstrengthg","oscstrength")

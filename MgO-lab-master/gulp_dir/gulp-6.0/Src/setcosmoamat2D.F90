  subroutine setcosmoamat2D
!
!  Constructs the COSMO A matrix between points on the SAS
!  2-D matrix version.
!
!   4/17 Created from setcosmoamat
...
!  Zero cosmoA
!
  cosmoA(1:npts,1:nptsonnode) = 0.0_dp
!
...
!
    ...
    enddo
    aa = aa/(ri*rnari**2)
    cosmoA(ipts,iptsloc) = aa
!
!  Coulomb correction - with weighting
!
    if (lperiodic) then
      cosmoA(ipts,iptsloc) = cosmoA(ipts,iptsloc) + qmeself*swi*swi
    endif
!
...
!
          ...
          cosmoA(jpts,iptsloc) = cosmoA(jpts,iptsloc) + aij*swi*swj
        endif
      enddo
      if (lperiodic.and.(ipts.ne.jpts)) then
        call qmatrixelementc(xji,yji,zji,0.0_dp,.true.,.false.,.false.,qme,dqme,d2qme)
        cosmoA(jpts,iptsloc) = cosmoA(jpts,iptsloc) + qme*swi*swj
      endif
      if (nprocs.eq.1) then
        cosmoA(ipts,jpts) = cosmoA(jpts,ipts)
      endif
    enddo
  enddo
!
!  Debugging output
!
  if (index(keyword,'amat').ne.0.and.ldebug) then
    if (ioproc) then
    ...
    do i = 1,npts
      iloc = npts2local(i)
      if (iloc.gt.0) then
        write(ioout,'(10f12.6)')(cosmoA(j,iloc),j=1,npts)
      endif
      call mpbarrier
    enddo
  endif

...
!
  ...
  if (status/=0) call outofmemory('dynamicn_fcd','d1cell')
!
...
!
    ...
    if (mm.gt.mintloc) then
      lradial = .true.
      matom = nbsptr(node2atom(mm-mintloc))
      mcrd  = 4
    else
      lradial = .false.
      matom = (mm-1)/3 + 1
      mcrd  = mm - 3*(matom - 1)
      matom = node2atom(matom)
    endif
!
!  Select step
!
...
!
  ...
  do i = 1,natomsonnode
    ...
    iyg = ixg + 1
    izg = ixg + 2
    d2cell(ixg,ix,nd2central) = 0.0_dp
    d2cell(iyg,ix,nd2central) = 0.0_dp
    d2cell(izg,ix,nd2central) = 0.0_dp
    d2cell(ixg,iy,nd2central) = 0.0_dp
    d2cell(iyg,iy,nd2central) = 0.0_dp
    d2cell(izg,iy,nd2central) = 0.0_dp
    d2cell(ixg,iz,nd2central) = 0.0_dp
    d2cell(iyg,iz,nd2central) = 0.0_dp
    d2cell(izg,iz,nd2central) = 0.0_dp
  enddo
!
!  Free local memory
!
  deallocate(d1cell,stat=status)
  if (status/=0) call deallocate_error('dynamicn_fcd','d1cell')
#ifdef TRACE
  call trace_out('dynamicn_fcd')
#endif
!
  return
  end

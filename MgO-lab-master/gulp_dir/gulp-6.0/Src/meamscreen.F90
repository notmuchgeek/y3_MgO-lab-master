  subroutine meamscreen(neamspeci,neamspecj,neamspeck,rij2,rik2,rjk2,Sikj,dSikjdr,lpartial,lgrad1)
!
...
!  neamspeci    = reference to neamfnspec for i
!  neamspecj    = reference to neamfnspec for j
!  neamspeck    = reference to neamfnspec for k
!
!  On exit :
!
...
!
!   4/09 Created 
!   8/14 Parameters made species specific and neamspeck is added to arguments
...
!
  ...
  integer(i4), intent(in)    :: neamspeck
...
!  Local variables
!
  integer(i4)                :: indij            ! Index for i/j in MEAM screening arrays
...
!
  ...
  C = Ctop*Cbot
!
  if (C.lt.meam_Cmin(indij,neamspeck)) then
!
...
!
  ...
  elseif (C.gt.meam_Cmax(indij,neamspeck)) then
!
...
!
    lpartial = .true.
    dxdC = 1.0_dp/(meam_Cmax(indij,neamspeck) - meam_Cmin(indij,neamspeck))
    x = (C - meam_Cmin(indij,neamspeck))*dxdC

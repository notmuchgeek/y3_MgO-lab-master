...
#ifdef TRACE
  call trace_out('getanyneighbour')
#endif
  return
  end
...
!  nneigh        = number of neighbours for each atom
!  neighno       = pointer to atom numbers of neighbours for each atom
!  ineigh        = cell indices of neighbours for each atom
!  rneigh        = distances of neighbours for each atom
!  xneigh        = x component of distances of neighbours for each atom
!  yneigh        = y component of distances of neighbours for each atom
!  zneigh        = z component of distances of neighbours for each atom
...
!   7/11 Bug in initialisation of neighbour numbers fixed
!  11/14 ineigh added to arguments
!   2/18 Trace added
!
!  Conditions of use:
!
...
!
  ...
  use current,        only : xclat, yclat, zclat
  use current,        only : ivec1cell, ivec2cell
  use current,        only : xvec1cell, yvec1cell, zvec1cell
  use current,        only : xvec2cell, yvec2cell, zvec2cell
...
!
  integer(i4), intent(in)                        :: maxneigh
  integer(i4), intent(out)                       :: ineigh(3,maxneigh,numat)
  integer(i4), intent(out)                       :: nneigh(numat)
...
!  11/14 ineigh added to arguments
!   2/18 Trace added
!   1/20 Checking of maxneigh corrected for two atom case when i = j
!
!  Conditions of use:
!
!  GULP is available free of charge to academic institutions
...
!
!  Julian Gale, CIC, Curtin University, January 2020
!
  use datatypes
...
!
  integer(i4), intent(in)                          :: maxneigh
  integer(i4), intent(out)                         :: ineigh(3_i4,maxneigh,nREBOatom)
...
!
  ...
  integer(i4), dimension(:,:,:), allocatable, save :: ntmp3
...
#ifdef TRACE
  call trace_in('getREBOneighbour')
#endif
!
!  Initialise lmaxneighok and nneigh
!
...
!
  if (nprocs.gt.1) then
    ...
    ineigh(1:3,1:maxneigh,1:nREBOatom) = 0
...
!  
                ...
                if (r2 .lt. bR22(nREBObo)) then
                  if (nneigh(i).ge.maxneigh.or..not.lmaxneighok) then
                    ...
                    ineigh(1,nneigh(nri),nri) = ivec2cell(1,jc)
                    ineigh(2,nneigh(nri),nri) = ivec2cell(2,jc)
                    ineigh(3,nneigh(nri),nri) = ivec2cell(3,jc)
            ...
            if (i.ne.j.or.ii.ne.iimid) then
              ...
              if (r2 .lt. bR22(nREBObo)) then
                if (nneigh(nri).ge.maxneigh.or..not.lmaxneighok) then
                  ...
                  neighno(nneigh(nri),nri) = j
                  nREBObond(nneigh(nri),nri) = nREBObo
                  ineigh(1,nneigh(nri),nri) = ivec1cell(1,ii)
                  ineigh(2,nneigh(nri),nri) = ivec1cell(2,ii)
                  ineigh(3,nneigh(nri),nri) = ivec1cell(3,ii)
                  rneigh(nneigh(nri),nri) = rij
                  xneigh(nneigh(nri),nri) = xji
                  yneigh(nneigh(nri),nri) = yji
                  zneigh(nneigh(nri),nri) = zji
...
!
            if (i.ne.j.or.ii.ne.iimid) then
              ...
              r2 = xji*xji + yji*yji + zji*zji
              if (r2 .lt. bR22(nREBObo)) then
                if (nri.eq.nrj.and.nneigh(nri).ge.maxneigh-1.or..not.lmaxneighok) then
                  lmaxneighok = .false.
                  nneigh(nri) = nneigh(nri) + 1
                  nneigh(nrj) = nneigh(nrj) + 1
                elseif (nneigh(nri).ge.maxneigh.or.nneigh(nrj).ge.maxneigh.or..not.lmaxneighok) then
                  lmaxneighok = .false.
                  nneigh(nri) = nneigh(nri) + 1
                  nneigh(nrj) = nneigh(nrj) + 1
                else
...
!
                  ...
                  ineigh(1,nneigh(nrj),nrj) = - ivec1cell(1,ii)
                  ineigh(2,nneigh(nrj),nrj) = - ivec1cell(2,ii)
                  ineigh(3,nneigh(nrj),nrj) = - ivec1cell(3,ii)
...
!
      ...
      enddo
      deallocate(ntmp2)
      allocate(ntmp3(3_i4,maxneigh,nREBOatom))
      call isumall(ineigh,ntmp3,3_i4*nREBOatom*maxneigh,"ineigh","getREBOneighbour")
      do i = 1,nREBOatom
        ineigh(1:3,1:nneigh(i),i) = ntmp3(1:3,1:nneigh(i),i)
      enddo
      deallocate(ntmp3)
...
#ifdef TRACE
  call trace_out('getREBOneighbour')
#endif
!
  return
  end
...
!   6/10 In parallel maxneigh is now globalised to set overall value of lmaxneighok
!  11/10 Algorithm corrected to avoid duplicate images in serial non-spatial case.
!  10/15 ineigh added to arguments
!  10/15 rtlist changed to rtclist
!  12/17 Bug in ineigh for j fixed
!   2/18 Trace added
!   1/20 Checking of maxneigh corrected for two atom case when i = j
!
!  Conditions of use:
...
!
  ...
  real(dp),    dimension(:,:),   allocatable, save :: rtmp2
#ifdef TRACE
  call trace_in('getReaxFFneighbour')
#endif
!
!  Initialise lmaxneighok and nneigh
!
...
!  
  if (nprocs.gt.1) then
    ...
    ineigh(1:3,1:maxneigh,1:numat) = 0
...
100 continue
    ...
    if (status/=0) call outofmemory('getReaxFFneighbour','ivec')
...
!
    call rtclist(nvector,reaxFFcutoff,xvec,yvec,zvec,ivec,imax,jmax,kmax,nmiddle,maxvector)
    if (nvector.gt.maxvector) then
      ...
      if (status/=0) call deallocate_error('getReaxFFneighbour','ivec')
      goto 100
    endif
    if (nprocs.gt.1) then
!-----------------------------------------------------------------
              ...
              if (r2.lt.bR22i) then
                if (r2.lt.reaxFFrmaxpair(ind)**2) then
                  if (nneigh(i).ge.maxneigh.or..not.lmaxneighok) then
                    ...
                    nneigh(i) = nneigh(i) + 1
                    neighno(nneigh(i),i) = j
                    ineigh(1,nneigh(i),i) = ivec(1,ii)
                    ineigh(2,nneigh(i),i) = ivec(2,ii)
                    ineigh(3,nneigh(i),i) = ivec(3,ii)
                    rneigh(nneigh(i),i) = rij
                    xneigh(nneigh(i),i) = xji
                    yneigh(nneigh(i),i) = yji
                    zneigh(nneigh(i),i) = zji
            ...
            if (i.ne.j.or.ii.ne.nmiddle) then
              ...
              if (r2.lt.reaxFFrmaxpair(ind)**2) then
                if (r2.lt.bR22i.and.r2.lt.bR22j) then
                  if (i.eq.j.and.nneigh(i).ge.maxneigh-1.or..not.lmaxneighok) then
                    lmaxneighok = .false.
                    nneigh(i) = nneigh(i) + 1
                    nneigh(j) = nneigh(j) + 1
                  elseif (nneigh(i).ge.maxneigh.or.nneigh(j).ge.maxneigh.or..not.lmaxneighok) then
                    lmaxneighok = .false.
                    nneigh(i) = nneigh(i) + 1
                    nneigh(j) = nneigh(j) + 1
                  else
                    ...
                    ineigh(1,nneigh(j),j) = - ivec(1,ii)
                    ineigh(2,nneigh(j),j) = - ivec(2,ii)
                    ineigh(3,nneigh(j),j) = - ivec(3,ii)
...
!
      ...
      enddo
      deallocate(ntmp2)
      allocate(ntmp3(3_i4,maxneigh,numat))
      call isumall(ineigh,ntmp3,3_i4*numat*maxneigh,"ineigh","getReaxFFneighbour")
      do i = 1,numat
        ineigh(1:3,1:nneigh(i),i) = ntmp3(1:3,1:nneigh(i),i)
      enddo
      deallocate(ntmp3)
...
#ifdef TRACE
  call trace_out('getReaxFFneighbour')
#endif
!
  return
  end
!
  subroutine getEDIPneighbour(maxneigh,nbosptr,nneigh,neighno,rneigh, &
                              xneigh,yneigh,zneigh,ineigh,latomdone,lmaxneighok)
!
!  Finds neighbours for an EDIP potential. New version based on getReaxFFneighbour.
!
!  On entry : 
!
!  maxneigh      = size of neighbour arrays
...
!  lmaxneighok   = if .true. then dimension maxneigh was sufficient
!
!  11/17 Created from getReaxFFneighbour
!  12/17 Bug in ineigh for j fixed
!   2/18 Trace added
!   1/20 Checking of maxneigh corrected for two atom case when i = j
!
...
!
  ...
  use current,        only : xvec2cell, yvec2cell, zvec2cell
  use parallel,       only : procid, nprocs
  use EDIPdata,       only : EDIPrmaxpair, EDIPrmax, EDIPcutoff
  use spatialbo,      only : lspatialok => lspatialBOok
  use spatialbo,      only : natomcell => natomcellbo
  use spatialbo,      only : ncellsearch => ncellsearchbo
  use spatialbo,      only : nspcell => nspcellbo
...
!
  ...
  real(dp),    dimension(:,:),   allocatable, save :: rtmp2
#ifdef TRACE
  call trace_in('getEDIPneighbour')
#endif
!
!  Initialise lmaxneighok and nneigh
!
...
100 continue
    allocate(ivec(3_i4,maxvector),stat=status)
    if (status/=0) call outofmemory('getEDIPneighbour','ivec')
    allocate(xvec(maxvector),stat=status)
...
!  Find lattice vectors needed
!
    call rtclist(nvector,EDIPcutoff,xvec,yvec,zvec,ivec,imax,jmax,kmax,nmiddle,maxvector)
    if (nvector.gt.maxvector) then
      maxvector = nvector
      deallocate(zvec,stat=status)
      ...
      deallocate(ivec,stat=status)
      if (status/=0) call deallocate_error('getEDIPneighbour','ivec')
      goto 100
    endif
    if (nprocs.gt.1) then
!-----------------------------------------------------------------
...
!
      ...
      deallocate(ntmp2)
      allocate(ntmp3(3_i4,maxneigh,numat))
      call isumall(ineigh,ntmp3,3_i4*numat*maxneigh,"ineigh","getEDIPneighbour")
      do i = 1,numat
        ineigh(1:3,1:nneigh(i),i) = ntmp3(1:3,1:nneigh(i),i)
      enddo
      deallocate(ntmp3)
...
#ifdef TRACE
  call trace_out('getEDIPneighbour')
#endif
!
  return
  end

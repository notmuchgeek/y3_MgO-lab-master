...
!  nks(1)  = x shrinking factor
!  nks(2)  = y shrinking factor
!  nks(3)  = z shrinking factor
!
...
!   2/20 Option added to centre k point grid at the origin
!
...
!
!  Find point at which to insert any new k points such that they remain in order of configuration
!
  if (nkpt.gt.0) then
    i = 1
    lfound = .false.
...
!
!  Set pointer to where the gamma point will be located so that it can be corrected afterwards.
!
  if (.not.lksym) then
...
!
!  Make space for new k point - needs amending to check existing k points for duplicates
!
            do kk = nkpt,ninsert,-1
              xkpt(kk + 1) = xkpt(kk)
              ykpt(kk + 1) = ykpt(kk)
...
!   2/18 nxks, nyks, nzks converted to a single array
!   2/20 Option added to centre k point grid at the origin
!   6/20 Extra check on maxkpt added
!
!  Conditions of use:
!
!  GULP is available free of charge to academic institutions

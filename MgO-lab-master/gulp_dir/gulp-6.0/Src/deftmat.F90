...
!
  ...
  lint = (nvar.gt.0)
  if (ndcon.eq.0.and.nvar.eq.n3f) then
#ifdef TRACE
    call trace_out('deftmat')
#endif
    return
  endif
!
...
!
      if (ndcon.gt.0) then
        do k = 1,ndcon
          ii = ncdvarind(k)
          io = ncdfixind(k)
          if (idoptindex(i).eq.ii.and.idopttype(i).eq.ncdvartyp(k)) then
            if (ncdfixtyp(k).eq.idopt_dx) then
              cmat(3*io-2) = dconco(k)
            elseif (ncdfixtyp(k).eq.idopt_dy) then
              cmat(3*io-1) = dconco(k)
            elseif (ncdfixtyp(k).eq.idopt_dz) then
              cmat(3*io) = dconco(k)
            elseif (ncdfixtyp(k).eq.idopt_dradius) then
              cmat(nofa+io) = dconco(k)
            endif
          endif
        enddo
      endif

...
!   8/17 Charge second derivatives removed from distribution since they 
...
!
  ...
  if (status/=0) call outofmemory('threefc','lijdone')
  allocate(nuniquejptr(numat),stat=status)
  if (status/=0) call outofmemory('threefc','nuniquejptr')
  allocate(ivec(3_i4,maxvector),stat=status)
  if (status/=0) call outofmemory('threefc','ivec')
  allocate(xvec(maxvector),stat=status)
  if (status/=0) call outofmemory('threefc','xvec')
  allocate(yvec(maxvector),stat=status)
  if (status/=0) call outofmemory('threefc','yvec')
  allocate(zvec(maxvector),stat=status)
  if (status/=0) call outofmemory('threefc','zvec')
!
...
!
      ...
      if (status/=0) call deallocate_error('threefc','zvec')
      deallocate(yvec,stat=status)
      if (status/=0) call deallocate_error('threefc','yvec')
      deallocate(xvec,stat=status)
      if (status/=0) call deallocate_error('threefc','xvec')
      deallocate(ivec,stat=status)
      if (status/=0) call deallocate_error('threefc','ivec')
...
!
              do kk = 1,3
                ...
                d2cell(n11,n31,ncind13m) = d2cell(n11,n31,ncind13m) - ed12
                d2cell(n31,n11,ncind13p) = d2cell(n31,n11,ncind13p) - ed12
                d2cell(n21,n31,ncind23m) = d2cell(n21,n31,ncind23m) - ed13
...
!
                  ...
                  d2cell(n11,n32,ncind13m) = d2cell(n11,n32,ncind13m) + trm1
                  d2cell(n32,n11,ncind13p) = d2cell(n32,n11,ncind13p) + trm1
!
                  trm1 = trm11 + trm12 - e2d(6)*r23v(kk)*r23v(kl)
                  d2cell(n21,n32,ncind23m) = d2cell(n21,n32,ncind23m) + trm1
                  d2cell(n32,n21,ncind23p) = d2cell(n32,n21,ncind23p) + trm1
                  d2cell(n22,n31,ncind23m) = d2cell(n22,n31,ncind23m) - trm13
...
!
  ...
  if (status/=0) call deallocate_error('threefc','nuniquejptr')
  deallocate(lijdone,stat=status)
  if (status/=0) call deallocate_error('threefc','lijdone')
!
!  Timing
!
  time2 = g_cpu_time()

...
!
  ...
  if (index(word,'fvec').eq.1) goto 280
  return
!***************************************
...
!
    ...
    if (lmodco) then
      if (ndimen(ncurr).eq.3) then
        nebfinalxyz(1,i,ncurr) = mod(nebfinalxyz(1,i,ncurr)+1000.0_dp,1.0_dp)
        nebfinalxyz(2,i,ncurr) = mod(nebfinalxyz(2,i,ncurr)+1000.0_dp,1.0_dp)
        nebfinalxyz(3,i,ncurr) = mod(nebfinalxyz(3,i,ncurr)+1000.0_dp,1.0_dp)
      elseif (ndimen(ncurr).eq.2) then
        nebfinalxyz(1,i,ncurr) = mod(nebfinalxyz(1,i,ncurr)+1000.0_dp,1.0_dp)
        nebfinalxyz(2,i,ncurr) = mod(nebfinalxyz(2,i,ncurr)+1000.0_dp,1.0_dp)
      elseif (ndimen(ncurr).eq.1) then
...
!
  if (ndimen(ncurr).eq.3) then
    do i = 1,3
      read(nru,*,err=99)(rvrep(j,i),j=1,3)
      iline = iline + 1
    enddo
!     
...
!     
    if (scalefactor*units.ne.1.0_dp) then
      do i = 1,3
        rvrep(1,i) = scalefactor*units*rvrep(1,i)
        rvrep(2,i) = scalefactor*units*rvrep(2,i)
        rvrep(3,i) = scalefactor*units*rvrep(3,i)
      enddo
    endif
    call uncell3D(rvrep,nebfinalcell(1,ncurr),nebfinalcell(2,ncurr),nebfinalcell(3,ncurr), &
                        nebfinalcell(4,ncurr),nebfinalcell(5,ncurr),nebfinalcell(6,ncurr))
  elseif (ndimen(ncurr).eq.2) then
    do i = 1,2
      read(nru,*,err=99)(rvrep(j,i),j=1,3)
      iline = iline + 1
...
!    
    ...
    call uncell2D(rvrep,nebfinalcell(1,ncurr),nebfinalcell(2,ncurr),nebfinalcell(3,ncurr))
  elseif (ndimen(ncurr).eq.1) then
    read(nru,*,err=99)(rvrep(j,1),j=1,3)
    iline = iline + 1
!   
!  Scale cell if necessary
!   
    if (scalefactor*units.ne.1.0_dp) then
      rvrep(1,1) = scalefactor*units*rvrep(1,1)
      rvrep(2,1) = scalefactor*units*rvrep(2,1)
      rvrep(3,1) = scalefactor*units*rvrep(3,1)
    endif
    call uncell1D(rvrep,nebfinalcell(1,ncurr))
    nebfinalcell(1,ncurr) = abs(floats(1))*units
  endif
  lwordok = .true.
  return

  subroutine real2ad(e2a,e12ap,e12ad,lgrad1,lgrad2)
!
...
!
  if (reg2(ncf).le.reg1(ncf).or.nreg2.eq.0) then
#ifdef TRACE
    call trace_out('real2ad')
#endif
    return
  endif
!*******************************************
...
!  Subtract region 2a - perfect region 1 interaction
!
          call real2a1d(e1,nati,ntypi,xcl,ycl,zcl,qli,oci,rai,nmi,indm,d1x,d1y,d1z, &
...
!
        if (.not.lr2asym.or.(mode2a.eq.2.or.mode2a.eq.4)) then
          call real2a1d(e12ap,nati,ntypi,xcl,ycl,zcl,qli,oci,rai,nmi,indm,d1x,d1y,d1z, &
            d1xe,d1ye,d1ze,dx,dy,dz,locg1,locg2,1_i4,2_i4,neqi,nip,lg1)
        else
          e12ap2 = 0.0_dp
          call real2a1d(e12ap2,nati,ntypi,xcl,ycl,zcl,qli,oci,rai,nmi,indm,d1x,d1y,d1z, &
...
!
        if (.not.lr2asym.or.(mode2a.eq.2.or.mode2a.eq.4)) then
          call real2a1d(e12ad,nati,ntypi,xcl,ycl,zcl,qli,oci,rai,nmi,indm,d1x,d1y,d1z, &
            d1xe,d1ye,d1ze,dx,dy,dz,lgrad1,lgrad2,1_i4,1_i4,neqi,nip,.true.)
        else
          e12ad2 = 0.0_dp
          call real2a1d(e12ad2,nati,ntypi,xcl,ycl,zcl,qli,oci,rai,nmi,indm,d1x,d1y,d1z, &
...
!
  if (lr2asym.and.(mode2a.eq.1.or.mode2a.eq.3)) then
    call real2a1fd(e12ap,e12ad,lgrad1,lgrad2)
  endif
  lr2pf = .false.
  r2dmax = sqrt(r2dmax)
  time2 = g_cpu_time()

...
!
  ...
  use current,        only : nstrains, nrelf2a, nrela2f, neqv, qa, qf
  use derivatives
...
!
  ...
  if (lopi) then
    ...
    do n = 1,nqatoms(j)
      k = nqatomptr(n,j)
      ka = nrelf2a(k)
      if (nrela2f(ka).eq.k.and.ka.eq.i) then
        kx = 3*(k - 1) + 1
        ky = kx + 1
        kz = ky + 1

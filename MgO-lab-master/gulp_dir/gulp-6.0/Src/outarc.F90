...
!  4/09/15 natmol changed to ndefmol for defect case
! 16/09/16 hmssg changed to hmssgio
!  7/04/18 Output file name truncation removed
!  8/08/19 Local arcfile now sized using maxwordlength
!
!  Conditions of use:
!
...
!
  ...
  character(len=maxwordlength) :: arcfilel
  character(len=maxwordlength) :: arcfile2
  integer(i4)                  :: i
  integer(i4)                  :: iarcv
  integer(i4)                  :: ii
  integer(i4)                  :: incf
...
!
    if (lmovie) then
      write(ioout,'(/,''  MSI Archive (version '',i1,'') open for Movie as '',a,/)') iarcv,trim(arcfile2)
    else
      write(ioout,'(/,''  MSI Archive (version '',i1,'') written as '',a,/)') iarcv,trim(arcfile2)
    endif
    if (loutshell) then
...
!
    ...
    ind = 0
    do i = 1,16
      if (hmssgio(i,1).ne.' ') then
        ind = ind + 1
        spacegroup(ind:ind) = hmssgio(i,ncf)
      endif
    enddo
    if (ndim.eq.3) then

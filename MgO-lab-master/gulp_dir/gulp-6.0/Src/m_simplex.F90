...
CONTAINS
  ...
  use fitting,     only : ncycp, nfitptr, nfitt
  use fitting,     only : fitscale=>scale
...
!
  ntest = jcyc/ncycp
  ntest = jcyc - ntest*ncycp
  if (ntest.eq.0.and.ioproc) then
!
!  Output current parameters
!
    write(ioout,'(/,''  Current parameters :'')')
    do i = 1,nfitt
      write(ioout,'(2x,i6,2x,f30.8)') i,x(nfitptr(i))*fitscale(nfitptr(i))
    enddo
    write(ioout,'(/)')
    call gflush(ioout)
  endif

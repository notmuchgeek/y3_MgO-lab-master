...
!   2/18 Trace added
!   8/19 String setting modified to ensure sizes are compatible
!
!  Conditions of use:
!
...
!*********
        ...
        words(nword) = string(1:min(maxlinelength,maxwordlength))
        iorder = iorder + 1
        nlorder(iorder) = 1
      else
!***********
...
!
        ...
        floatwords(nfloat) = string(1:min(maxlinelength,maxwordlength))
...
!
  ...
  enddo
#ifdef TRACE
  call trace_out('linepronc')
#endif
!
  return
  end
!
  subroutine lineprown(line_in,iword,wordi,iline)
!
!  Subroutine searches a line for the iword'th word and returns
!  as a full line length strength.
!
!   2/17 Created from linepronc
!   2/18 Trace added
!
!  Conditions of use:
!
...
!  Passed variables
!
  character(len=maxlinelength)   :: line_in            ! Line that has been read in
  character(len=maxlinelength)   :: wordi              ! On return this is the iword'th word
  integer(i4)                    :: iword              ! Number of word being searched for
  integer(i4)                    :: iline              ! Line number
!
!  Local variables
!
  character(len=1)               :: c1
  ...
  integer(i4)                    :: nstart
  integer(i4)                    :: nsw
  integer(i4)                    :: nwordfound
  logical                        :: lfrst
  logical                        :: lspace
  logical                        :: lword
#ifdef TRACE
  call trace_in('lineprown')
#endif
!
!  Initialise return value of wordi
!
  wordi = ' '
!
  decipoint = '.'
...
!*******************
  ...
  if (iupper.eq.0) iupper = maxlinelength
  nwordfound = 0
  wordloop: do while (ilower.lt.iupper)
    lspace = .false.
    i = ilower - 1
!
!  Find space before alphanumeric character
...
!  Word  *
!*********
        nwordfound = nwordfound + 1
        if (nwordfound.eq.iword) then
          wordi = string
          exit wordloop
        endif
      endif

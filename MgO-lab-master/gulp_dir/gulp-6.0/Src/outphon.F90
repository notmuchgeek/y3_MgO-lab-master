  subroutine outphon(nlkpt,nukpt,leigloc,mtv)
!
...
!        constants. 
!   5/14 Trap for istep1 being 0 added
!  10/14 freq array now accessed from module
!   8/16 Option to weight phonon DOS by IR intensity added
!   6/18 Parallel I/O trap added
!   6/20 Argument ncfoc changed to mtv
!   1/21 Length of pword increased
!   2/21 Check on normalisation of PDOS added
!
!  Conditions of use:
!
...
!
  ...
  use dispersion
  use gulp_files
  use frequencies, only : freq, IRintensity
...
!  Passed variables
!
  integer(i4),                    intent(in)   :: mtv       ! Number of frequencies
  integer(i4),                    intent(in)   :: nlkpt     ! First k point
  integer(i4),                    intent(in)   :: nukpt     ! Last k point
  logical,                        intent(in)   :: leigloc   ! If true then eigenvectors were computed
!
!  Local variables
!
  character(len=1)                             :: blank(60)
  character(len=1)                             :: star(60)
  character(len=maxwordlength),           save :: pword
  integer(i4)                                  :: i
...
!
  ...
  if (lphono) then
    if (phonfile(1:1).ne.' '.and.lfrst) then
      ...
      iend = min(iend,maxwordlength-4_i4)
      pword = phonfile
...
!
!  Find maximum weight for projected DOS
!
          wmaxp = 0.0d0
          do i = 1,nbox
            wmaxp = max(wmaxp,wght(i))
          enddo
          if (wmaxp.gt.0.0_dp) then
            rwmaxp = 1.0_dp/wmaxp
          else
            rwmaxp = 0.0_dp
          endif
!
      ...
      if (status/=0) call deallocate_error('outphon','iatpro')
      deallocate(itmp,stat=status)
      if (status/=0) call deallocate_error('outphon','itmp')
    endif
!*****************************
...
!*****************************
    if (lphono) then
      ...
      if (phonfile(1:1).ne.' ') then
        if (lfrst) then
          write(ioout,'(''  Phonon density of states written as '',a)') pword
        else
          write(ioout,'(''  Phonon density of states appended to '',a)') pword
        endif
      else
        if (lfrst) then
...
!
      ...
      if (lphono) then
        ...
        if (phonfile(1:1).ne.' ') then
          if (lfrst) then
            write(ioout,'(''  Phonon dispersion written as '',a)') pword
          else
            write(ioout,'(''  Phonon dispersion appended to '',a)') pword
          endif
        else
          if (lfrst) then
...
!
  ...
  if (status/=0) call deallocate_error('outphon','wghtb')
  deallocate(wght,stat=status)
  if (status/=0) call deallocate_error('outphon','wght')
!
  return
  end

...
!
  ...
  if (status/=0) call outofmemory('manyp','npotikptr')
  allocate(npotjkptr(npote),stat=status)
  if (status/=0) call outofmemory('manyp','npotjkptr')
!
...
!
  ...
  if (status/=0) call deallocate_error('manyp','npotjkptr')
  deallocate(npotikptr,stat=status)
  if (status/=0) call deallocate_error('manyp','npotikptr')

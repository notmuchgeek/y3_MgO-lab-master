...
!   8/16 Modified so that eta is constant for SPME calculations
!  
!  Conditions of use:
!
...
!
!  Julian Gale, Curtin University, August 2016
!
  use g_constants
  use control
  use current
...
!********************************
  if (targetrradmax.gt.0.0_dp) then
    ...
    eta = seta*seta
  elseif (lspme.and.ndim.eq.3) then
    eta = rspeed0*sqrt(dble(numat))*pi*(0.25_dp*vol4pi)**2

  function lmatchpair(nat1,ntype1,nat2,ntype2,nspecpair,nat1pair,ntype1pair, &
                      nat2pair,ntype2pair,lwildcard)
!
...
!
  integer(i4)              :: npair
  integer(i4)              :: nwildcard1
  integer(i4)              :: nwildcard2
  logical                  :: lmatchpair1
  logical                  :: lmatchpair2
!
  lmatchpair = .false.
  npair = 0
!
!  Set wildcard numbers based on core or shell of each type
!
...
!
  do while (.not.lmatchpair.and.npair.lt.nspecpair)
    npair = npair + 1
    lmatchpair1 = (((nat1.eq.nat1pair(npair).or.(lwildcard.and.nat1pair(npair).eq.nwildcard1)) &
                  .and.(ntype1.eq.ntype1pair(npair).or.ntype1pair(npair).eq.0)).and. &
                   ((nat2.eq.nat2pair(npair).or.(lwildcard.and.nat2pair(npair).eq.nwildcard2)) &
                  .and.(ntype2.eq.ntype2pair(npair).or.ntype2pair(npair).eq.0)))
    lmatchpair2 = (((nat2.eq.nat1pair(npair).or.(lwildcard.and.nat2pair(npair).eq.nwildcard2)) &
                  .and.(ntype2.eq.ntype1pair(npair).or.ntype1pair(npair).eq.0)).and. &
                   ((nat1.eq.nat2pair(npair).or.(lwildcard.and.nat1pair(npair).eq.nwildcard1)) &
                  .and.(ntype1.eq.ntype2pair(npair).or.ntype2pair(npair).eq.0)))

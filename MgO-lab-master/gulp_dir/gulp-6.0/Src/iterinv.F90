...
#ifdef TRACE
  call trace_in('iterinv')
#endif
!
!  Set sparsity pattern equal to dense form
!
  maxnonzero = n
  allocate(jcoef(lda,maxnonzero),stat=status)
...
!
  call jcg(n,lda,maxnonzero,jcoef,A,B,C,iwksp,nw,wksp,iparm,rparm,info)
!

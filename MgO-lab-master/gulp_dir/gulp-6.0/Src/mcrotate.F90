...
!***************************
    do i = 1,ntrialatom
      ...
      xafrac(ii) = saverotated(1,i)
      yafrac(ii) = saverotated(2,i)
      zafrac(ii) = saverotated(3,i)
    enddo
...
!
        ...
        saverotated(1,ntrialatom) = xafrac(ii)
        saverotated(2,ntrialatom) = yafrac(ii)
        saverotated(3,ntrialatom) = zafrac(ii)
!
...
!
    if (ntrialatom.eq.0) then
#ifdef TRACE
      call trace_out('mcrotate')
#endif
      return
    endif
!
...
!
      call cart2frac(ndim,dx-xoff+xcent,dy-yoff+ycent,dz-zoff+zcent,rv, &
                     xafrac(ii),yafrac(ii),zafrac(ii),icx,icy,icz)
    enddo
    if (.not.lmodco) then
!
...
!
      ...
      do i = 1,ntrialatom
        ii = nptrtrialatom(i)
        xcent = xcent + xafrac(ii)
        ycent = ycent + yafrac(ii)
        zcent = zcent + zafrac(ii)
      enddo
      xcent = xcent / dble(ntrialatom)
      ycent = ycent / dble(ntrialatom)
      zcent = zcent / dble(ntrialatom)
!
      xoff = dmod(xcent+1000.0_dp,1.0_dp) - xcent
      yoff = dmod(ycent+1000.0_dp,1.0_dp) - ycent
      zoff = dmod(zcent+1000.0_dp,1.0_dp) - zcent
      do i = 1,ntrialatom
        ii = nptrtrialatom(i)
        xafrac(ii) = xafrac(ii) + xoff
        yafrac(ii) = yafrac(ii) + yoff
        zafrac(ii) = zafrac(ii) + zoff
      enddo
    endif
  endif
#ifdef TRACE

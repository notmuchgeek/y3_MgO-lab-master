  subroutine specialcom(loptm,lcheck)
!
!  Check which rigid molecule centre of mass variables are allowed to move due to special positions
...
!
    xcom = molcom(1,nmf)*r1x + molcom(2,nmf)*r2x + molcom(3,nmf)*r3x
    ycom = molcom(1,nmf)*r1y + molcom(2,nmf)*r2y + molcom(3,nmf)*r3y
    zcom = molcom(1,nmf)*r1z + molcom(2,nmf)*r2z + molcom(3,nmf)*r3z
!
!  Set up quaternion matrix
!
    call setquaternion(nmf,molQa(1,nm),rQ,drQ,drQ2,.false.,.false.)
!
    ltmploc(1:3) = .true.
    lpossok(1:nposs) = .true.
!
...
!
      ...
      neqvna = neqv(na)
!*********************************************************
!  First pass - individual x, y and z for centre of mass *
!*********************************************************
      iiloop: do ii = 1,3
        xx(1) = xorig
        xx(2) = yorig
...
!*********************************************
      if (nposs.gt.0) then
        npossloop: do ii = 1,nposs
          ...
          xx(3) = zorig + zposs(ii)
!
!  Only test combinations where shifted coordinates haven't already been flagged for optimisation
!
          lxposs = (abs(xposs(ii)).gt.1.0d-6)
          lyposs = (abs(yposs(ii)).gt.1.0d-6)
          lzposs = (abs(zposs(ii)).gt.1.0d-6)
          if (ltmploc(1).and.lxposs) then
            lpossok(ii) = .false.
            cycle npossloop
          endif
          if (ltmploc(2).and.lyposs) then
            lpossok(ii) = .false.
            cycle npossloop
          endif
          if (ltmploc(3).and.lzposs) then
            lpossok(ii) = .false.
            cycle npossloop
          endif
!****************************
...
!
    enddo mloop
!
!  Count number possibilities that are OK
!
    npossok = 0
    do ii = 1,nposs
      if (lpossok(ii)) then
...
!
            ...
            do k = 1,ncon
              ...
              if (ncvartypcfg(ncfst+k).eq.iopt_xcom.and.ncfixtypcfg(ncfst+k).eq.iopt_ycom.and. &
                  nvk.eq.nm.and.nfk.eq.nm) then
                lfoundxy = .true.
                nxy1 = k
              elseif (ncvartypcfg(ncfst+k).eq.iopt_ycom.and.ncfixtypcfg(ncfst+k).eq.iopt_xcom.and. &
                  nvk.eq.nm.and.nfk.eq.nm) then
                lfoundxy = .true.
                nxy1 = k
              elseif (ncvartypcfg(ncfst+k).eq.iopt_ycom.and.ncfixtypcfg(ncfst+k).eq.iopt_zcom.and. &
                  nvk.eq.nm.and.nfk.eq.nm) then
                lfoundyz = .true.
                nyz1 = k
              elseif (ncvartypcfg(ncfst+k).eq.iopt_zcom.and.ncfixtypcfg(ncfst+k).eq.iopt_ycom.and. &
                  nvk.eq.nm.and.nfk.eq.nm) then
                lfoundyz = .true.
                nyz1 = k
              elseif (ncvartypcfg(ncfst+k).eq.iopt_xcom.and.ncfixtypcfg(ncfst+k).eq.iopt_zcom.and. &
                  nvk.eq.nm.and.nfk.eq.nm) then
                lfoundxz = .true.
                nxz1 = k
              elseif (ncvartypcfg(ncfst+k).eq.iopt_zcom.and.ncfixtypcfg(ncfst+k).eq.iopt_xcom.and. &
                  nvk.eq.nm.and.nfk.eq.nm) then
                lfoundxz = .true.
                nxz1 = k
              endif
...
!
            elseif (lfoundxy.and.lfoundxz) then
              ...
              if (nvk1.ne.nvk2) then
                ...
                call outerror('Badly defined constraints for '//'molecule '//molno,0_i4)
                call stopnow('specialcom')
              endif
              lfound = .true.
              if (ncvartyp(nxy1).eq.iopt_xcom) then
                ltmploc(1) = .true.
              elseif (ncvartyp(nxy1).eq.iopt_ycom) then
                ltmploc(2) = .true.
              elseif (ncvartyp(nxy1).eq.iopt_zcom) then
                ltmploc(3) = .true.
              endif
            ...
            elseif (lfoundxz.and.lfoundyz) then
              ...
              lfound = .true.
              if (ncvartyp(nxz1).eq.iopt_xcom) then
                ltmploc(1) = .true.
              elseif (ncvartyp(nxz1).eq.iopt_ycom) then
                ltmploc(2) = .true.
              elseif (ncvartyp(nxz1).eq.iopt_zcom) then
                ltmploc(3) = .true.
              endif
!
!  If constraints have been found for one pair
!
            elseif (lfoundxy.and.loptm(1,nm).and.loptm(2,nm)) then
              ncv = ncvarind(nxy1)
              if (ncv.eq.nm.and.ncvartyp(nxy1).eq.iopt_xcom) then
                ...
                ltmploc(1) = .true.
                ncon = ncon + 1
                ncvarindcfg(ncfst+ncon) = nm
                ncvartypcfg(ncfst+ncon) = iopt_xcom
                ncfixindcfg(ncfst+ncon) = nm
                ncfixtypcfg(ncfst+ncon) = iopt_zcom
                ...
                ncon = ncon + 1
                ncvarindcfg(ncfst+ncon) = nm
                ncvartypcfg(ncfst+ncon) = iopt_ycom
                ncfixindcfg(ncfst+ncon) = nm
                ncfixtypcfg(ncfst+ncon) = iopt_zcom
            ...
            elseif (lfoundyz.and.loptm(2,nm).and.loptm(3,nm)) then
              ncv = ncvarind(nyz1)
              if (ncv.eq.nm.and.ncvartyp(nyz1).eq.iopt_ycom) then
                if (ncontot.ge.maxcontot) then
                  maxcontot = ncontot + 10
                  call changemaxcontot
                endif
                ...
                ncvartypcfg(ncfst+ncon) = iopt_ycom
                ncfixindcfg(ncfst+ncon) = nm
                ncfixtypcfg(ncfst+ncon) = iopt_xcom
                ...
                ncon = ncon + 1
                ncvarindcfg(ncfst+ncon) = nm
                ncvartypcfg(ncfst+ncon) = iopt_zcom
                ncfixindcfg(ncfst+ncon) = nm
                ncfixtypcfg(ncfst+ncon) = iopt_xcom
            ...
            elseif (lfoundxz.and.loptm(1,nm).and.loptm(3,nm)) then
              ncv = ncvarind(nxz1)
              if (ncv.eq.nm.and.ncvartyp(nxz1).eq.iopt_xcom) then
                if (ncontot.ge.maxcontot) then
                  maxcontot = ncontot + 10
                  call changemaxcontot
                endif
                ...
                ncvartypcfg(ncfst+ncon) = iopt_xcom
                ncfixindcfg(ncfst+ncon) = nm
                ncfixtypcfg(ncfst+ncon) = iopt_ycom
...
!
            ...
            do k = 1,ncon
              nvk = ncvarindcfg(ncfst+k)
              nfk = ncfixindcfg(ncfst+k)
              if (ncvartypcfg(ncfst+k).eq.iopt_ycom.and.ncfixtypcfg(ncfst+k).eq.iopt_zcom.and. &
                  nvk.eq.nm.and.nfk.eq.nm) then
                lfound = .true.
                ltmploc(2) = .true.
                coeff = zposs(ii)*ryp
...
!
            ...
            do k = 1,ncon
              nvk = ncvarindcfg(ncfst+k)
              nfk = ncfixindcfg(ncfst+k)
              if (ncvartypcfg(ncfst+k).eq.iopt_xcom.and.ncfixtypcfg(ncfst+k).eq.iopt_zcom.and. &
                  nvk.eq.nm.and.nfk.eq.nm) then
                lfound = .true.
                ltmploc(1) = .true.
                coeff = zposs(ii)*rxp
...
!
      if (loptm(1,nm).and..not.ltmploc(1)) then
        call itow(molno,nm,6_i4)
        call outerror('Badly defined flags for '//'molecule '//molno,0_i4)
        call stopnow('specialcom')
      endif
      if (loptm(2,nm).and..not.ltmploc(2)) then
        call itow(molno,nm,6_i4)
        call outerror('Badly defined flags for '//'molecule '//molno,0_i4)
        call stopnow('specialcom')
      endif
      if (loptm(3,nm).and..not.ltmploc(3)) then
        call itow(molno,nm,6_i4)
        call outerror('Badly defined flags for '//'molecule '//molno,0_i4)
        call stopnow('specialcom')
      endif
...
!
      if (loptm(1,nm)) loptm(1,nm) = ltmploc(1)
      if (loptm(2,nm)) loptm(2,nm) = ltmploc(2)
      if (loptm(3,nm)) loptm(3,nm) = ltmploc(3)
    endif
!

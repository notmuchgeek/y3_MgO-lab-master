...
!  12/20 Correction to call for psiscreenderv by passing nreli rather than i
!
!  Conditions of use:
!
...
!
  emany = 0.0_dp
  do i = 1,nasym
    neamspeci = neamfnspecptr(nrela2f(i))
    rhoi = scrho(1,i)
...
!
  ...
  if (lgrad2) then
    ...
    if (status/=0) call outofmemory('manysd2','npotikptr')
    allocate(npotjkptr(npote),stat=status)
    if (status/=0) call outofmemory('manysd2','npotjkptr')
  endif
!
!  Set up cutoffs
!
...
!
      if (lMEAMfn) then
        frho(1:maxmeamcomponent) = scrho(1:maxmeamcomponent,nrelj)
...
!
                    ...
                    if (lstr) then
                      do is = 1,nstrains
                        ns = nstrptr(is)
                        derivrs(is) = derivrs(is) + d1bas*Sij*dr2ds(ns)
                      enddo
                      if (lgrad2) then
!
...
!
                    call psiscreenderv(nreli,j,npartial,partial,xcd,ycd,zcd,ebas,Sij,lstr)
                  endif
                endif
!
...
!
                        ...
                        call symdervrot(l,nrela2f(la),derivl)
                        xdrv(la) = xdrv(la) + derivl(1)
                        ydrv(la) = ydrv(la) + derivl(2)
                        zdrv(la) = zdrv(la) + derivl(3)
!
                        if (lstr) then
                          rstrl(1:nstrains) = rstrl(1:nstrains) + partial%sa_drhotots(1:nstrains,np)*ofct
                        endif
                      enddo
...
!
          ...
          if (lgrad2) then
            ...
            enddo
            if (lstr) then
              deriv2s(1:nstrains2) = deriv2s(1:nstrains2) + ofct*rscrhoi*drhototij2s(1:nstrains2) &
                                                          + ofct*rscrhoj*drhototji2s(1:nstrains2)
              deriv2m(1:nstrains,1:3) = deriv2m(1:nstrains,1:3) + ofct*rscrhoi*drhototij2m(1:nstrains,1:3) &
                                                                + ofct*rscrhoj*drhototji2m(1:nstrains,1:3)
              ind = 0
              do kl = 1,nstrains
                do km = 1,kl
                  ind = ind + 1
...
!*****************************
          if (lsg1.or.lgrad2) then
            rstrl(1:nstrains)  = rstrl(1:nstrains)  + derivs(1:nstrains)
            rstrlr(1:nstrains) = rstrlr(1:nstrains) + derivrs(1:nstrains)
!******************************
...
!
            if (lMEAMfn) then
              frho(1:maxmeamcomponent) = scrho(1:maxmeamcomponent,nrelk)
...
!
                      if (lMEAMden) then
                        call meamrho(nati,ntypi,natk,ntypk,rik,rpot(m),xcd1,ycd1,zcd1,rhoik,rhoki,drhoik,drhoki, &
                                     ...
                                     1.0_dp,1.0_dp,.true.,lstr,.true.,.false.,twopot(1,m))
              ...
              if (lanyvalidik) then
                if (j.ne.nreli) then
                  if (lMEAM) then
                    ...
                    dt3 = rscrhoi*ofctijk
                    do kl = 1,nstrains
                      derv3(ix,kl) = derv3(ix,kl) - dt1*drhototikssum(kl)*drhototij(1)
                      derv3(iy,kl) = derv3(iy,kl) - dt1*drhototikssum(kl)*drhototij(2)
                      derv3(iz,kl) = derv3(iz,kl) - dt1*drhototikssum(kl)*drhototij(3)
!
                      derv3(ix,kl) = derv3(ix,kl) - dt3*drhototijk2msum(kl,1)
                      derv3(iy,kl) = derv3(iy,kl) - dt3*drhototijk2msum(kl,2)
                      derv3(iz,kl) = derv3(iz,kl) - dt3*drhototijk2msum(kl,3)
                    enddo
                  else
                    dt1 = rscrhoi3*ofctijk
                    do kl = 1,nstrains
...
!
                if (lMEAM) then
                  do kl = 1,nstrains
                    do km = 1,nstrains
                      elcom(km,kl) = elcom(km,kl) + 0.5_dp*dt1*drhototikssum(km)*drhototijs(kl)
                      elcom(km,kl) = elcom(km,kl) + 0.5_dp*dt1*drhototikssum(kl)*drhototijs(km)
                      elcom(km,kl) = elcom(km,kl) + 0.5_dp*dt3*drhototijk2ssum(km,kl)
                      elcom(km,kl) = elcom(km,kl) + 0.5_dp*dt3*drhototijk2ssum(kl,km)
                    enddo
                  enddo
                else
                  do kl = 1,nstrains
              ...
              if (lanyvalidjk) then
                if (j.ne.nreli) then
                  if (lMEAM) then
                    ...
                    dt4 = rscrhoj*ofctijk
                    do kl = 1,nstrains
                      derv3(ix,kl) = derv3(ix,kl) - dt2*drhototjkssum(kl)*drhototji(1)
                      derv3(iy,kl) = derv3(iy,kl) - dt2*drhototjkssum(kl)*drhototji(2)
                      derv3(iz,kl) = derv3(iz,kl) - dt2*drhototjkssum(kl)*drhototji(3)
!
                      derv3(ix,kl) = derv3(ix,kl) - dt4*drhototjik2msum(kl,1)
                      derv3(iy,kl) = derv3(iy,kl) - dt4*drhototjik2msum(kl,2)
                      derv3(iz,kl) = derv3(iz,kl) - dt4*drhototjik2msum(kl,3)
                    enddo
                  else
                    dt2 = rscrhoj3*ofctijk
                    do kl = 1,nstrains
...
!
                if (lMEAM) then
                  do kl = 1,nstrains
                    do km = 1,nstrains
                      elcom(km,kl) = elcom(km,kl) + 0.5_dp*dt2*drhototjkssum(km)*drhototjis(kl)
                      elcom(km,kl) = elcom(km,kl) + 0.5_dp*dt2*drhototjkssum(kl)*drhototjis(km)
                      elcom(km,kl) = elcom(km,kl) + 0.5_dp*dt4*drhototjik2ssum(km,kl)
                      elcom(km,kl) = elcom(km,kl) + 0.5_dp*dt4*drhototjik2ssum(kl,km)
                    enddo
                  enddo
                else
                  do kl = 1,nstrains
...
!
  if (.not.lfreeze) then
    if (lsg1) then
      if (lMEAM) then
        do i = 1,nstrains
          rstrd(i) = rstrd(i) + 0.5_dp*(rstrl(i) + rstrlr(i))
        enddo
      else
        do i = 1,nstrains
    ...
    if (lsg1) then
      if (lMEAM) then
        do i = 1,nstrains
          rstrd(i) = rstrd(i) + (rstrl(i) + rstrlr(i))
          rstrd(i)   = rstrd(i)   + rstrlr(i)
        enddo
      else
        do i = 1,nstrains
...
!
  if (lgrad2) then
    ...
    if (status/=0) call deallocate_error('manysd2','npotjkptr')
    deallocate(npotikptr,stat=status)
    if (status/=0) call deallocate_error('manysd2','npotikptr')
  endif

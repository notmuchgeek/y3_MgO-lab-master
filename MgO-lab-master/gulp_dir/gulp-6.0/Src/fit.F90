...
!   9/15 Bond order fitting flags changed
!   3/17 Modifications made to allow for new variable order in iopt
!   7/17 Call to setoptptr added after setting of lfreeze
!   1/18 Trace added
...
!   9/19 Displacing of shells from zero stopped by default
!   9/19 ReaxFF charge arrays moved to reaxFFdata
!  11/20 Tersoff reorganised
!   1/20 Correction to setting of scale for bond order omega/lambda
!
!  Conditions of use:
!
...
!
  ...
  lfreqout = .false.
!
!  If simultaneous optimisation of shells and fitting add variables to fitting parameters
!
  if (index(keyword,'simu').gt.0.and..not.lrelax) then
    do nc = 1,ncfg
      ...
      lfcphon(nc) = .false.
!
!  Loop over atom coordinates
!
      do i = ninternalmin,ninternalmax
        io = ioptindex(i)
        it = iopttype(i)
        if (it.eq.iopt_xf.or.it.eq.iopt_yf.or.it.eq.iopt_zf) then
!
!  Shell coordinates
!
          if (lsymopt) then
            if (iatn(io).gt.maxele) then
              nfit = nfit + 1
...
!*****************************
  ...
  nfhwords = nfit*(nfit+1)/2
  allocate(fhess(nfhwords),stat=status)
...
!
        ...
        if (nvs.eq.iopt_xf) then
          scale(i) = xcfg(nsft+nv)
        elseif (nvs.eq.iopt_yf) then
          scale(i) = ycfg(nsft+nv)
        elseif (nvs.eq.iopt_zf) then
          scale(i) = zcfg(nsft+nv)
        endif
...
!
        scale(i) = onepot(nv)
      elseif (np.eq.39) then
!
!  Gasteiger species parameter A
!
        scale(i) = gastspec(1,nv)
      elseif (np.eq.40) then
!
!  Gasteiger species parameter B
!
        scale(i) = gastspec(2,nv)
      elseif (np.eq.41) then
!
!  Gasteiger species parameter C
!
        scale(i) = gastspec(3,nv)
      elseif (np.eq.42) then
!
!  Gasteiger species parameter Q0
!
        scale(i) = gastspec(4,nv)
      endif
    elseif (nt.eq.2) then
!-----------------------
!  Two-body variables  -
...
!
        scale(i) = BOccoeffR(1,np)
      elseif (nv.eq.12) then
!
!  Repulsive BO theta c2
!
        scale(i) = BOccoeffR(2,np)
      elseif (nv.eq.13) then
!
!  Repulsive BO theta c3
!
        scale(i) = BOccoeffR(3,np)
      elseif (nv.eq.14) then
!
!  Repulsive BO theta c4
!
        scale(i) = BOccoeffR(4,np)
      elseif (nv.eq.15) then
!
!  Repulsive BO theta c5
!
        scale(i) = BOccoeffR(5,np)
      elseif (nv.eq.16) then
!
...
!
  do i = 1,nfit
    nt = nftyp(i)
    np = nfpot(i)
    if (scale(i).eq.0.0_dp.and.(nt.ne.1.or.np.gt.2)) then
      nwarn = nwarn + 1
...
!
      ifail = - 2
      call fitbfgs(xctmp,fsumsq,gc,fxtol,fftol,fgtol,maxfcal,fhess,nfhwords,lhess2D,ifail)
    endif
  endif
  if (ioproc) then
!
...
!
  ...
  if (status/=0) call deallocate_error('fit','fhess')
!
...
!
  call outpot
!
!  Return structural parameters to original values at the end of a relax fitting run
!
  if (lrelax) then
    ncfold = 0
    do i = 1,nobs
...
!
          if (iatn(ind).le.maxele) then
            dum = fobs(i)
            if (it.eq.iopt_xf) then
              xcfg(nsft+ind) = dum
            elseif (it.eq.iopt_yf) then
              ycfg(nsft+ind) = dum
            elseif (it.eq.iopt_zf) then
              zcfg(nsft+ind) = dum
            endif
!
...
!
        ...
        elseif (it.eq.iopt_xcom.or.it.eq.iopt_ycom.or.it.eq.iopt_zcom) then
!
!  Centre of mass coordinate
!
          dum = fobs(i)
          if (it.eq.iopt_xcom) then
            molcoma(1,ind) = dum
          elseif (it.eq.iopt_ycom) then
            molcoma(2,ind) = dum
          elseif (it.eq.iopt_zcom) then
            molcoma(3,ind) = dum
          endif
!
!  Take care of constraints etc by calling setup
!
          call setup(.true.)
        elseif (it.eq.iopt_xqtn.or.it.eq.iopt_yqtn.or.it.eq.iopt_zqtn) then
!
!  Quaternions
!
          dum = fobs(i)
          if (it.eq.iopt_xqtn) then
            molQa(1,ind) = dum
          elseif (it.eq.iopt_yqtn) then
            molQa(2,ind) = dum
          elseif (it.eq.iopt_zqtn) then
            molQa(3,ind) = dum
          endif
!     
!  Take care of constraints etc by calling setup
...
!
  ...
  if (status/=0) call deallocate_error('fit','fres')
  deallocate(wvec,stat=status)
  if (status/=0) call deallocate_error('fit','wvec')
  deallocate(xctmp,stat=status)
  if (status/=0) call deallocate_error('fit','xctmp')
  deallocate(xcoriginal,stat=status)
  if (status/=0) call deallocate_error('fit','xcoriginal')
  deallocate(xc,stat=status)
  if (status/=0) call deallocate_error('fit','xc')
  deallocate(gc,stat=status)
  if (status/=0) call deallocate_error('fit','gc')
!
!  Timing
!
  time1 = g_cpu_time()

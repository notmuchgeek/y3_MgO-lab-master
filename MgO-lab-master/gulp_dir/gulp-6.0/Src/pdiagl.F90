  subroutine pdiagl(nk,mcv,maxd2,derv2,dervi,eigr,eigi,freq,fscale,lvectors,lprint,ifail)
!
...
!   8/14 k point index passed as argument
!   2/18 Trace added
!
!  On entry :
!
...
!
    ...
    if (status/=0) call outofmemory('pdiagl','iw')
    allocate(w1(lwork),stat=status)
    if (status/=0) call outofmemory('pdiagl','w1')
    allocate(cwork(lcwork),stat=status)
    if (status/=0) call outofmemory('pdiagl','cwork')
!
...
!
  ...
  if (status/=0) call deallocate_error('pdiagl','cwork')
  deallocate(w1,stat=status)
  if (status/=0) call deallocate_error('pdiagl','w1')
!
  t2 = g_cpu_time()
  tdiag = tdiag + t2 - t1
!
...
!
    call phoncopy2l(nk,derv2,dervi,eigr,maxd2,0_i4,0_i4,maxd2,mcv)
!

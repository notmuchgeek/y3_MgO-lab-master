...
!
  use current,        only : ndim, nstrains, nstrptr
  use eam
...
!
  ...
  integer(i4)                          :: is                  ! Looping index over strains
  integer(i4)                          :: np                  ! Looping index over partial screening atom
  integer(i4)                          :: ns                  ! Point from strain number to component
...
#ifdef TRACE
  call trace_in('meamtotalrhoscreenderv')
#endif
!
  if (m.gt.0) then
...
!
          do is = 1,nstrains
            ns = nstrptr(is)
            partial%sa_drhotots(is,np) = drhotot*Sij_rest*partial%sa_dSikjdr(1,np)*drij2ds(ns) + &
                                         drhotot*Sij_rest*partial%sa_dSikjdr(2,np)*drik2ds(ns) + &
                                         drhotot*Sij_rest*partial%sa_dSikjdr(3,np)*drjk2ds(ns)
          enddo
        endif
      enddo
!
...
!
      ...
      if (lstr) then
        partial%sa_drhotots(1:nstrains,1:npartial) = rscrhofn*partial%sa_drhotots(1:nstrains,1:npartial)
      endif
    endif
  endif
#ifdef TRACE
  call trace_out('meamtotalrhoscreenderv')
#endif
!
  return
  end

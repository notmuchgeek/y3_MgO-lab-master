...
!
  use configurations, only : rvcfg, straincfg
  use g_constants
  use control
  use costfunction
  use current
...
!
  do i = 1,nasym
    xafrac(i) = mod(xafrac(i)+10.0_dp,1.0_dp)
    yafrac(i) = mod(yafrac(i)+10.0_dp,1.0_dp)
    zafrac(i) = mod(zafrac(i)+10.0_dp,1.0_dp)
    if (xafrac(i).lt.0.0_dp) xafrac(i) = 1.0_dp + xafrac(i)
    if (yafrac(i).lt.0.0_dp) yafrac(i) = 1.0_dp + yafrac(i)
    if (zafrac(i).lt.0.0_dp) zafrac(i) = 1.0_dp + zafrac(i)
  enddo
  lfirst = .true.
!
...
!
  ...
  r3z = rv(3,3)
!
!  Since there is no symmetry copy asymmetric fractional coordinates to full set
!
  do i = 1,numat
    xfrac(i) = xafrac(i)
    yfrac(i) = yafrac(i)

  subroutine fourooppd(xkv,ykv,zkv)
!
...
!
!   6/13 Created from fourooppd.f
!  12/16 derv2dk added back
!   1/17 Completed and tested
...
#ifdef TRACE
  call trace_in('fourooppd')
#endif
!
!  Allocate local memory
!
  allocate(nuniqueptr(numat),stat=status)
  if (status/=0) call outofmemory('fourooppd','nuniqueptr')
  allocate(nuniquelocptr(numat),stat=status)
  if (status/=0) call outofmemory('fourooppd','nuniquelocptr')
!*************************
...
!***************************************************
!  Loop over middle site 1 / i - central atom case *
!***************************************************
    ix = -2
    iy = -1
    iz = 0
...
!
  ...
  if (status/=0) call deallocate_error('fourooppd','nuniquelocptr')
  deallocate(nuniqueptr,stat=status)
  if (status/=0) call deallocate_error('fourooppd','nuniqueptr')
!
!  All tidying up of derivatives is handled by four so we can just return here
!
#ifdef TRACE
  call trace_out('fourooppd')
#endif
  return
  end

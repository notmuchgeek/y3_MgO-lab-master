...
!  Zero cosmoA
!
  cosmoA(1:(npts*(npts+1)/2),1) = 0.0_dp
!
...
!
    if (lperiodic) then
      cosmoA(kk,1) = cosmoA(kk,1) + qmeself*swi*swi
    endif
!
...
!
  if (index(keyword,'amat').ne.0.and.ldebug.and.ioproc) then
    ...
    kk = 0
    do i = 1,npts
      write(ioout,'(10f12.6)')(cosmoA(kk+j,1),j=1,i)
      kk = kk + i
    enddo
  endif
!

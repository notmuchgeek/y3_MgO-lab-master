...
!   2/19 Format changed to satisfy Intel compiler
...
!
    ...
    if (nshell.ne.0.and.ladiabatic) then
      if (ioproc) then
        ...
        write(ioout,'(''  until the forces on the shells are less than '',d12.5,'' or for a maximum of '')') sgtol
...
!
    ...
    if (ldip.and.ioproc) then
      if (dipfile.ne.' ') then
        open(37,file=dipfile,status='unknown',form='formatted')
      else
        open(37)
      endif
    endif

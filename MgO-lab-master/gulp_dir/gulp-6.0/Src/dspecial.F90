...
!**************************************
  do na = 1,ndasym
    ...
    lgeneral = .false.
    indi = na - 1
    lopx = (iop(3*indi+1).eq.1)
    lopy = (iop(3*indi+2).eq.1)
    lopz = (iop(3*indi+3).eq.1)
...
!
          ...
          lfoundxz = .false.
          do k = 1,ndcon
            nvk = ncdvarind(ncfst+k)
            nfk = ncdfixind(ncfst+k)
            if (nvk.eq.indi.and.nfk.eq.indi) then
              if (ncdvartyp(ncfst+k).eq.idopt_dx.and.ncdfixtyp(ncfst+k).eq.idopt_dy) then
                lfoundxy = .true.
                nxy1 = k
              elseif (ncdvartyp(ncfst+k).eq.idopt_dy.and.ncdfixtyp(ncfst+k).eq.idopt_dx) then
                lfoundxy = .true.
                nxy1 = k
              elseif (ncdvartyp(ncfst+k).eq.idopt_dy.and.ncdfixtyp(ncfst+k).eq.idopt_dz) then
                lfoundyz = .true.
                nyz1 = k
              elseif (ncdvartyp(ncfst+k).eq.idopt_dz.and.ncdfixtyp(ncfst+k).eq.idopt_dy) then
                lfoundyz = .true.
                nyz1 = k
              elseif (ncdvartyp(ncfst+k).eq.idopt_dx.and.ncdfixtyp(ncfst+k).eq.idopt_dz) then
                lfoundxz = .true.
                nxz1 = k
              elseif (ncdvartyp(ncfst+k).eq.idopt_dz.and.ncdfixtyp(ncfst+k).eq.idopt_dx) then
                lfoundxz = .true.
                nxz1 = k
              endif
            endif
...
!
          elseif (lfoundxy.and.lfoundxz) then
            nvk1 = ncdvarind(nxy1)
            nvk2 = ncdvarind(nxz1)
            if (nvk1.ne.nvk2.and.ncdvartyp(nxy1).ne.ncdvartyp(nxz1)) then
          ...
          elseif (lfoundxy.and.lfoundyz) then
            nvk1 = ncdvarind(nxy1)
            nvk2 = ncdvarind(nyz1)
            if (nvk1.ne.nvk2.and.ncdvartyp(nxy1).ne.ncdvartyp(nyz1)) then
              call itow(atomno,na,6_i4)
              call outerror('Badly defined constraints for atom '//atomno,0_i4)
              call stopnow('dspecial')
            endif
          ...
          elseif (lfoundxz.and.lfoundyz) then
            nvk1 = ncdvarind(nxz1)
            nvk2 = ncdvarind(nyz1)
            if (nvk1.ne.nvk2.and.ncdvartyp(nxz1).ne.ncdvartyp(nyz1)) then
              call itow(atomno,na,6_i4)
              call outerror('Badly defined constraints for atom '//atomno,0_i4)
              call stopnow('dspecial')
            endif
...
!
          elseif (lfoundxy) then
            ncv = ncdvarind(nxy1)
            if (ncv.eq.indi.and.ncdvartyp(nxy1).eq.idopt_dx) then
              iop(indx) = 1
              ndcon = ndcon + 1
              ...
              ncdvarind(ncfst+ndcon) = indi
              ncdvartyp(ncfst+ndcon) = idopt_dx
              ncdfixind(ncfst+ndcon) = indi
              ncdfixtyp(ncfst+ndcon) = idopt_dz
              ...
              ncdvarind(ncfst+ndcon) = indi
              ncdvartyp(ncfst+ndcon) = idopt_dy
              ncdfixind(ncfst+ndcon) = indi
              ncdfixtyp(ncfst+ndcon) = idopt_dz
          ...
          elseif (lfoundyz) then
            ncv = ncdvarind(nyz1)
            if (ncv.eq.indi.and.ncdvartyp(nyz1).eq.idopt_dy) then
              iop(indy) = 1
              ndcon = ndcon + 1
              if (ndcon.ge.maxdcon) then
                maxdcon  =  ndcon + 10
              ...
              ncdvartyp(ncfst+ndcon) = idopt_dy
              ncdfixind(ncfst+ndcon) = indi
              ncdfixtyp(ncfst+ndcon) = idopt_dx
              ...
              ncdvarind(ncfst+ndcon) = indi
              ncdvartyp(ncfst+ndcon) = idopt_dz
              ncdfixind(ncfst+ndcon) = indi
              ncdfixtyp(ncfst+ndcon) = idopt_dx
          ...
          elseif (lfoundxz) then
            ncv = ncdvarind(nxz1)
            if (ncv.eq.indi.and.ncdvartyp(nxz1).eq.idopt_dx) then
              iop(indx) = 1
              ndcon = ndcon + 1
              if (ndcon.ge.maxdcon) then
                maxdcon  =  ndcon + 10
              ...
              ncdvartyp(ncfst+ndcon) = idopt_dx
              ncdfixind(ncfst+ndcon) = indi
              ncdfixtyp(ncfst+ndcon) = idopt_dy
...
!
          ...
          do k = 1,ndcon
            ...
            nfk = ncdfixind(ncfst+k)
            if (nvk.eq.indi.and.nfk.eq.indi) then
              if (ncdvartyp(ncfst+k).eq.idopt_dy.and.ncdfixtyp(ncfst+k).eq.idopt_dz) then
                lfound = .true.
                iop(indy) = 1
                coeff = zposs(ii)*ryp
                diff = abs(coeff-dconco(ncfst+k))
...
!
          ...
          do k = 1,ndcon
            ...
            nfk = ncdfixind(ncfst+k)
            if (nvk.eq.indi.and.nfk.eq.indi) then
              if (ncdvartyp(ncfst+k).eq.idopt_dx.and.ncdfixtyp(ncfst+k).eq.idopt_dz) then
                lfound = .true.
                iop(indx) = 1
                coeff = zposs(ii)*rxp
                diff = abs(coeff-dconco(ncfst+k))

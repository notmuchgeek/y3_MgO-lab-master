...
!   5/14 Trap added for conflict between conv keyword and mcstrain
!   4/16 nmcswappair added
!   4/16 Correction to only/any logic for mcswap
...
!
!  Check that this is not a conv run
!
  if (lconv) then
    call outerror('strain probability cannot be used in a conv calculation',iline)
    call stopnow('mcword')
  endif
  pstrain = floats(1)
  lwordok = .true.
...
!
!  Increment number of mcswaps
!
  nmcswaps = nmcswaps + 1
!
!  Check dimensions
!
  if (nmcswaps.gt.maxmcswaps) then
    maxmcswaps = nmcswaps + 2_i4
    call changemaxmcswaps
  endif
!
  pswap(nmcswaps) = floats(1)
  pswapsum = pswapsum + pswap(nmcswaps)
  if (nfloat.gt.1) then
    nmcswappair(nmcswaps) = abs(nint(floats(2)))
  endif
!
!  Are any sub-options specified?
!
  if (nword.gt.0) then
    if (index(words(2),'only').eq.0) then
!
!  Any sub-option
!
      lmcswapany(nmcswaps) = .true.
    else
!
!  Only sub-option
!
      lmcswapany(nmcswaps) = .false.
      nword1 = 1
      if (index(words(1),'onl').eq.1) nword1 = 2
!
!  Read species list
!
      nmcswapspec(nmcswaps) = 0
      do i = nword1,nword
        call worsy(words(i),lsymbol,.false.)
        if (lsymbol) then
          nmcswapspec(nmcswaps) = nmcswapspec(nmcswaps) + 1
!
!  Check that there is space in the array for this species
!
          if (nmcswapspec(nmcswaps).gt.maxmcswapspec) then
            maxmcswapspec = nmcswapspec(nmcswaps) + 5
...
!
          ...
          nmcswapnat(nmcswapspec(nmcswaps),nmcswaps) = inat
          nmcswaptype(nmcswapspec(nmcswaps),nmcswaps) = itype
        endif
      enddo
!
!  If number of actual species is zero then stop
!
      if (nmcswapspec(nmcswaps).eq.0) then

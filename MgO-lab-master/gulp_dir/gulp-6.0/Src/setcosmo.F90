...
!   5/16 Reference to nspecptr with nrel2 corrected to nrelat
!   9/19 nrel2 / nrelat changed to nrela2f / nrelf2a
!
  use cosmic
  use current
  use element
  use species, only : lvdwinspec, vdwspec
  implicit none
!
!  Local variables
!
...
!
  do i = 1,nasym
    ...
    elseif (lvdwinspec(ns).and.(vdwspec(ns).lt.0.1_dp.or.vdwspec(ns).gt.10.0_dp)) then
      call outerror('VDW radius for element '//atsym(iat)//' is not suited for a COSMO run',0_i4)
      call stopnow('setcosmo')
    endif
  enddo
...
!
  do i = 1,numat
    ...
    ns = nspecptr(nrelf2a(i))
    if (lvdwinspec(ns)) then
      atsrad(i) = vdwspec(ns) + rsolv - drsolv
    else

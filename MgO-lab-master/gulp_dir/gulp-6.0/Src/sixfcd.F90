...
!
  ...
  integer(i4)                                  :: ncindp(15)
  integer(i4)                                  :: ncindm(15)
  integer(i4)                                  :: nsixtype
  integer(i4)                                  :: ni
  integer(i4)                                  :: niimax
  integer(i4)                                  :: nj
...
!****************************************************
!  Loop over middle site 1 / i - local middle atom  *
!****************************************************
...
!
          ...
          icsxyz(1,2) = ivec1cell(1,ii)
          icsxyz(2,2) = ivec1cell(2,ii)
          icsxyz(3,2) = ivec1cell(3,ii)
!*****************************************
...
!
              ...
              icsxyz(1,3) = ivec1cell(1,jj)
              icsxyz(2,3) = ivec1cell(2,jj)
              icsxyz(3,3) = ivec1cell(3,jj)
!************************************
...
!
                  ...
                  icsxyz(1,4) = ivec1cell(1,ll)
                  icsxyz(2,4) = ivec1cell(2,ll)
                  icsxyz(3,4) = ivec1cell(3,ll)
!*****************************************
...
!
                      ...
                      icsxyz(1,5) = ivec1cell(1,ii) + ivec1cell(1,mm)
                      icsxyz(2,5) = ivec1cell(1,ii) + ivec1cell(2,mm)
                      icsxyz(3,5) = ivec1cell(1,ii) + ivec1cell(3,mm)
!************************************
...
!
                          ...
                          if (lmolok) then
                            if (ndim.eq.0) then
                              ...
                              if (lintra_only.and..not.lsamemol) cycle nnloop
                              if (linter_only.and.lsamemol) cycle nnloop
                            endif
                          endif
!
!  Distance checking
!
                          if (r622.gt.tr5.and.(.not.lbtyp.or..not.lbonded)) cycle nnloop
!
!  n has been accepted
!
                          isatom(6) = n
                          ...
                          icsxyz(1,6) = ivec1cell(1,ii) + ivec1cell(1,mn)
                          icsxyz(2,6) = ivec1cell(1,ii) + ivec1cell(2,mn)
                          icsxyz(3,6) = ivec1cell(1,ii) + ivec1cell(3,mn)
!********************************
...
!
                          do ivec = 1,6
                            do jvec = 1,6
                              isvec(1,jvec,ivec) = icsxyz(1,ivec) - icsxyz(1,jvec)
                              isvec(2,jvec,ivec) = icsxyz(2,ivec) - icsxyz(2,jvec)
                              isvec(3,jvec,ivec) = icsxyz(3,ivec) - icsxyz(3,jvec)
!
...
!  Find cell index
!
                              if (abs(isvec(1,jvec,ivec)).gt.nd2cell(1).or. &
                                  abs(isvec(2,jvec,ivec)).gt.nd2cell(2).or. &
                                  abs(isvec(3,jvec,ivec)).gt.nd2cell(3)) then
!
!  Find cell index - if outside user range then assign to central cell
!
                                ncindm(indvec) = nd2central
                                ncindp(indvec) = nd2central
                              else
!
!  Compute index
!
                                ncindp(indvec) = nd2cellptr(nd2cell(1)+1+isvec(1,jvec,ivec), &
                                                            nd2cell(2)+1+isvec(2,jvec,ivec), &
                                                            nd2cell(3)+1+isvec(3,jvec,ivec))
                                ncindm(indvec) = nd2cellptr(nd2cell(1)+1-isvec(1,jvec,ivec), &
                                                            nd2cell(2)+1-isvec(2,jvec,ivec), &
                                                            nd2cell(3)+1-isvec(3,jvec,ivec))
                              endif
...
!
                            if (ia.eq.1) then
                              d2cell(isxyz(1,ja),isxyzl(1),ncindm(ivec)) = &
                                d2cell(isxyz(1,ja),isxyzl(1),ncindm(ivec)) - e1d(ivec)
                              d2cell(isxyz(2,ja),isxyzl(2),ncindm(ivec)) = &
                                d2cell(isxyz(2,ja),isxyzl(2),ncindm(ivec)) - e1d(ivec)
                              d2cell(isxyz(3,ja),isxyzl(3),ncindm(ivec)) = &
                                d2cell(isxyz(3,ja),isxyzl(3),ncindm(ivec)) - e1d(ivec)
                            endif
                            if (ja.eq.1) then
                              d2cell(isxyz(1,ia),isxyzl(1),ncindp(ivec)) = &
                                d2cell(isxyz(1,ia),isxyzl(1),ncindp(ivec)) - e1d(ivec)
                              d2cell(isxyz(2,ia),isxyzl(2),ncindp(ivec)) = &
                                d2cell(isxyz(2,ia),isxyzl(2),ncindp(ivec)) - e1d(ivec)
                              d2cell(isxyz(3,ia),isxyzl(3),ncindp(ivec)) = &
                                d2cell(isxyz(3,ia),isxyzl(3),ncindp(ivec)) - e1d(ivec)
                            endif
!
                            do jvec = ivec,15
                              indvec = indvec + 1 
                              ...
                              do ialp = 1,3
                                do ibet = 1,3
                                  ...
                                  if (ia.eq.1) then
                                    if (ia.gt.ib) then
                                      d2cell(isxyz(ibet,ib),isxyzl(ialp),ncindp(kb2(ib,ia))) =  &
                                        d2cell(isxyz(ibet,ib),isxyzl(ialp),ncindp(kb2(ib,ia))) + d2trm
                                    elseif (ia.lt.ib) then
                                      d2cell(isxyz(ibet,ib),isxyzl(ialp),ncindm(kb2(ib,ia))) =  &
                                        d2cell(isxyz(ibet,ib),isxyzl(ialp),ncindm(kb2(ib,ia))) + d2trm
                                    endif
!
                                    if (ia.gt.jb) then
                                      d2cell(isxyz(ibet,jb),isxyzl(ialp),ncindp(kb2(jb,ia))) = &
                                        d2cell(isxyz(ibet,jb),isxyzl(ialp),ncindp(kb2(jb,ia))) - d2trm
                                    elseif (ia.lt.jb) then
                                      d2cell(isxyz(ibet,jb),isxyzl(ialp),ncindm(kb2(jb,ia))) = &
                                        d2cell(isxyz(ibet,jb),isxyzl(ialp),ncindm(kb2(jb,ia))) - d2trm
                                    endif
                                  endif
                                  if (ja.eq.1) then
                                    if (ja.gt.jb) then
                                      d2cell(isxyz(ibet,jb),isxyzl(ialp),ncindp(kb2(jb,ja))) = &
                                        d2cell(isxyz(ibet,jb),isxyzl(ialp),ncindp(kb2(jb,ja))) + d2trm
                                    elseif (ja.lt.jb) then
                                      d2cell(isxyz(ibet,jb),isxyzl(ialp),ncindm(kb2(jb,ja))) = &
                                        d2cell(isxyz(ibet,jb),isxyzl(ialp),ncindm(kb2(jb,ja))) + d2trm
                                    endif
!
                                    if (ja.gt.ib) then
                                      d2cell(isxyz(ibet,ib),isxyzl(ialp),ncindp(kb2(ib,ja))) = &
                                        d2cell(isxyz(ibet,ib),isxyzl(ialp),ncindp(kb2(ib,ja))) - d2trm
                                    elseif (ja.lt.ib) then
                                      d2cell(isxyz(ibet,ib),isxyzl(ialp),ncindm(kb2(ib,ja))) = &
                                        d2cell(isxyz(ibet,ib),isxyzl(ialp),ncindm(kb2(ib,ja))) - d2trm
                                    endif
                                  endif
!
                                  if (ivec.ne.jvec) then
                                    if (ib.eq.1) then
                                      if (ia.gt.ib) then
                                        d2cell(isxyz(ialp,ia),isxyzl(ibet),ncindm(kb2(ia,ib))) = &
                                        d2cell(isxyz(ialp,ia),isxyzl(ibet),ncindm(kb2(ia,ib))) + d2trm
                                      elseif (ia.lt.ib) then
                                        d2cell(isxyz(ialp,ia),isxyzl(ibet),ncindp(kb2(ia,ib))) = &
                                        d2cell(isxyz(ialp,ia),isxyzl(ibet),ncindp(kb2(ia,ib))) + d2trm
                                      endif
!
                                      if (ja.gt.ib) then
                                        d2cell(isxyz(ialp,ja),isxyzl(ibet),ncindm(kb2(ja,ib))) = &
                                        d2cell(isxyz(ialp,ja),isxyzl(ibet),ncindm(kb2(ja,ib))) - d2trm
                                      elseif (ja.lt.ib) then
                                        d2cell(isxyz(ialp,ja),isxyzl(ibet),ncindp(kb2(ja,ib))) = &
                                        d2cell(isxyz(ialp,ja),isxyzl(ibet),ncindp(kb2(ja,ib))) - d2trm
                                      endif
                                    endif
!
                                    if (jb.eq.1) then
                                      if (ia.gt.jb) then
                                        d2cell(isxyz(ialp,ia),isxyzl(ibet),ncindm(kb2(ia,jb))) = &
                                        d2cell(isxyz(ialp,ia),isxyzl(ibet),ncindm(kb2(ia,jb))) - d2trm
                                      elseif (ia.lt.jb) then
                                        d2cell(isxyz(ialp,ia),isxyzl(ibet),ncindp(kb2(ia,jb))) = &
                                        d2cell(isxyz(ialp,ia),isxyzl(ibet),ncindp(kb2(ia,jb))) - d2trm
                                      endif
!
                                      if (ja.gt.jb) then
                                        d2cell(isxyz(ialp,ja),isxyzl(ibet),ncindm(kb2(ja,jb))) = &
                                        d2cell(isxyz(ialp,ja),isxyzl(ibet),ncindm(kb2(ja,jb))) + d2trm
                                      elseif (ja.lt.jb) then
                                        d2cell(isxyz(ialp,ja),isxyzl(ibet),ncindp(kb2(ja,jb))) = &
                                        d2cell(isxyz(ialp,ja),isxyzl(ibet),ncindp(kb2(ja,jb))) + d2trm
                                      endif
                                    endif
                                  endif
                                enddo
...
!
    ...
    enddo iloop
!*************************************************
!  Loop over middle site 1 / i - local end atom  *
!*************************************************
    iloop2: do i = 1,numat
      ni = nat(i)
      ntypi = nftype(i)
...
!  Loop over cell vectors
!
                        nnloop2: do mn = 1,niimax
                          x62 = x62t + xvec1cell(mn)
                          y62 = y62t + yvec1cell(mn)
                          z62 = z62t + zvec1cell(mn)
                          r622 = x62*x62 + y62*y62 + z62*z62
                          if (r622.lt.1d-10) cycle nnloop2
!
!  Prevent atoms i and n being the same atom
!
                          if (n.eq.i.and.mn.eq.iimid) cycle nnloop2
!
!  Prevent atoms j and n being the same atom
!
                          if (n.eq.j.and.mn.eq.ii) cycle nnloop2
!
!  Prevent atoms m and n being the same atom
!
                          if (n.eq.m.and.mn.eq.mm) cycle nnloop2
!
!  Reset lmolok to preloop value
!
                          lmolok = lmolokjn
...
!
                          ...
                          if (lmolok) then
                            if (ndim.eq.0) then
                              if (linter_only) cycle nnloop2
                              if (lbtyp) then
                                call bonded(lbonded,l2bonds,nbtypejn,nbtypejn2,j,n,0_i4,0_i4,0_i4)
                                if (.not.lbonded) cycle nnloop2
                              endif
                            else
                              call lintoijk(nxx,nyy,nzz,mn,imaxl,jmaxl,kmaxl)
                              if (lbtyp) then
                              ...
                              if (lintra_only.and..not.lsamemol) cycle nnloop2
                              if (linter_only.and.lsamemol) cycle nnloop2
                            endif
                          endif
!
!  Distance checking
!
                          if (r622.gt.tr5.and.(.not.lbtyp.or..not.lbonded)) cycle nnloop2
!
!  n has been accepted
!
                          isatom(6) = n
...
!
                            if (ia.eq.3) then
                              if (ia.gt.ja) then
                                d2cell(isxyz(1,ja),isxyzl(1),ncindp(ivec)) = &
                                  d2cell(isxyz(1,ja),isxyzl(1),ncindp(ivec)) - e1d(ivec)
                                d2cell(isxyz(2,ja),isxyzl(2),ncindp(ivec)) = &
                                  d2cell(isxyz(2,ja),isxyzl(2),ncindp(ivec)) - e1d(ivec)
                                d2cell(isxyz(3,ja),isxyzl(3),ncindp(ivec)) = &
                                  d2cell(isxyz(3,ja),isxyzl(3),ncindp(ivec)) - e1d(ivec)
                              else
                                d2cell(isxyz(1,ja),isxyzl(1),ncindm(ivec)) = &
                                  d2cell(isxyz(1,ja),isxyzl(1),ncindm(ivec)) - e1d(ivec)
                                d2cell(isxyz(2,ja),isxyzl(2),ncindm(ivec)) = &
                            ...
                            if (ja.eq.3) then
                              if (ia.gt.ja) then
                                d2cell(isxyz(1,ia),isxyzl(1),ncindm(ivec)) = &
                                  d2cell(isxyz(1,ia),isxyzl(1),ncindm(ivec)) - e1d(ivec)
                                d2cell(isxyz(2,ia),isxyzl(2),ncindm(ivec)) = &
                                  d2cell(isxyz(2,ia),isxyzl(2),ncindm(ivec)) - e1d(ivec)
                                d2cell(isxyz(3,ia),isxyzl(3),ncindm(ivec)) = &
                                  d2cell(isxyz(3,ia),isxyzl(3),ncindm(ivec)) - e1d(ivec)
                              else
                                d2cell(isxyz(1,ia),isxyzl(1),ncindp(ivec)) = &
                                  d2cell(isxyz(1,ia),isxyzl(1),ncindp(ivec)) - e1d(ivec)
                                d2cell(isxyz(2,ia),isxyzl(2),ncindp(ivec)) = &

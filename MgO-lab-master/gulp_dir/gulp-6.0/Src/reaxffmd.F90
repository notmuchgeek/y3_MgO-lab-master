...
!   3/14 literativeQ replaced qiter in keyword check
!   8/14 nboatom now used to screen atoms without a reaxff type
!   8/15 Argument added to setreaxffQ calls
!   8/15 QMMM electrostatic modifications added
!  10/15 ineigh added
!   4/16 ldoregions flag added to dutils calls
!   6/17 Iterative charge solution forced for parallel case
...
!   8/17 Correction to the parallelisation of ebond
!   2/18 Trace added
...
!   8/20 Globalisation of deltap added to handle need for deltap of j as well as i
!   8/20 Correction to VDW handling so that fixed charge species are included
...
!
  ...
  if (status/=0) call outofmemory('reaxFF','nboatomptr')
  ...
  if (status/=0) call outofmemory('reaxFF','nbodummyptr')
!
...
!
  ...
  if (status/=0) call outofmemory('reaxFF','numattodonptr')
...
100 continue
  ...
  if (allocated(rneigh)) then
    ...
    if (status/=0) call deallocate_error('reaxFF','deltalplp')
    ...
    if (status/=0) call deallocate_error('reaxFF','ineigh')
...
!
  ...
  if (status/=0) call outofmemory('reaxFF','ineigh')
  ...
  if (status/=0) call outofmemory('reaxFF','deltalplp')
  if (lgrad1) then
...
!
      ...
      if (status/=0) call outofmemory('reaxffmd','StatMPI')
!
      do i = 1,numat
...
!
      ...
      if (status/=0) call deallocate_error('reaxffmd','StatMPI')
    else
#endif
!
      call mpbarrier
...
!
      ...
      if (status/=0) call outofmemory('reaxffmd','nntmp')
      allocate(botmp(2*ntmp),stat=status)
      if (status/=0) call outofmemory('reaxffmd','botmp')
...
!
          if (ioproc) then
            ...
            call MPI_IRecv(nntmp,ntmp,MPI_integer,nnode, &
                           ntag,MPI_Comm_World,Request(1),MPIerror)
            call MPI_IRecv(botmp,ntmp,MPI_double_precision,nnode, &
                           ntag2,MPI_Comm_World,Request(2),MPIerror)
          else
!
!  Pass data to ioproc for writing
!
...
!
      ...
      if (status/=0) call deallocate_error('reaxffmd','botmp')
      deallocate(nntmp,stat=status)
      if (status/=0) call deallocate_error('reaxffmd','nntmp')
    else
#endif
      do i = 1,numat
        if (procid.eq.numattodonptr(i)) then
...
!
    ...
    call sumall(sum,deltap,numat,"reaxffmd","deltap")
  endif
!*****************************
...
!
            eij = - 0.5_dp*(reaxFFDe(1,nboij)*BOij_s*expij + reaxFFDe(2,nboij)*BOij_pi + reaxFFDe(3,nboij)*BOij_pipi)
!
...
!
            if (lgrad1) then
              call d1add(j,maxneigh,mneigh,nneigh,neighno,xneigh,yneigh,zneigh,nfreeatom,nbodummyptr,d1j,.false.,.true.)
            endif
          endif
!
!  End condition section on i or j being associated with moving atom
...
!
    if (lgrad1.and.ii.le.numattodo_main) then
      call d1add(i,maxneigh,mneigh,nneigh,neighno,xneigh,yneigh,zneigh,nfreeatom,nbodummyptr,d1i,.false.,.true.)
    endif
  enddo iloop
!***************************************************
...
!   
    ...
    call sumall(sum,sum0,3_i4*numat,"reaxffmd","delta")
...
!
          if (abs(reaxFFlp(2,nspeci)).gt.1.0d-12) then
            deloneddeltalp = reaxFFlp(2,nspeci)*trm1lp*(trm1lp*deltalplp(i)*explp*reaxFFlam(30) + 1.0_dp)
            deloneddeltai = deloneddeltalp*ddeltalpddeltaelp
          else
...
!
          call d1add(i,maxneigh,mneigh,nneigh,neighno,xneigh,yneigh,zneigh,nfreeatom,nbodummyptr,d1i,.true.,.true.)
!
...
!
                              ...
                              if (lstr) then
                                call real1strterm(ndim,xki,yki,zki,0.0_dp,0.0_dp,0.0_dp,dr2ds,d2r2dx2,d2r2dsdx,d2r2ds2,.false.)
                                rstrdloc(1:nstrains) = 0.0_dp
                                do kl = 1,nstrains
                                  ns = nstrptr(kl)
                                  rstrdloc(kl) = rstrdloc(kl) + dehbdrik*dr2ds(ns)
                                enddo
                                do kl = 1,nstrains
...
!
                                ...
                                if (lstr) then
                                  call real1strterm(ndim,xkj,ykj,zkj,0.0_dp,0.0_dp,0.0_dp,dr2ds,d2r2dx2,d2r2dsdx,d2r2ds2,.false.)
                                  rstrdloc(1:nstrains) = 0.0_dp
                                  do kl = 1,nstrains
                                    ns = nstrptr(kl)
                                    rstrdloc(kl) = rstrdloc(kl) + dehbdrjk*dr2ds(ns)
                                  enddo
                                  do kl = 1,nstrains 
                                    rstrd(kl) = rstrd(kl) + rstrdloc(kl)
                                  enddo
...
!
                                ...
                                do kl = 1,nstrains
                                  ns = nstrptr(kl)
                                  rstrdloc(kl) = rstrdloc(kl) + dehbdtheta*dthetadrij*dr2ds(ns)
                                enddo
!
                                call real1strterm(ndim,xki,yki,zki,0.0_dp,0.0_dp,0.0_dp,dr2ds,d2r2dx2,d2r2dsdx,d2r2ds2,.false.)
                                do kl = 1,nstrains
                                  ns = nstrptr(kl)
                                  rstrdloc(kl) = rstrdloc(kl) + dehbdtheta*dthetadrik*dr2ds(ns)
                                enddo
!
                                call real1strterm(ndim,xkj,ykj,zkj,0.0_dp,0.0_dp,0.0_dp,dr2ds,d2r2dx2,d2r2dsdx,d2r2ds2,.false.)
                                do kl = 1,nstrains
                                  ns = nstrptr(kl)
                                  rstrdloc(kl) = rstrdloc(kl) + dehbdtheta*dthetadrjk*dr2ds(ns)
                                enddo
!
                                do kl = 1,nstrains
                                  rstrd(kl) = rstrd(kl) + rstrdloc(kl)
...
!***************************************
  t3 = g_cpu_time()
  if (lreaxFFQ) then
    if (literativeQ.or.nprocs.gt.1) then
      call setreaxffQiter(nboatom,nboatomptr,nboatomRptr,nbos,nbosptr,qreaxFF,eself,.false.)
    else
      call setreaxffQ(nboatom,nboatomRptr,nbos,nbosptr,qreaxFF,eself,.false.)
!
...
!
                              if (lstr) then
                                ...
                                do kl = 1,nstrains
                                  ns = nstrptr(kl)
                                  rstrdloc(kl) = rstrdloc(kl) + (devdwdrij + decouldrij)*dr2ds(ns)
                                enddo
                                do kl = 1,nstrains 
                                  rstrd(kl) = rstrd(kl) + rstrdloc(kl)
                                enddo
...
!
  ...
  if (status/=0) call deallocate_error('reaxFF','numattodonptr')

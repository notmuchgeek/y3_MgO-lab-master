...
!   3/18 Checking for fitting pointers that exceed maxfit now added
!   1/19 maxwordlength changes added
!  12/19 Checking for options that are within GA block added
!
!  Conditions of use:
!
...
!
  ...
  if (index(word,'gexp').eq.1) goto 40
  if (index(word,'tpxo').eq.1) goto 40
  if (index(word,'muta').eq.1) goto 40
  if (index(word,'dmax').eq.1) goto 40
  if (index(word,'dmin').eq.1) goto 40
  if (index(word,'disc').eq.1) goto 40
  if (index(word,'conf').eq.1) goto 40
  if (index(word,'best').eq.1) goto 40
  if (index(word,'uniq').eq.1) goto 40
  if (index(word,'tour').eq.1) goto 40
  if (index(word,'cros').eq.1) goto 40
  if (index(word,'seed').eq.1) goto 40
  if (index(word,'grid').eq.1) goto 40
  if (index(word,'anne').eq.1) goto 40
  if (index(word,'ante').eq.1) goto 40
  if (index(word,'anfa').eq.1) goto 40
  if (index(word,'ttol').eq.1) goto 40
  if (index(word,'cost').eq.1) goto 40
  if (index(word,'conj').eq.1) goto 40
  return
!**********************
!  Genetic Algorithm  *
!**********************
...
!
!  Check whether any value in ntemp exceeds maxfit
!
    nmax = maxval(ntemp(1:n))
    if (nmax.gt.maxfit) then
      maxfit = nmax
      call changemaxfit
    endif
!

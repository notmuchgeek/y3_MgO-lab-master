...
!   8/15 Switch after lower option added
...
!
  ...
  if (lhess2D) then
    write(ioout,'(''  General 2-D hessian to be used'',/)')
  else
    write(ioout,'(''  Lower half triangular hessian to be used'',/)')
  endif
  if (lminch) then
    ...
    elseif (minchcrit.eq.3) then
      write(ioout,'(''  After lower_symmetry operation '')')
    endif
  endif
  if (lmstpch) then
    if (mstpchcrit.eq.1) then
      ichcrit = nint(stpchcrit)
      write(ioout,'(''  Switch max step size to '',f16.8,'' after '',i4,'' cycles of optimisation'',/)') stepmxch,ichcrit
    elseif (mstpchcrit.eq.2) then
      write(ioout,'(''  Switch max step size to '',f16.8,'' when gnorm is less than '',f12.6,/)') stepmxch,stpchcrit
    endif
  endif

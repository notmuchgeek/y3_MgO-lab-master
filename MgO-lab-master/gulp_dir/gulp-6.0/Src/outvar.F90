...
!
  ...
  use optimisation
  use six
  use species,      only : natspec, ntypspec
  use two
  implicit none
!
!  Passed variables
...
!
        ...
        if (imode.eq.1) then
          write(ioout,'(10x,i4,10x,f16.6,10x,a15,2x,i2,1x,i3,1x,a)') i,w1i,fitlabel(1,nf,1),nccf,nv,namecrd(icrd)
        else
          write(ioout,'(4x,i6,6x,f16.6,1x,f16.6,1x,a15,2x,i2,1x,i3,1x,a)') i,w1o,w1i,fitlabel(1,nf,1),nccf,nv,namecrd(icrd)
        endif
      elseif (nf.eq.2) then
!
...
!
      ...
      elseif (nf.ge.39.and.nf.le.41) then
!
!  Gasteiger species parameters
!
        w1i = xctmp(i)
        w1o = xcoriginal(i)
        call label(natspec(nv),ntypspec(nv),lab1)
        if (natspec(nv).gt.maxele) then
          cstyp1 = 's'
        else
          cstyp1 = 'c'
        endif

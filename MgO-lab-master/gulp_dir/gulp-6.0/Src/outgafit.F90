...
!   6/18 Corrections to use of nf vs np added
!
!  Conditions of use:
!
...
!
  ...
  use fitting
  use four,      only : nforty
  use gaconf
  use genetic
  use iochannels
  use six,       only : nsixty
  use m_three,   only : nthrty
  use two
  use uffdata
  implicit none
!
...
!***********************
  do ng = 1,ngroup
    ...
    jbase = 4*(ng-1)
    jbasef = jbase
    if (ngabest.gt.0) jbasef = jbasef + 2*ngacfg
    write(ioout,'(''--------------------------------------------------------------------------------'')')
...
!-----------------------
        ...
        do j = 1,jmax
          w1(j) = xconf(i,jbase+j)*scale(i)
        enddo
        write(ioout,'(i3,2x,a15,7x,4(1x,f12.5))') i,fitlabel(nv,np,2),(w1(j),j=1,jmax)
      elseif (nt.eq.3) then
!-------------------------
...
!-------------------------
        ...
        do j = 1,jmax
          w1(j) = xconf(i,jbase+j)*scale(i)
        enddo
        write(ioout,'(i3,2x,a15,1x,i4,2x,4(1x,f12.5))') i,fitlabel(nv,np,3),nt,(w1(j),j=1,jmax)
      elseif (nt.eq.4) then
!------------------------
...
!------------------------
        ...
        do j = 1,jmax
          w1(j) = xconf(i,jbase+j)*scale(i)
        enddo
        write(ioout,'(i3,2x,a15,1x,i4,2x,4(1x,f12.5))') i,fitlabel(nv,np,4),nt,(w1(j),j=1,jmax)
      elseif (nt.eq.5) then
!------------------------
...
!------------------------
        ...
        write(ioout,'(i3,2x,a15,a5,1x,a1,4(1x,f12.6))') i,fitlabel(nv,1,5),lab1,cstyp1,(w1(j),j=1,jmax)
      elseif (nt.eq.6) then
!-------------------------
...
!-------------------------
        do j = 1,jmax
          w1(j) = xconf(i,jbase+j)*scale(i)
        enddo
        write(ioout,'(i3,2x,a15,1x,i4,2x,4(1x,f12.5))') i,fitlabel(nv,1,6),nt,(w1(j),j=1,jmax)
      elseif (nt.eq.7) then
!-----------------------
!  Six-body parameters -
!-----------------------
        ...
        do j = 1,jmax
          w1(j) = xconf(i,jbase+j)*scale(i)
        enddo
        write(ioout,'(i3,2x,a15,1x,i4,2x,4(1x,f12.5))') i,fitlabel(nv,np,7),nt,(w1(j),j=1,jmax)
      elseif (nt.eq.9) then
!-------------------
!  EDIP potentials -
!-------------------
        do j = 1,jmax
          w1(j) = xconf(i,jbase+j)*scale(i)
        enddo
        write(ioout,'(i3,2x,a15,1x,i4,2x,4(1x,f12.5))') i,fitlabel(nv2,nv,9),nt,(w1(j),j=1,jmax)
      endif
    enddo
    write(ioout,'(''--------------------------------------------------------------------------------'',/)')
  enddo

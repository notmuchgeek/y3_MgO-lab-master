  function lmatchany(nat1,ntype1,nspecany,natany,ntypeany,lwildcard)
!
...
!                       (mod(nat1,maxele).eq.0 => wildcard atomic no.)
!                       (ntype1.eq.0 => wildcard type number)
!
...
!
  do while (.not.lmatchany.and.nany.lt.nspecany)
    nany = nany + 1
    lmatchany = ((nat1.eq.natany(nany).or.(lwildcard.and.natany(nany).eq.nwildcard)) &
                 .and.(ntype1.eq.ntypeany(nany).or.ntypeany(nany).eq.0))
  enddo
!
  return
  end
...
!     
!  Julian Gale, CIC, Curtin University, July 2008
!
  use datatypes
  use element, only : maxele
  implicit none

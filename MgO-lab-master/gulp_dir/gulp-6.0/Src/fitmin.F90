...
!
!  12/19 Saving of initial vector added
!
!**********************************************************************
  use control
  use fitting
...
!
  ...
  real(dp)                                     :: xxm  
#ifdef TRACE
  call trace_in('fitmin')
#endif
!
!  Allocate local memory
!
  allocate(xsave(nvar),stat=status)
  if (status/=0) call outofmemory('fitmin','xsave')
...
!
  ...
  if (status/=0) call deallocate_error('fitmin','xsave')
#ifdef TRACE
  call trace_out('fitmin')
#endif
!
  return
  end

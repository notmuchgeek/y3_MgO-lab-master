...
!
  ...
  use current
  use gulp_files,   only : arcfile, larc, lmovie
  use gulp_lengths
  use iochannels
  use g_neb
...
!
    arcfilel(1:maxwordlength) = arcfile(1:maxwordlength)
    inddot = index(arcfilel,'.')
    call endstring(arcfilel,len(arcfilel),indend)
    if (indend.eq.0) then
      arcfilel(maxwordlength-8:maxwordlength) = '_sync.arc'
    else
      if (inddot.eq.0) then
...
!
  do i = 1,nasym
    ...
    if (.not.lopttmp(ix)) then
      diff = abs(xafrac(ix) - nebfinalxyz(1,i,ncf))
    ...
    if (.not.lopttmp(iy)) then
      diff = abs(yafrac(iy) - nebfinalxyz(2,i,ncf))
      if (diff.gt.1.0d-4) then
        call outerror('fixed variables must be the same for initial and final structures',0_i4)
        call stopnow('runsync')
      endif
    ...
    if (.not.lopttmp(iz)) then
      diff = abs(zafrac(iz) - nebfinalxyz(3,i,ncf))
      if (diff.gt.1.0d-4) then
        call outerror('fixed variables must be the same for initial and final structures',0_i4)
        call stopnow('runsync')
      endif
...
!
      do n = 1,nvar
        ...
        elseif (iopttype(n).eq.iopt_xf) then
          i = ioptindex(n)
          ix = 1
          xcrep(n,nrp+1) = nebreplicaxyz(ix,i,ii)

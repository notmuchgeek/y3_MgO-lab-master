...
!   2/18 Trace added
!   7/20 Trap for NaN in coordinates added since this can cause infinite looping
!
!  Conditions of use:
!
!  GULP is available free of charge to academic institutions
...
!
  lincludeself = .false.
!
!  Check coordinates are sensible to prevent a runaway search
!
  if (isnan(xij0).or.isnan(yij0).or.isnan(zij0)) then
    call outerror('interatomic vector passed to rfind3D contains NaN',0_i4)
    call stopnow('rfind3D')
  endif
!
...
!  Check coordinates are sensible to prevent a runaway search
!
  if (isnan(xik0).or.isnan(yik0).or.isnan(zik0)) then
    call outerror('interatomic vector passed to rfindeither3D contains NaN',0_i4)
    call stopnow('rfindeither3D')
  endif
  if (isnan(xjk0).or.isnan(yjk0).or.isnan(zjk0)) then
    call outerror('interatomic vector passed to rfindeither3D contains NaN',0_i4)
    call stopnow('rfindeither3D')
  endif
!
...
!  Check coordinates are sensible to prevent a runaway search
!
  if (isnan(xil0).or.isnan(yil0).or.isnan(zil0)) then
    call outerror('interatomic vector passed to rfindeither33D contains NaN',0_i4)
    call stopnow('rfindeither33D')
  endif
  if (isnan(xjl0).or.isnan(yjl0).or.isnan(zjl0)) then
    call outerror('interatomic vector passed to rfindeither33D contains NaN',0_i4)
    call stopnow('rfindeither33D')
  endif
  if (isnan(xkl0).or.isnan(ykl0).or.isnan(zkl0)) then
    call outerror('interatomic vector passed to rfindeither33D contains NaN',0_i4)
    call stopnow('rfindeither33D')
  endif
!
...
!
  if (isnan(xij0).or.isnan(yij0).or.isnan(zij0)) then
    call outerror('interatomic vector passed to rfind2D contains NaN',0_i4)
    call stopnow('rfind2D')
  endif
!
!  nsafety is the number of search loops allowed without finding a new distance
!
...
!
  if (isnan(xik0).or.isnan(yik0).or.isnan(zik0)) then
    call outerror('interatomic vector passed to rfindeither2D contains NaN',0_i4)
    call stopnow('rfindeither2D')
  endif
  if (isnan(xjk0).or.isnan(yjk0).or.isnan(zjk0)) then
    call outerror('interatomic vector passed to rfindeither2D contains NaN',0_i4)
    call stopnow('rfindeither2D')
...
!
  if (isnan(xil0).or.isnan(yil0).or.isnan(zil0)) then
    call outerror('interatomic vector passed to rfindeither32D contains NaN',0_i4)
    call stopnow('rfindeither32D')
  endif
  if (isnan(xjl0).or.isnan(yjl0).or.isnan(zjl0)) then
    call outerror('interatomic vector passed to rfindeither32D contains NaN',0_i4)
    call stopnow('rfindeither32D')
...
!
  if (isnan(xij0).or.isnan(yij0).or.isnan(zij0)) then
    call outerror('interatomic vector passed to rfind1D contains NaN',0_i4)
    call stopnow('rfind1D')
  endif
!
!  Find image of j nearest to i 
!
...
!
  if (isnan(xik0).or.isnan(yik0).or.isnan(zik0)) then
    call outerror('interatomic vector passed to rfindeither1D contains NaN',0_i4)
    call stopnow('rfindeither1D')
  endif
  if (isnan(xjk0).or.isnan(yjk0).or.isnan(zjk0)) then
    call outerror('interatomic vector passed to rfindeither1D contains NaN',0_i4)
    call stopnow('rfindeither1D')
...
!
  if (isnan(xil0).or.isnan(yil0).or.isnan(zil0)) then
    call outerror('interatomic vector passed to rfindeither31D contains NaN',0_i4)
    call stopnow('rfindeither31D')
  endif
  if (isnan(xjl0).or.isnan(yjl0).or.isnan(zjl0)) then
    call outerror('interatomic vector passed to rfindeither31D contains NaN',0_i4)
    call stopnow('rfindeither31D')

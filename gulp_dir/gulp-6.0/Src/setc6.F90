...
!
!  Julian Gale, CIC, Curtin University, February 2015
!
  use control
  use current
  use species
...
!*************************
  ...
  do i = 1,npote
    ...
    elseif (nptype(i).eq.57) then
      if (abs(twopot(3,i)).gt.0.0_dp.and.abs(twopot(4,i)).gt.0.0_dp.and.abs(twopot(5,i)).gt.0.0_dp) then
        nc6 = nc6 + 1
      endif
    endif
  enddo
    ...
    if (nspec1(i).eq.nspec2(i).and.nptyp1(i).eq.nptyp2(i)) then
      ...
      do j = 1,nspec
        if (nspec1(i).eq.natspec(j).and.(nptyp1(i).eq.ntypspec(j).or.nptyp1(i).eq.0)) then
          ...
          elseif (nptype(i).eq.57) then
            c6spec(j) = sqrt(twopot(3,i)*twopot(4,i)*twopot(5,i)**6)
          else
    ...
    do i = 1,npote
      if (.not.lokpot(i)) then
        ...
        if (c6trm1.eq.0.0.and.c6trm2.ne.0.0) then
          ...
          elseif (nptype(i).eq.57) then
            c6trm1 = (twopot(3,i)*twopot(4,i)*twopot(5,i)**6)/c6trm2
          endif
          do j = 1,nspec
            if (nspec1(i).eq.natspec(j).and.(nptyp1(i).eq.ntypspec(j).or.nptyp1(i).eq.0)) then
              c6spec(j) = c6trm1
        ...
        elseif (c6trm2.eq.0.0.and.c6trm1.ne.0.0) then
          ...
          elseif (nptype(i).eq.57) then
            c6trm2 = (twopot(3,i)*twopot(4,i)*twopot(5,i)**6)/c6trm1
          endif
          do j = 1,nspec
            if (nspec2(i).eq.natspec(j).and.(nptyp2(i).eq.ntypspec(j).or.nptyp2(i).eq.0)) then
              c6spec(j) = c6trm2
        ...
        elseif (c6trm1.ne.0.0.and.c6trm2.ne.0.0) then
          ...
          elseif (nptype(i).eq.57) then
            pdiff = twopot(3,i)*twopot(4,i)*twopot(5,i)**6 - c6trm1*c6trm2
          else
            pdiff = 0.0_dp
          endif
      ...
      do n = 1,npote
        if (nspec1(n).eq.nati.and.nspec2(n).eq.natj.and. &
          ...
          elseif (nptype(n).eq.57) then
            c6pair = c6pair + twopot(3,n)*twopot(4,n)*twopot(5,n)**6
          endif

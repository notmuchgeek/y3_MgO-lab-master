...
!   3/15 Units of potential corrected from eV to V
!   6/15 Bug in case of no symmetry for 3-D case fixed
!   1/18 Trace added
!
!  Conditions of use:
!
...
!******************
  if (ioproc) then
    ...
    if (ndim.eq.3) then
      write(ioout,'(''         (Fractional)              (V)             x          y          z'')')
    elseif (ndim.eq.2) then
      write(ioout,'(''  (Frac)     (Frac)    (Angs)      (V)             x          y          z'')')
    elseif (ndim.eq.1) then
      write(ioout,'(''  (Frac)     (Angs)    (Angs)      (V)             x          y          z'')')
    else
      write(ioout,'(''         (Angstroms)               (V)             x          y          z'')')
    endif
    write(ioout,'(''-------------------------------------------------------------------------------'')')
  endif
!***********************************
...
!
            ...
            xsite = xs*r1x + ys*r2x + zs*r3x
            ysite = xs*r1y + ys*r2y + zs*r3y
            zsite = xs*r1z + ys*r2z + zs*r3z
          endif
        elseif (ndim.eq.2) then
          xsite = xs*r1x + ys*r2x
          ysite = xs*r1y + ys*r2y

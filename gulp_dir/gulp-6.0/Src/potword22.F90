...
!  ipot(3,*) = pointer to self-pair potential for atom 1
!  ipot(4,*) = pointer to self-pair potential for atom 2
!
...
!
  ...
  do while (.not.lfound.and.np.lt.npote)
    ...
    if (nptype(np).eq.45.or.nptype(np).eq.55) then
      lfound = (lmatch(nspec1(npote),nptyp1(npote),nspec1(np),nptyp1(np),.true.).and. &
                lmatch(nspec1(npote),nptyp1(npote),nspec2(np),nptyp2(np),.true.)) 
    endif
  enddo
  if (lfound) then
    ipot(3,npote) = np
  else
    call outerror('Baskes potential has no matching self term',iline)
    call stopnow('potword22')
  endif
!
  lfound = .false.
  ...
  do while (.not.lfound.and.np.lt.npote)
    np = np + 1
    if (nptype(np).eq.45.or.nptype(np).eq.55) then
      lfound = (lmatch(nspec2(npote),nptyp2(npote),nspec1(np),nptyp1(np),.true.).and. &
                lmatch(nspec2(npote),nptyp2(npote),nspec2(np),nptyp2(np),.true.)) 
    endif
  enddo
  if (lfound) then
    ipot(4,npote) = np

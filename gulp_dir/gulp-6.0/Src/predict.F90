...
!
!  maxmvar = max #variables and thus used in dimensions of arrays
!  nvar = actual #variables and thus used in do loops of calcs
!  mcfg = #configurations to optimise upon return
!  xconf(maxmvar,2*maxgcfg) array to store parent+children configs
!  xbest(maxmvar,maxbcfg)   array with best configs to optimise
!
  use configurations
  use control
  use current
...
!
  ...
  lfreezeok = (.not.lnoexclude.and.ncell.eq.0.and..not.lfreeloc)

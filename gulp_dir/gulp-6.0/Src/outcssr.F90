...
!  Local variables
!
  character(len=maxwordlength)  :: cssrfile2
  character(len=maxwordlength)  :: cssrfilel
  character(len=5)              :: lab
...
!
      ...
      if (ncfg.gt.1) then
        write(ioout,'(''  CSSR file written for configuration '',i4,'' as '',a)') ncf,trim(cssrfile2)
      else
        write(ioout,'(''  CSSR file written as '',a)') trim(cssrfile2)
      endif
!
      call setup(.false.)
!
...
!
        ...
        if (nspcg(ncf).eq.0) then
          ...
          write(iout,'(21x,3f8.3,4x,''SPGR =  '',i3,1x,11a1)') alpha,beta,gamma,nspcg(ncf),(hmssgio(i,ncf),i=1,11)
        endif
      else
!

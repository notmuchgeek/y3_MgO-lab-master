...
!
!  Subroutine calculates properties that depend on the applied frequency.
!
!   3/02 Created
...
!
  ...
  use g_constants
  use current
  use gulp_files,     only : losc, oscfile
  use iochannels
...
!
  ...
  real(dp)                 :: volume
!
!  In parallel only ioproc needs to go through this routine
!
  if (.not.ioproc) return
#ifdef TRACE
  call trace_in('omegaproperty')
#endif
!
...
!
    dlcomega(1:3,1:3) = dcmplx(diconh(1:3,1:3),0.0_dp)
    do m = 1,modes
      freqfactor = dcmplx(freq(m)**2 - ome**2,-omegadamping(ncf)*ome)

...
!  11/18 Missing factor of 6 added for xyz term in 3rd order
!
!  Conditions of use:
!
...
#ifdef TRACE
  call trace_in('meamtotalrhocrossderv')
#endif
!
...
!
  ...
  if (lstr) then
    drhotot2s(1:nstrains,1:nstrains) = 0.0_dp
...
!
          ...
          dgamma2(1:3) = 0.0_dp
          if (lstr) then
            dgamma1s(1:nstrains) = 0.0_dp
            dgamma2s(1:nstrains) = 0.0_dp
          endif
!
...
!
          if (nterm.gt.1) then
            ...
            if (lstr) then
              do i = 2,nterm
                dgamma1s(1:nstrains) = dgamma1s(1:nstrains) + dgammadrhoh*coeff(i)*scrho(i)*drhos1(1:nstrains,i)
                dgamma2s(1:nstrains) = dgamma2s(1:nstrains) + dgammadrhoh*coeff(i)*scrho(i)*drhos2(1:nstrains,i)
              enddo
            endif
          endif
!
...
!
          ...
          if (lstr) then
            dgamma1s(1:nstrains) = dgamma1s(1:nstrains) - 2.0_dp*gamma*rrho0*drhos1(1:nstrains,1)
            dgamma2s(1:nstrains) = dgamma2s(1:nstrains) - 2.0_dp*gamma*rrho0*drhos2(1:nstrains,1)
          endif
!
...
!
          if (lstr) then
            d2gamma2s(1:nstrains,1:nstrains) = 0.0_dp
...
!
          ...
          enddo
          if (lstr) then
            drhotot2s(1:nstrains,1:nstrains) = drhotot2s(1:nstrains,1:nstrains) + &
              rho0*dgofgammadgamma*d2gamma2s(1:nstrains,1:nstrains)
            drhotot2m(1:nstrains,1:3) = drhotot2m(1:nstrains,1:3) + rho0*dgofgammadgamma*d2gamma2m(1:nstrains,1:3)
            do ns1 = 1,nstrains
              do ns2 = 1,nstrains
...
#ifdef TRACE
  call trace_out('meamtotalrhocrossderv')
#endif
!
  return
  end

...
!
              if (lasu1) then
                do kl = 1,6
                  ks = nstrptr(kl)
                  rstrd(kl) = rstrd(kl) + ed11*dr2ds(ks,1)*rsfct
                  rstrd(kl) = rstrd(kl) + ed12*dr2ds(ks,2)*rsfct
                  rstrd(kl) = rstrd(kl) + ed13*dr2ds(ks,3)*rsfct
                enddo
              endif
!
              ...
              if (lgrad2.and.lstr) then
                ...
                if (lasu1) then
                  do kk = 1,nstrains
                    ...
                    do kl = kk,nstrains
                      kt = nstrptr(kl)
                      sderv2(kl,kk) = sderv2(kl,kk) + (dr2ds(ks,1)*tm2(kl,1) + dr2ds(ks,2)*tm2(kl,2) + dr2ds(ks,3)*tm2(kl,3))*rsfct
                      sderv2(kl,kk) = sderv2(kl,kk) + (d2r2ds2(kt,ks,1)*ed11 + d2r2ds2(kt,ks,2)*ed12 + d2r2ds2(kt,ks,3)*ed13)*rsfct
                    enddo
                  enddo
!
!  Mixed derivatives
...
!
                    derv3(n1x,kk) = derv3(n1x,kk) - (ed11*d2r2dsdx(ks,1,1) + ed12*d2r2dsdx(ks,1,2))*neqi
                    derv3(n1y,kk) = derv3(n1y,kk) - (ed11*d2r2dsdx(ks,2,1) + ed12*d2r2dsdx(ks,2,2))*neqi
                    derv3(n1z,kk) = derv3(n1z,kk) - (ed11*d2r2dsdx(ks,3,1) + ed12*d2r2dsdx(ks,3,2))*neqi
                  enddo
                endif
                if (lasu2) then
                  do kk = 1,6
...
!
                    derv3(n2x,kk) = derv3(n2x,kk) + (ed11*d2r2dsdx(ks,1,1) - ed13*d2r2dsdx(ks,1,3))*neqj
                    derv3(n2y,kk) = derv3(n2y,kk) + (ed11*d2r2dsdx(ks,2,1) - ed13*d2r2dsdx(ks,2,3))*neqj
                    derv3(n2z,kk) = derv3(n2z,kk) + (ed11*d2r2dsdx(ks,3,1) - ed13*d2r2dsdx(ks,3,3))*neqj
                  enddo
                endif
                if (lasu3) then
                  do kk = 1,6
...
!
                    derv3(n3x,kk) = derv3(n3x,kk) + (ed12*d2r2dsdx(ks,1,2) + ed13*d2r2dsdx(ks,1,3))*neqk
                    derv3(n3y,kk) = derv3(n3y,kk) + (ed12*d2r2dsdx(ks,2,2) + ed13*d2r2dsdx(ks,2,3))*neqk
                    derv3(n3z,kk) = derv3(n3z,kk) + (ed12*d2r2dsdx(ks,3,2) + ed13*d2r2dsdx(ks,3,3))*neqk
                  enddo
                endif
              endif
            endif

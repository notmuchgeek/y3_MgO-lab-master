...
!   5/17 Intent added for arguments
!   2/18 Trace added
    ...
    if (ldsl) then
      ...
      do i = 1,nloopi
        ...
        if (lfound.and.rhoi.gt.1.0d-12) then
          if (lMEAMfn) then
            frho(1:maxmeamcomponent) = dscrho(1:maxmeamcomponent,i)
...
!
    if (lsymalg) then
      do i = 1,ndpasym2a
        ...
        if (lfound.and.rhoi.gt.1.0d-12) then
          if (lMEAMfn) then
            frho(1:maxmeamcomponent) = dscrhor2d(1:maxmeamcomponent,i)
            call meamfnderv(neamfn,neamspeci,frho,rhoi,eeam,rscrhoi,rscrhoi3,rscrhoi5,.false.,.false.,.false.)
          else
...
!
    ...
    do i = 1,nr1
      ...
      if (lfound.and.rhoi.gt.1.0d-12) then
        if (lMEAMfn) then
          frho(1:maxmeamcomponent) = scrho(1:maxmeamcomponent,nrelpi)
          call meamfnderv(neamfn,neamspeci,frho,rhoi,eeam,rscrhoi,rscrhoi3,rscrhoi5,.false.,.false.,.false.)
        else
...
!
    do i = 1,npreg2
      ...
      if (lfound.and.rhoi.gt.1.0d-12) then
        if (lMEAMfn) then
          frho(1:maxmeamcomponent) = scrho(1:maxmeamcomponent,npsi)
          call meamfnderv(neamfn,neamspeci,frho,rhoi,eeam,rscrhoi,rscrhoi3,rscrhoi5,.false.,.false.,.false.)
        else
...
!
    ...
    elseif (imode.eq.2) then
      ...
      npsi = npsite(i)
      if (lMEAM) then
        scrhoi(1:maxmeamcomponent) = scrho(1:maxmeamcomponent,nrelf2a(i))
      else
        scrhoi(1) = scrho(1,nrelf2a(i))
      endif
    endif
!
    neamspeci = 0
...
!
        ...
        if (imode.eq.1) then
          ...
          npsj = npsite(j)
          if (lMEAM) then
            scrhoj(1:maxmeamcomponent) = scrho(1:maxmeamcomponent,nrelf2a(npsj))
          else
            scrhoj(1) = scrho(1,nrelf2a(npsj))
          endif
        endif
        jx = 3*(j - 1) + 1
        jy = jx + 1
...
!
                  if (lgrad2) then
                    deriv2(1) = deriv2(1) + d2bas*Sij*xcd*xcd + d1bas*Sij
                    deriv2(2) = deriv2(2) + d2bas*Sij*xcd*ycd
                    deriv2(3) = deriv2(3) + d2bas*Sij*xcd*zcd
                    deriv2(4) = deriv2(4) + d2bas*Sij*ycd*ycd + d1bas*Sij
                    deriv2(5) = deriv2(5) + d2bas*Sij*ycd*zcd
                    deriv2(6) = deriv2(6) + d2bas*Sij*zcd*zcd + d1bas*Sij
                  endif
                endif
              endif
            enddo
...
!
              if (imode.eq.1) then
                ...
                npsk = npsite(k)
                if (lMEAM) then
                  scrhok(1:maxmeamcomponent) = scrho(1:maxmeamcomponent,nrelf2a(npsk))
                else
                  scrhok(1) = scrho(1,nrelf2a(npsk))
                endif
              endif
            else
!

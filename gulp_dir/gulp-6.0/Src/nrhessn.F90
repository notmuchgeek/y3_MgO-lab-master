  subroutine nrhessn(hesinv,maxhess,lhess2D,diag,nvar,ldiag)
!
...
!
!  NB: Finite difference version where Hessian was generated directly
!
!   5/19 Created from nrhess
!  12/19 Diagonal elements now taken from hesinv since this was generated directly
!   3/20 Correction to setting of diagonal Hessian elements
...
!
  if (lhess2D) then
    ...
    if (nprocs.gt.1) then
      ...
      if (status/=0) call outofmemory('nrhessn','temp')
!
      call sumall(diag,temp,nvar,"diag","nrhessn")
      diag(1:nvar) = temp(1:nvar)
!
      deallocate(temp,stat=status)
      if (status/=0) call deallocate_error('nrhessn','temp')
    endif
  else
    ind = 0
    do i = 1,nvar
...
!
      ...
      if (status/=0) call outofmemory('nrhessn','StatMPI')
    endif
    call mpbarrier
#endif
    if (lhess2D) then
...
!
    ...
    if (lioproconly.and.lhess2D) then
      ...
      if (status/=0) call deallocate_error('nrhessn','StatMPI')
      deallocate(temp,stat=status)
      if (status/=0) call deallocate_error('nrhessn','temp')
    endif
#endif
...
!***************************************
    ...
    if (status/=0) call outofmemory('nrhessn','ipivot')
!
...
!
      ...
      if (status/=0) call outofmemory('nrhessn','iwrk')
      allocate(wrk(lwrk),stat=status)
      if (status/=0) call outofmemory('nrhessn','wrk')
!
...
!
      ...
      if (status/=0) call deallocate_error('nrhessn','wrk')
      deallocate(iwrk,stat=status)
      if (status/=0) call deallocate_error('nrhessn','iwrk')
      allocate(iwrk(liwrk),stat=status)
      if (status/=0) call outofmemory('nrhessn','iwrk')
      allocate(wrk(lwrk),stat=status)
      if (status/=0) call outofmemory('nrhessn','wrk')
...
!
    ...
    if (status/=0) call deallocate_error('nrhessn','ipivot')
  else
#endif
!
!  Factorise matrix
!
    ...
    if (status/=0) call outofmemory('nrhessn','kpvt')
    if (lhess2D) then
...
!
    ...
    if (status/=0) call deallocate_error('nrhessn','kpvt')
#ifdef MPI
  endif
#endif
  t2i = g_cpu_time()

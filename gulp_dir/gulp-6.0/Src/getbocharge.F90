...
!   2/18 Trace added
!   1/19 Finite strain derivatives added
!   1/19 Logic for calling real1strterm corrected
...
100 continue
  ...
  if (allocated(rneigh)) then
    ...
    if (status/=0) call deallocate_error('getBOcharge','ineigh')
...
!
  ...
  if (status/=0) call outofmemory('getBOcharge','ineigh')
  ...
  if (lgrad1) then
    ...
    do i = 1,numat
      ...
      do nn = 1,nneigh(i)
        ...
        nqatomcell(1:3,nn,i) = ineigh(1:3,nn,i)
        qatomxyz(1,nn,i) = xneigh(nn,i)
        qatomxyz(2,nn,i) = yneigh(nn,i)
        qatomxyz(3,nn,i) = zneigh(nn,i)
      enddo
    enddo
  endif
!*****************************
...
!
          if (ndim.gt.0.and.lstr) then
            do kl = 1,nstrains
              ks = nstrptr(kl)
              dqds(kl,i) = dqds(kl,i) + d1trm*dr2ds(ks)
              dqds(kl,j) = dqds(kl,j) - d1trm*dr2ds(ks)
            enddo
          endif
          if (lgrad2) then
!***********************
...
!
              ...
              ijyz = ijzz - 1
!
              d2qdxyz2(ijxx,i) = d2qdxyz2(ijxx,i) - d2trm*d2r2dx2(1)
              d2qdxyz2(ijxy,i) = d2qdxyz2(ijxy,i) - d2trm*d2r2dx2(6)
              d2qdxyz2(ijxz,i) = d2qdxyz2(ijxz,i) - d2trm*d2r2dx2(5)
              d2qdxyz2(ijyy,i) = d2qdxyz2(ijyy,i) - d2trm*d2r2dx2(2)
              d2qdxyz2(ijyz,i) = d2qdxyz2(ijyz,i) - d2trm*d2r2dx2(4)
              d2qdxyz2(ijzz,i) = d2qdxyz2(ijzz,i) - d2trm*d2r2dx2(3)
...
!
              ...
              ijyz = ijzz - 1
!
              d2qdxyz2(ijxx,j) = d2qdxyz2(ijxx,j) + d2trm*d2r2dx2(1)
              d2qdxyz2(ijxy,j) = d2qdxyz2(ijxy,j) + d2trm*d2r2dx2(6)
              d2qdxyz2(ijxz,j) = d2qdxyz2(ijxz,j) + d2trm*d2r2dx2(5)
              d2qdxyz2(ijyy,j) = d2qdxyz2(ijyy,j) + d2trm*d2r2dx2(2)
              d2qdxyz2(ijyz,j) = d2qdxyz2(ijyz,j) + d2trm*d2r2dx2(4)
              d2qdxyz2(ijzz,j) = d2qdxyz2(ijzz,j) + d2trm*d2r2dx2(3)
            ...
            if (ndim.gt.0.and.lstr) then
              ...
              do kk = 1,nstrains
                ...
                do kl = 1,kk
                  kt = nstrptr(kl)
                  ind = ind + 1
                  d2qds2(ind,i) = d2qds2(ind,i) + d2trm*dr2ds(ks)*dr2ds(kt) + d1trm*d2r2ds2(ks,kt)
                  d2qds2(ind,j) = d2qds2(ind,j) - d2trm*dr2ds(ks)*dr2ds(kt) - d1trm*d2r2ds2(ks,kt)
                enddo
!
                d2qdxyzs(kk,jix,i) = d2qdxyzs(kk,jix,i) + d2trm*dr2ds(ks)*xji + d1trm*d2r2dsdx(ks,1)
                d2qdxyzs(kk,jiy,i) = d2qdxyzs(kk,jiy,i) + d2trm*dr2ds(ks)*yji + d1trm*d2r2dsdx(ks,2)
                d2qdxyzs(kk,jiz,i) = d2qdxyzs(kk,jiz,i) + d2trm*dr2ds(ks)*zji + d1trm*d2r2dsdx(ks,3)
                d2qdxyzs(kk,ijx,j) = d2qdxyzs(kk,ijx,j) + d2trm*dr2ds(ks)*xji + d1trm*d2r2dsdx(ks,1)
                d2qdxyzs(kk,ijy,j) = d2qdxyzs(kk,ijy,j) + d2trm*dr2ds(ks)*yji + d1trm*d2r2dsdx(ks,2)
                d2qdxyzs(kk,ijz,j) = d2qdxyzs(kk,ijz,j) + d2trm*dr2ds(ks)*zji + d1trm*d2r2dsdx(ks,3)
              enddo
            endif
          endif
        endif

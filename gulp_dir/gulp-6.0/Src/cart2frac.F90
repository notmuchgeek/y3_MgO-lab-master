...
!  icx     = cell shift in a parameter direction
!  icy     = cell shift in b parameter direction
!  icz     = cell shift in c parameter direction
!
...
!
    ...
    if (lmodco) then
      ...
      yo = rmat(2,4)
      zo = rmat(3,4)
      xf = dmod(rmat(1,4)+1000.0_dp,1.0_dp)
      yf = dmod(rmat(2,4)+1000.0_dp,1.0_dp)
      zf = dmod(rmat(3,4)+1000.0_dp,1.0_dp)
      icx = nint(xf-xo)
      icy = nint(yf-yo)
      icz = nint(zf-zo)
    else
...
!
    ...
    if (lmodco) then
      xo = rmat(1,4)
      yo = rmat(2,4)
      xf = dmod(rmat(1,3)+1000.0_dp,1.0_dp)
      yf = dmod(rmat(2,3)+1000.0_dp,1.0_dp)
      icx = nint(xf-xo)
      icy = nint(yf-yo)
    else
      xf = rmat(1,3)
...
!       
    ...
    if (lmodco) then
      xo = rmat(1,4)
      xf = dmod(rmat(1,1)+1000.0_dp,1.0_dp)
      icx = nint(xf-xo)
    else
      xf = rmat(1,1)
    endif
...
#ifdef TRACE
  call trace_out('cart2frac')
#endif
!
  return
  end
!
  subroutine cart2frac_nomod(ndim,xc,yc,zc,rv,xf,yf,zf)
!
!  Converts Cartesian coordinates to fractional coordinates
!  but without altering the sign of the coordinates by using mod
!
!  On entry : 
!
!  ndim    = number of dimensions (0-3)
...
#ifdef TRACE
  call trace_out('cart2frac_nomod')
#endif
!
  return
  end

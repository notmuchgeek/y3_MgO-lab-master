...
!************************************
  ...
  do i = 1,numat
    if (nat(i).gt.maxele) then
      if (.not.lregion1only.or.nregionno(nsft+nrelf2a(i)).eq.1) then
        ii = ii + 1
        iiptr(i) = ii
        ibocshptr(ii) = 0
...
!
          ...
          do while (ni.lt.nspcellat(ixyz).and..not.lfound)
            ...
            if (j.lt.i.and.nat(j).gt.maxele) then
              if (.not.lregion1only.or.nregionno(nsft+nrelf2a(j)).eq.1) then
                jj = iiptr(j)
                xd = xinbox(j) - xi
                yd = yinbox(j) - yi
                zd = zinbox(j) - zi

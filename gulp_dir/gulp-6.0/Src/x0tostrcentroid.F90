...
#ifdef TRACE
  call trace_in('x0tostrcentroid')
#endif
!
!  Allocate local workspace
!
  allocate(nmlist(numat),stat=status)
  if (status/=0) call outofmemory('x0tostrcentroid','nmlist')
  allocate(xsave(numat),stat=status)
  if (status/=0) call outofmemory('x0tostrcentroid','xsave')
  allocate(ysave(numat),stat=status)
  if (status/=0) call outofmemory('x0tostrcentroid','ysave')
  allocate(zsave(numat),stat=status)
  if (status/=0) call outofmemory('x0tostrcentroid','zsave')
!
...
!
  do i = 1,nstrains
    xstr(i) = - strain(i)/(1.0_dp + strain(i))
  enddo
  if (ndim.eq.3) then
...
!
  do i = 1,nasym
    xafrac(i) = xfrac(nrela2f(i))
    yafrac(i) = yfrac(nrela2f(i))
    zafrac(i) = zfrac(nrela2f(i))
  enddo
!********************************************************************************
...
!
  do i = 1,numat
    call cart2frac(ndim,xclat(i),yclat(i),zclat(i),rv,xfrac(i),yfrac(i),zfrac(i),icx,icy,icz)
  enddo
!
!  Deallocate local workspace
!
  deallocate(zsave,stat=status)
  if (status/=0) call deallocate_error('x0tostrcentroid','zsave')
  deallocate(ysave,stat=status)
  if (status/=0) call deallocate_error('x0tostrcentroid','ysave')
  deallocate(xsave,stat=status)
  if (status/=0) call deallocate_error('x0tostrcentroid','xsave')
  deallocate(nmlist,stat=status)
  if (status/=0) call deallocate_error('x0tostrcentroid','nmlist')
#ifdef TRACE
  call trace_out('x0tostrcentroid')
#endif
!
  return
  end

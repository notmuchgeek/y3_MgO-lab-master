  subroutine fourimp12d(eimp,lgrad1,lgrad2,imode,xderv,yderv,zderv)
!
...
!
  ...
  if (status/=0) call outofmemory('fourimp12d','natmiddle')
  allocate(ntypmiddle(2*nfor),stat=status)
  if (status/=0) call outofmemory('fourimp12d','ntypmiddle')
  allocate(nptrnforimp(nfor),stat=status)
  if (status/=0) call outofmemory('fourimp12d','nptrnforimp')
!
...
!
  ...
  enddo
!
!  Set looping range for loops that will be parallelised
!
  if (imode.eq.1) then
    lpmin = 1
    lpmax = nreg1onnode
...
!
            ...
            if (r422.gt.tr3.and.(.not.lbtyp.or..not.lbondedkl)) cycle lloop
!
!  Check r41 is OK
!
            x41 = x42 + x21
...
!  Loop over middle site 2 / j  *
!********************************
  jloop2:  do j = procid+1,ntreg2,nprocs
    lin2 = .false.
    nj = nr2a(j)
    ntypj = ntr2a(j)
    if (lnodisp) then
...
!  Prevent atoms j and l being the same atom
!           
            if (l.eq.k.or.l.eq.i) cycle lloop2
!  
!  Molecularity check
!           
            lbondedkl = .false.
...
!
            r422 = x42*x42 + y42*y42 + z42*z42
            if (r422.lt.1d-12) cycle lloop2
            if (r422.gt.tr3.and.(.not.lbtyp.or..not.lbondedkl)) cycle lloop2
!
!  Check r41 is OK
!
            x41 = x42 + x21
...
!
            ...
            z43 = z42 - z32
            r432 = x43*x43 + y43*y43 + z43*z43
            if (r432.lt.1.0d-12) cycle lloop2
!*********************************
!  Valid four-body term located  *
!*********************************
!
...
!**************************
  ...
  enddo jloop2
!
!  Only need to do the next part if this is a second derivative run:
!
  if (lgrad2) then
!
!********************************
...
!********************************
  jloop3:  do j = 1,ntot
    ...
    if (.not.lmatchany(nj,ntypj,nmiddle,natmiddle,ntypmiddle,.true.)) cycle jloop3
!***************************************
!  Region 1 for k :                    *
!  Loop over second middle site 3 / k  *
!***************************************
    kloop3: do kloc = lpmin,lpmax,nproclp
      if (imode.eq.1) then
        k = node2reg1(kloc)
      else
        k = kloc
      ...
      if (.not.lmatchany(nk,ntypk,nmiddle,natmiddle,ntypmiddle,.true.)) cycle kloop3
      if (.not.lmatchpair(nj,ntypj,nk,ntypk,nfor,nfspec2,nfptyp2,nfspec3,nfptyp3,.true.)) cycle kloop3
!
      x32 = xc3 - xc2
      y32 = yc3 - yc2
      z32 = zc3 - zc2
...
!  Distance checking
!     
      if (r322.gt.tr2max.and.(.not.lanybtyp.or..not.lbondedjk)) cycle kloop3
!
!  Set counter for number of valid i/l end atom combinations
!
      niltor = 0
...
!  Distance checking for j-k
!
        if (r322.gt.tr2.and.(.not.lbtyp.or..not.lbondedjk)) cycle pots3
!       
!  Check for intra and but not in same molecule
!       
...
!  Prevent atoms j and l being the same atom
!           
            if (l.eq.k.or.l.eq.i) cycle lloop3
!  
!  Molecularity check
!           
            lbondedkl = .false.
...
!
            r422 = x42*x42 + y42*y42 + z42*z42
            if (r422.lt.1d-12) cycle lloop3
            if (r422.gt.tr3.and.(.not.lbtyp.or..not.lbondedkl)) cycle lloop3
!
!  Check r41 is OK
!
            x41 = x42 + x21
...
!
            ...
            z43 = z42 - z32
            r432 = x43*x43 + y43*y43 + z43*z43
            if (r432.lt.1.0d-12) cycle lloop3
!*********************************
!  Valid four-body term located  *
!*********************************
!
...
!***************************************
    kloop4: do k = 1,ntot
      ...
      if (.not.lmatchany(nk,ntypk,nmiddle,natmiddle,ntypmiddle,.true.)) cycle kloop4
      if (.not.lmatchpair(nj,ntypj,nk,ntypk,nfor,nfspec2,nfptyp2,nfspec3,nfptyp3,.true.)) cycle kloop4
!
      x32 = xc3 - xc2
      y32 = yc3 - yc2
      z32 = zc3 - zc2
...
!  Distance checking
!     
      if (r322.gt.tr2max.and.(.not.lanybtyp.or..not.lbondedjk)) cycle kloop4
!
!  Set counter for number of valid i/l end atom combinations
!
      niltor = 0
...
!  Distance checking for j-k
!
        if (r322.gt.tr2.and.(.not.lbtyp.or..not.lbondedjk)) cycle pots4
!       
!  Check for intra and but not in same molecule
!       
...
!  Loop over end site 1 / i  *
!*****************************
        iloop4: do iloc = lpmin,lpmax,nproclp
          if (imode.eq.1) then
            i = node2reg1(iloc)
          else
            i = iloc
...
!
          ...
          if (lmol.and.lneedmol) then
            ...
            if (lmolok.and.linter_only) cycle iloop4
            if (.not.lmolok.and.lintra_only) cycle iloop4
            if (lmolok.and.lbtyp) then
              if (imode.eq.1) then
                if (lregion1j) then
                  lbondedij = .false.
...
!  Check for intra and but not in same molecule
!
          if (lintra_only.and..not.lmolok) cycle iloop4
          if (lbtyp.and..not.lmolok) cycle iloop4
          x21 = xc2 - xc1
          y21 = yc2 - yc1
          z21 = zc2 - zc1
!
...
!
          r212 = x21*x21 + y21*y21 + z21*z21
          if (r212.lt.1.0d-12) cycle iloop4
          if (r212.gt.tr1.and.(.not.lbtyp.or..not.lbondedij)) cycle iloop4
!
!  Check r31 is OK
!
          x31 = x32 + x21
          ...
          z31 = z32 + z21
          r312 = x31*x31 + y31*y31 + z31*z31
          if (r312.lt.1.0d-10) cycle iloop4
!**********************************
!  Loop over last end site 4 / l  *
!**********************************
          lloop4: do l = 1,ntot
...
!  Check l is allowed for n
!           
            if (.not.lmatch(nl,ntypl,nt4,ntyp4,.true.)) cycle lloop4
!  
!  Prevent atoms j and l being the same atom
!           
            if (l.eq.k.or.l.eq.i) cycle lloop4
!  
!  Molecularity check
!           
            lbondedkl = .false.
            if (lmol.and.lneedmol) then
              ...
              if (lmolok.and.linter_only) cycle lloop4
              if (.not.lmolok.and.lintra_only) cycle lloop4
              if (lmolok.and.lbtyp) then
                if (imode.eq.1) then
                  if (lregion1j.and.lregion1l) then
                    icm = 1
...
!
                ...
                if (.not.lbondedkl) cycle lloop4
              endif
            else
              lmolok = .false.
            endif
            if (lbtyp.and..not.lmolok) cycle lloop4
            x42 = xc4 - xc2
            y42 = yc4 - yc2
            z42 = zc4 - zc2
!
...
!
            r422 = x42*x42 + y42*y42 + z42*z42
            if (r422.lt.1d-12) cycle lloop4
            if (r422.gt.tr3.and.(.not.lbtyp.or..not.lbondedkl)) cycle lloop4
!
!  Check r41 is OK
!
            x41 = x42 + x21
            ...
            z41 = z42 + z21
            r412 = x41*x41 + y41*y41 + z41*z41
            if (r412.lt.1.0d-12) cycle lloop4
!
!  Check r43 is OK
!
            x43 = x42 - x32
            ...
            z43 = z42 - z32
            r432 = x43*x43 + y43*y43 + z43*z43
            if (r432.lt.1.0d-12) cycle lloop4
!*********************************
!  Valid four-body term located  *
!*********************************
!
...
!
        if (lanydreiding) then
          call isumall(niltor,niltortot,1_i4,"fourimp12d","niltor")
        else
          niltortot = niltor
        endif
!*******************************
...
!
          ...
          if (lmol.and.lneedmol) then
            ...
            if (lmolok.and.linter_only) cycle iloop5
            if (.not.lmolok.and.lintra_only) cycle iloop5
            if (lmolok.and.lbtyp) then
              if (imode.eq.1) then
                if (lregion1i.and.lregion1j) then
                  lbondedij = .false.
...
!  Check for intra and but not in same molecule
!
          if (lintra_only.and..not.lmolok) cycle iloop5
          if (lbtyp.and..not.lmolok) cycle iloop5
          x21 = xc2 - xc1
          y21 = yc2 - yc1
          z21 = zc2 - zc1
!
...
!
          r212 = x21*x21 + y21*y21 + z21*z21
          if (r212.lt.1.0d-12) cycle iloop5
          if (r212.gt.tr1.and.(.not.lbtyp.or..not.lbondedij)) cycle iloop5
!
!  Check r31 is OK
!
          x31 = x32 + x21
          ...
          z31 = z32 + z21
          r312 = x31*x31 + y31*y31 + z31*z31
          if (r312.lt.1.0d-10) cycle iloop5
!**********************************
!  Loop over last end site 4 / l  *
!**********************************
          lloop5: do lloc = lpmin,lpmax,nproclp
            if (imode.eq.1) then
              l = node2reg1(lloc)
            else
              l = lloc
...
!  Check l is allowed for n
!           
            if (.not.lmatch(nl,ntypl,nt4,ntyp4,.true.)) cycle lloop5
!  
!  Prevent atoms j and l being the same atom
!           
            if (l.eq.k.or.l.eq.i) cycle lloop5
!  
!  Molecularity check
!           
            lbondedkl = .false.
            if (lmol.and.lneedmol) then
              ...
              if (lmolok.and.linter_only) cycle lloop5
              if (.not.lmolok.and.lintra_only) cycle lloop5
              if (lmolok.and.lbtyp) then
                if (imode.eq.1) then
                  if (lregion1j) then
                    icm = 1
...
!
                ...
                if (.not.lbondedkl) cycle lloop5
              endif
            else
              lmolok = .false.
            endif
            if (lbtyp.and..not.lmolok) cycle lloop5
            x42 = xc4 - xc2
            y42 = yc4 - yc2
            z42 = zc4 - zc2
!
...
!
            r422 = x42*x42 + y42*y42 + z42*z42
            if (r422.lt.1d-12) cycle lloop5
            if (r422.gt.tr3.and.(.not.lbtyp.or..not.lbondedkl)) cycle lloop5
!
!  Check r41 is OK
!
            x41 = x42 + x21
            ...
            z41 = z42 + z21
            r412 = x41*x41 + y41*y41 + z41*z41
            if (r412.lt.1.0d-12) cycle lloop5
!
!  Check r43 is OK
!
            x43 = x42 - x32
            ...
            z43 = z42 - z32
            r432 = x43*x43 + y43*y43 + z43*z43
            if (r432.lt.1.0d-12) cycle lloop5
!*********************************
!  Valid four-body term located  *
!*********************************
!
...
!
  ...
  enddo jloop3
!
!  End of second derivative condition
!
  endif
!
!  Free local memory
!
  ...
  if (status/=0) call deallocate_error('fourimp12d','nptrnforimp')
  deallocate(ntypmiddle,stat=status)
  if (status/=0) call deallocate_error('fourimp12d','ntypmiddle')
  deallocate(natmiddle,stat=status)
  if (status/=0) call deallocate_error('fourimp12d','natmiddle')
#ifdef TRACE
  call trace_out('fourimp12d')
#endif
!
  return
  end

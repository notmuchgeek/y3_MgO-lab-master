...
!                                                                            !
!  Julian Gale, CIC, Curtin University, 2019                                 !
...
!     files
!
  character(len=maxwordlength), dimension(:), pointer :: pdffiles => null()
!
...
!
  ...
  integer(i4),     dimension(:),     allocatable    :: nrelf2a_cores
...
!
  ...
  character(len=maxwordlength)                      :: pdffilecfg
...
!  nrelf2a_cores, fracvec, atomvec,cartvec, arraymasses_kgmol,arraymasses_amu
...
!
  allocate(nrelf2a_cores(natomfull), stat=ierr) !corrected ers29 8th nov 06
  if (ierr .ne. 0) call outofmemory("allocateneutronarrays", "nrelf2a_cores")
  nrelf2a_cores = 0
...
!  first check none have been previously allocated
!
  if (allocated(nrelf2a_cores)) then
    deallocate(nrelf2a_cores,stat=ierr)
    if (ierr .ne. 0) call deallocate_error("deallocateneutronarrays","nrelf2a_cores")
  endif
...
!
  ...
  use g_constants, only : hbar_js, boltz
  implicit none
  real(dp)             :: bose_eins
...
!
    do i = 1,numat
      inat = iatn(nrelf2a(i))    ! use pointer to asym atom
      itype = natype(nrelf2a(i)) ! use pointer to asym atom
      ii = iocptr(i)
...
!  for PDFS
!
  call realloc_ch(maxwordlength,pdffiles,maxcfg,ierror) 
...
!  for frequency units
!
  call realloc_ch(6_i4,unitsnamecfgs,maxcfg,ierror) 
...
!
  ...
  cmpxdiv = dcmplx(realsoln, imsoln)
...
!
!  Takes the real and imaginary eigenvectors, and frequencies
  ...
  if (present(eig_i)) then !not at gamma point, so complex
    atomloop_ng : do ind=0, (nmodes-1), 3 !steps of three
      ...
      iloop_ng: do i=1, nmodes ! loop over all 3n branches
        ...
        polar(1) = dcmplx(eig_r(ind+1, i), eig_i(ind+1, i))
        polar(2) = dcmplx(eig_r(ind+2, i), eig_i(ind+2, i))
        polar(3) = dcmplx(eig_r(ind+3, i), eig_i(ind+3, i))
...
!!        polar = polar*euler(dot_product(veckcart,cartvec(iatnumber,1:3)))
    ...
    atomloop : do ind=0, (nmodes-1), 3 !steps of three
      ...
      iloop: do i=1, nmodes ! loop over all 3n branches
        !     set up complex vector
        polar(1) = dcmplx(eig_r(ind+1, i), 0.0_dp)
        polar(2) = dcmplx(eig_r(ind+2, i), 0.0_dp)
        polar(3) = dcmplx(eig_r(ind+3, i), 0.0_dp)
        !     put polar into common epolar_array
        epolar_array(1:3,i,iatnumber, icurrentk)=polar(1:3)
        !     put into omega array
        omega_array(i,icurrentk) = frequency(i)*cmtorads
...
!  11/19 Modify to allow for X-rays
!
  use g_constants
  use control,      only : lxray
  use current
  use element
...
!  X-ray case
!
      rbbari = 0.28179403_dp*dble(ni)*occuf(npatptr(i))
    else
!
!  Neutron case
!
...
!
  use element, only : maxele
  use current !contains nrelf2a etc
  implicit none
  ...
  do i = 1,n
    ...
    if ((inat .le. maxele).and. (inat .ne. 0)) then ! it must be a core
      ...
      if (icorecount .gt. n) then !this should never happen
        ...
        call stopnow("nrelf2a_setup")
      endif
      nrelf2a_cores(icorecount) = nrelf2a(i) 
    endif
  enddo

  end subroutine nrelf2a_setup
...
!
  ...
  do iw = 1,nword         ! put words in lower case
    call stolc(words(iw),maxwordlength)
  enddo

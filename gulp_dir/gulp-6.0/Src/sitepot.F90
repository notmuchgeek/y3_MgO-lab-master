...
!   2/18 Trace added
!   2/20 Modifications for npotsitescfg
!   2/20 Error in setting sites for non-symmetry 3D case corrected
!   2/20 Modified so that cartesian coordinates for site potentials are
...
!
  ...
  use symmetry,      only : rop, w1, vit, lsymopt
  use times
#ifdef TRACE
...
!
    if (ioproc) then
      ...
      write(ioout,'(''-------------------------------------------------------------------------------'')')
      if (lpotsitecartcfg(ncf)) then
        write(ioout,'('' Site   Cartesian Coordinates       Potential        Derivatives (V/Angs)'')')
      else
        write(ioout,'('' Site   Fractional Coordinates      Potential        Derivatives (V/Angs)'')')
      endif

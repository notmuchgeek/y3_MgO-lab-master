  subroutine phonon_fc(lprint,fc,nobsmodeptr0,nobsmode)
!
...
!
!  10/14 Created from phonon
!   2/15 Call to outshengBTE routines added
!   3/15 Position of call to outshengBTEfc2s moved
...
!   4/17 Call to setphonptr used to replace local calculation 
!        of phonon atoms
!   1/18 Trace added
...
!
  ...
  if (status/=0) call outofmemory('phonon_fc','meanKEperatom')
...
!
  ...
  if (status/=0) call outofmemory('phonon_fc','mcvptr')
  allocate(mcvrptr(mcv),stat=status)
  if (status/=0) call outofmemory('phonon_fc','mcvrptr')
!
  do m = 1,mcv
    mcvptr(m) = m 
    mcvrptr(m) = m 
...
!
  if (leigloc) then
    ...
    if (status/=0) call outofmemory('phonon_fc','savefreq')
    allocate(oscstrength(3,3,mcv),stat=status)
    if (status/=0) call outofmemory('phonon_fc','oscstrength')
    allocate(ramstrength(3,3,mcv),stat=status)
    if (status/=0) call outofmemory('phonon_fc','ramstrength')
  endif
!
...
!
  if (lthermal) then
    ...
    if (status/=0) call outofmemory('phonon_fc','Sij')
  endif
!
...
!
  ...
  do i = 1,3*ncfoc
    if (fmass(i).eq.0.0_dp) then
      call outerror('site has total mass of zero in phonon_fc',0_i4)
      call stopnow('phonon_fc')
    endif
...
!     
        ...
        if (status/=0) call outofmemory('phonon_fc','ipivot')                  
        allocate(wtmp(3*msv),stat=status)
        if (status/=0) call outofmemory('phonon_fc','wtmp')
!
...
!
        ...
        if (status/=0) call deallocate_error('phonon_fc','wtmp')
        deallocate(ipivot,stat=status)
        if (status/=0) call deallocate_error('phonon_fc','ipivot')  
!
        t2i = g_cpu_time()
        tmati = tmati + t2i - t1i
        if (ifail.ne.0) then
...
!
        ...
        if (status/=0) call outofmemory('phonon_fc','kpvt')
        ...
        if (status/=0) call outofmemory('phonon_fc','ctmp')
        call zhidi(eigr,msv,msv,kpvt,det,inert,ctmp,job)
        deallocate(ctmp,stat=status)
        if (status/=0) call deallocate_error('phonon_fc','ctmp')
        deallocate(kpvt,stat=status)
        if (status/=0) call deallocate_error('phonon_fc','kpvt')
        t2i = g_cpu_time()
        tmati = tmati + t2i - t1i
!
...
!
          ...
          if (status/=0) call outofmemory('phonon_fc','averfreq')
!
!  Store derv2
!
...
!
          ...
          if (status/=0) call deallocate_error('phonon_fc','averfreq')
        endif
!
!  Output frequencies / DOS / intensities
!
        call peigeng(mcv,freq(1,k-nlkpt+1),ncore,ncfoc,iocptr,maxd2,eigr, &
...
!**************************************************
  ...
  if (status/=0) call deallocate_error('phonon_fc','eigr')
  if (allocated(savederv2)) then
    if (lintegerKpoint_present) then
      deallocate(savedervi,stat=status)
      if (status/=0) call deallocate_error('phonon_fc','savedervi')
    endif
    deallocate(savederv2,stat=status)
    if (status/=0) call deallocate_error('phonon_fc','savederv2')
  endif
!*************************************
...
!
  if (lthermal) then
    ...
    if (status/=0) call deallocate_error('phonon_fc','Sij')
  endif
  if (leigloc) then
    deallocate(ramstrength,stat=status)
    if (status/=0) call deallocate_error('phonon_fc','ramstrength')
    deallocate(oscstrength,stat=status)
    if (status/=0) call deallocate_error('phonon_fc','oscstrength')
    deallocate(savefreq,stat=status)
    if (status/=0) call deallocate_error('phonon_fc','savefreq')
  endif
  ...
  if (status/=0) call deallocate_error('phonon_fc','mcvrptr')
  deallocate(mcvptr,stat=status)
  if (status/=0) call deallocate_error('phonon_fc','mcvptr')
  deallocate(meanKEperatom,stat=status)
  if (status/=0) call deallocate_error('phonon_fc','meanKEperatom')
!

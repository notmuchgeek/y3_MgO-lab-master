...
!
      ...
      do n = 1,nvar  
        if (iopttype(n).eq.iopt_strain) then
          cgradrep(ioptindex(n)) = gcrep(n,nrp+1)
        elseif (iopttype(n).eq.iopt_strain) then
          cgradrep(ioptindex(n)) = gcrep(n,nrp+1)
        elseif (iopttype(n).eq.iopt_xf) then
          gradrep(1,ioptindex(n)) = gcrep(n,nrp+1)
        elseif (iopttype(n).eq.iopt_yf) then
          gradrep(2,ioptindex(n)) = gcrep(n,nrp+1)
        elseif (iopttype(n).eq.iopt_zf) then
          gradrep(3,ioptindex(n)) = gcrep(n,nrp+1)
        elseif (iopttype(n).eq.iopt_radius) then
          gradrep(4,ioptindex(n)) = gcrep(n,nrp+1)
        endif
      enddo
!
      write(ioout,'(''********************************************************************************'')')

...
!   2/18 Trace added
!   7/18 Value of i passed to rfind corrected for symmetry
!   7/18 New routine psiscreendervs called in place of psiscreenderv
...
!
    if (.not.lMEAMden.and.rhoi.eq.0.0_dp.and.rhoj.eq.0.0_dp) cycle ijloop
!
    xcrd = xclat(j) - xal
    ycrd = yclat(j) - yal
    zcrd = zclat(j) - zal
...
!*********************************
    call rfind(xcrd,ycrd,zcrd,cut2,0.0_dp,0.0_dp,.false.,.false.,nreli,j,0_i4,0_i4,0_i4,0_i4,nmolonly,lself,0_i4,nor)
!  
...
!
                  call psiscreendervs(nreli,j,npartial,partial,xcd,ycd,zcd,ebas,Sij,strfct,lstr)
                endif
              endif
!
...
!
                      if (lstr) then
                        rstrd(1:nstrains) = rstrd(1:nstrains) + partial%sa_drhotots(1:nstrains,np)*ofct*strfct
                      endif
                    enddo

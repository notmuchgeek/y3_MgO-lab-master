  subroutine outfreq(leigloc,imode,freq,ir,ncfoc)
!
...
!   8/16 Use of channel 51 for storing frequencies removed
!   8/16 Comment symbols added to header for dens file
...
!
  ...
  logical,                         intent(in)  :: leigloc
  real(dp),                        intent(in)  :: freq(*)
  real(dp),                        intent(in)  :: ir(*)
!
!  Local variables
!
  character(len=1)                             :: blank(60)
  character(len=maxwordlength)                 :: pword
...
!*****************************
  if (lphono) then
    close(17)
    if (phonfile(1:1).ne.' ') then
      write(ioout,'(/,''  Vibrational density of states written as '',a)') pword
    else

...
!   8/14 neamfnmeamcombotype replaced by nmeamcombotype 
!   2/18 Trace added
!  10/18 Changes for strain module added
!
!  Conditions of use:
!
...
!
  use current,       only : nstrains, nstrains2
  use eam
...
#ifdef TRACE
  call trace_in('meamtotalrhoderv')
#endif
!
...
!
  ...
  if (lstr) then
    drhotots(1:nstrains) = 0.0_dp
    if (lgrad2) then
      drhotot2s(1:nstrains2) = 0.0_dp
      drhotot2m(1:nstrains,1:3) = 0.0_dp
    endif
  endif
  if (m.gt.0) then
...
!
        ...
        if (lstr) then
          do i = 1,nterm
            drhotots(1:nstrains) = drhotots(1:nstrains) + coeff(i)*scrho(i)*drhos(1:nstrains,i)
          enddo
        endif
        if (lgrad2) then
          do i = 1,nterm
          ...
          if (lstr) then
            do i = 1,nterm
              drhotot2s(1:nstrains2) = drhotot2s(1:nstrains2) + coeff(i)*scrho(i)*drho2s(1:nstrains2,i)
              drhotot2m(1:nstrains,1:3) = drhotot2m(1:nstrains,1:3) + coeff(i)*scrho(i)*drho2m(1:nstrains,1:3,i)
              ind = 0
              do ns1 = 1,nstrains
                do ns2 = 1,ns1
                  ind = ind + 1
...
!
        ...
        if (lstr) then
          drhotots(1:nstrains) = rrho*drhotots(1:nstrains)
        endif
        if (lgrad2) then
          drhotot2(1:6) = rrho*drhotot2(1:6)
          if (lstr) then
            drhotot2s(1:nstrains2)    = rrho*drhotot2s(1:nstrains2)
            drhotot2m(1:nstrains,1:3) = rrho*drhotot2m(1:nstrains,1:3)
          endif
        endif
!
...
!
          if (nterm.gt.1) then
            ...
            if (lstr) then
              do i = 2,nterm
                dgammas(1:nstrains) = dgammas(1:nstrains) + dgammadrhoh*coeff(i)*scrho(i)*drhos(1:nstrains,i)
              enddo
            endif
          endif
!
...
!
          ...
          if (lstr) then
            dgammas(1:nstrains) = dgammas(1:nstrains) - 2.0_dp*gamma*rrho0*drhos(1:nstrains,1)
          endif
          if (lgrad2) then
!
...
!
            if (lstr) then
              d2gamma2s(1:nstrains2) = 0.0_dp
              d2gamma2m(1:nstrains,1:3) = 0.0_dp
              if (nterm.gt.1) then
                ind = 0
                do ns1 = 1,nstrains
                  do ns2 = 1,ns1
...
!
          ...
          if (lstr) then
            drhotots(1:nstrains) = drhotots(1:nstrains) + drhos(1:nstrains,1)*gofgamma + rho0*dgofgammadgamma*dgammas(1:nstrains)
          endif
          if (lgrad2) then
            ...
            enddo
            if (lstr) then
              drhotot2s(1:nstrains2) = drhotot2s(1:nstrains2) + drho2s(1:nstrains2,1)*gofgamma &
                                                              + rho0*dgofgammadgamma*d2gamma2s(1:nstrains2)
              drhotot2m(1:nstrains,1:3) = drhotot2m(1:nstrains,1:3) + drho2m(1:nstrains,1:3,1)*gofgamma &
                                                                    + rho0*dgofgammadgamma*d2gamma2m(1:nstrains,1:3)
              ind = 0
              do ns1 = 1,nstrains
                do ns2 = 1,ns1
                  ind = ind + 1
...
#ifdef TRACE
  call trace_out('meamtotalrhoderv')
#endif
!
  return
  end

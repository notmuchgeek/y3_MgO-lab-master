...
!   8/19 Arc extension added to md archive file name if missing
!
!  Conditions of use:
!
...
!
    ...
    if (index(mdafil,'.arc').eq.0.and.index(mdafil,'.car').eq.0) then
      call endstring(mdafil,len(mdafil),iend)
      if (iend.eq.0) iend = len(mdafil) - 3
      mdafil(iend:iend+3) = '.arc'
    endif
  else

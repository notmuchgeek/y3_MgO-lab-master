  subroutine fouroop12d(efor,lgrad1,lgrad2,imode,xderv,yderv,zderv)
!
...
!****************************************
!  First pass: Middle atom derivatives  *
...
!*********************************************
!  Second pass: End atom second derivatives  *
!  Loop over middle site 1 / i               *
!*********************************************
      iloop2: do i = 1,ntot
        lregion1i = (i.le.nr1)
...
!  Only 3 bonds check
!
        if (lbtyp.and.lonly3oop(n).and.nbondi.ne.3) cycle iloop2
!
!  i has been accepted
!
!******************************************
!  Loop over first end site 2 / j         *
!  Only consider local atoms in region 1  *
!******************************************
        jloop2: do jloc = jmin,jmax,nprocj
          if (imode.eq.1) then
            j = node2reg1(jloc)
          else
            j = jloc
...
!
          r212 = x21*x21 + y21*y21 + z21*z21
          if (r212.lt.1d-10) cycle jloop2
          if ((r212.gt.tr1.or.r212.lt.tr1min).and.(.not.lbtyp.or..not.lbonded)) cycle jloop2
          r21 = sqrt(r212)
!************************************
!  Loop over second end site 3 / k  *
!************************************
...
!
            if (k.eq.j) cycle kloop2
!
!  Check k is allowed for n
!
            lmatch2 = lmatchanyof2(nk,ntypk,ntp3,nt3,ntyp3,tr2,tr2min,ntp4,nt4,ntyp4,tr3,tr3min)
            if (.not.lmatch2) cycle kloop2
!
!  Prevent atoms i and k being the same atom
!
            if (k.eq.i) cycle kloop2
!
!  Molecularity check
!
            lbonded = .false.
...
!
            r312 = x31*x31 + y31*y31 + z31*z31
            if (r312.lt.1d-10) cycle kloop2
            if ((r312.gt.tr2.or.r312.lt.tr2min).and.(.not.lbtyp.or..not.lbonded)) cycle kloop2
            r31 = sqrt(r312)
!**********************************
!  Loop over last end site 4 / l  *
!**********************************
...
!
              r412 = x41*x41 + y41*y41 + z41*z41
              if (r412.lt.1d-10) cycle lloop2
              if ((r412.gt.tr3.or.r412.lt.tr3min).and.(.not.lbtyp.or..not.lbonded)) cycle lloop2
              r41 = sqrt(r412)
!*********************************
!  Valid four-body term located  *
!*********************************
...
!  for symmetry adapted run otherwise we can skip gradients.
!
              if (ldsl.and..not.lin1.and..not.lin2.and..not.lin3.and..not.lin4) cycle lloop2
!*****************************
!  Out of plane derivatives  *
!*****************************
!
...
!*********************************************************************
!  Third pass: middle atom in region 2 to complete energy/gradients  *
!  Loop over middle site 1 / i                                       *
!*********************************************************************
    iloop3: do i = procid+1,ntreg2,nprocs
      lregion1i = .false.
      lin1 = .false.
      ni = nr2a(i)
      ntypi = ntr2a(i)
...
!  Only 3 bonds check
!
      if (lbtyp.and.lonly3oop(n).and.nbondi.ne.3) cycle iloop3
!
!  i has been accepted
!
!***********************************
...
!
        lmatch3 = lmatchanyof3(nj,ntypj,ntp2,nt2,ntyp2,tr1,tr1min,ntp3,nt3,ntyp3,tr2,tr2min,ntp4,nt4,ntyp4,tr3,tr3min)
        if (.not.lmatch3) cycle jloop3
!
!  Prevent atoms i and j being the same atom
!
        if (j.eq.i+nr1) cycle jloop3
!
        lbonded = .false.
        if (lmol.and.lneedmol) then
          lmolok = (nmi.eq.nmj.and.nmi.gt.0)
...
!
        r212 = x21*x21 + y21*y21 + z21*z21
        if (r212.lt.1d-10) cycle jloop3
        if ((r212.gt.tr1.or.r212.lt.tr1min).and.(.not.lbtyp.or..not.lbonded)) cycle jloop3
        r21 = sqrt(r212)
!************************************
!  Loop over second end site 3 / k  *
!************************************
...
!
          lmatch2 = lmatchanyof2(nk,ntypk,ntp3,nt3,ntyp3,tr2,tr2min,ntp4,nt4,ntyp4,tr3,tr3min)
          if (.not.lmatch2) cycle kloop3
!
!  Prevent atoms i and k being the same atom
!
          if (k.eq.i+nr1) cycle kloop3
!
!  Molecularity check
!
          lbonded = .false.
...
!
          r312 = x31*x31 + y31*y31 + z31*z31
          if (r312.lt.1d-10) cycle kloop3
          if ((r312.gt.tr2.or.r312.lt.tr2min).and.(.not.lbtyp.or..not.lbonded)) cycle kloop3
          r31 = sqrt(r312)
!**********************************
!  Loop over last end site 4 / l  *
!**********************************
...
!
            r412 = x41*x41 + y41*y41 + z41*z41
            if (r412.lt.1d-10) cycle lloop3
            if ((r412.gt.tr3.or.r412.lt.tr3min).and.(.not.lbtyp.or..not.lbonded)) cycle lloop3
            r41 = sqrt(r412)
!*********************************
!  Valid four-body term located  *
!*********************************
...
!  for symmetry adapted run otherwise we can skip gradients.
!
            if (ldsl.and..not.lin2.and..not.lin3.and..not.lin4) cycle lloop3
!*****************************
!  Out of plane derivatives  *
!*****************************
            if (lgrad1) then

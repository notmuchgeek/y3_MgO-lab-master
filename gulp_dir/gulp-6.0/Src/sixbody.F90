...
!   6/06 Derivatives up to second order finished
!  12/07 Unused variables removed
!   9/15 Third derivatives added
!   1/17 Case where phi angles are 0 or 180 trapped in derivatives
!   2/18 Trace added
!   9/18 Handling of terms where numerical stability comes in 
!        improved to ensure consistency of energy and forces
!
!  On entry :
!
...
!
  ...
  real(dp)                 :: cos1a2d(120),cos3a2d(120)
  real(dp)                 :: cos1b2d(120),cos3b2d(120)
  real(dp)                 :: cos1a3d(680),cos3a3d(680)
  real(dp)                 :: cos1b3d(680),cos3b3d(680)
  ...
  real(dp)                 :: ctrm2b
  real(dp)                 :: oopda
  real(dp)                 :: oopd1a(15),oopd2a(120),oopd3a(680)
  real(dp)                 :: oopdb
  real(dp)                 :: oopd1b(15),oopd2b(120),oopd3b(680)
  real(dp)                 :: rctrm1a
  real(dp)                 :: rctrm1b
  real(dp)                 :: rctrm2a
  real(dp)                 :: rctrm2b
  real(dp)                 :: rctrm1ad1(15),rctrm1ad2(120),rctrm1ad3(680)
  real(dp)                 :: rctrm1bd1(15),rctrm1bd2(120),rctrm2ad3(680)
  real(dp)                 :: rctrm2ad1(15),rctrm2ad2(120),rctrm1bd3(680)
  real(dp)                 :: rctrm2bd1(15),rctrm2bd2(120),rctrm2bd3(680)
  real(dp)                 :: rrctrm1a
  real(dp)                 :: rrctrm1b
  real(dp)                 :: rrctrm2a
  real(dp)                 :: rrctrm2b
  ...
  real(dp)                 :: sin1a3d(680),sin3a3d(680)
  real(dp)                 :: sin1b1d(15),sin3b1d(15)
  real(dp)                 :: sin1b2d(120),sin3b2d(120)
  real(dp)                 :: sin1b3d(680),sin3b3d(680)
  real(dp)                 :: sin1a
  real(dp)                 :: sin1b
  real(dp)                 :: sin3a
  real(dp)                 :: sin3b
  ...
  real(dp)                 :: top3a(680),bot3a(680)
  real(dp)                 :: top1b(15),bot1b(15)
  real(dp)                 :: top2b(120),bot2b(120)
  real(dp)                 :: top3b(680),bot3b(680)
#ifdef TRACE
  call trace_in('sixbody')
#endif
!
!  Set up kb2
!
...
!$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
    ...
    rctrm1a = sqrt(ctrm1a)
!
!  Trap energy contributions to be consistent with forces
!  where derivatives become ill-defined
!
    if (abs(ctrm2a).lt.1.0d-12) then
      rctrm2a = 0.0_dp
    else
      rctrm2a = sqrt(ctrm2a)
    endif
    rctrm1b = sqrt(ctrm1b)
    if (abs(ctrm2b).lt.1.0d-12) then
      rctrm2b = 0.0_dp
    else
      rctrm2b = sqrt(ctrm2b)
    endif
...
!
!  Trap case where phi angles are 0 or 180
!
      if (abs(ctrm2a).lt.1.0d-12) then
        rrctrm2a = 0.0_dp
      else
          ...
          do i = 1,15
            do j = i,15
              ...
              do k = j,15
                ...
                ik = kb2(k,i)
                jk = kb2(k,j)
                rctrm1ad3(ii) = - rctrm1a*rrctrm1a*(cos1a*cos1a3d(ii) + &
                                                    cos1a1d(i)*cos1a2d(jk) + &
                                                    cos1a1d(j)*cos1a2d(ik) + &
                                                    cos1a1d(k)*cos1a2d(ij))
                rctrm1ad3(ii) = rctrm1ad3(ii) - rctrm1a*(rrctrm1a**2)*cos1a*( &
                                   3.0_dp*cos1a1d(i)*cos1a1d(j)*cos1a1d(k) + &
                                   cos1a*(cos1a1d(i)*cos1a2d(jk) + &
                                          cos1a1d(j)*cos1a2d(ik) + &
                                          cos1a1d(k)*cos1a2d(ij) + &
                                   3.0_dp*rrctrm1a*cos1a1d(i)*cos1a1d(j)*cos1a1d(k)))
!
                rctrm2ad3(ii) = - rctrm2a*rrctrm2a*(cosphia*cosp3da(ii) + &
                                                    cosp1da(i)*cosp2da(jk) + &
                                                    cosp1da(j)*cosp2da(ik) + &
                                                    cosp1da(k)*cosp2da(ij))
                rctrm2ad3(ii) = rctrm2ad3(ii) - rctrm2a*(rrctrm1a**2)*cosphia*( &
                                   3.0_dp*cosp1da(i)*cosp1da(j)*cosp1da(k) + &
                                   cosphia*(cosp1da(i)*cosp2da(jk) + &
                                            cosp1da(j)*cosp2da(ik) + &
                                            cosp1da(k)*cosp2da(ij) + &
                                   3.0_dp*rrctrm1a*cosp1da(i)*cosp1da(j)*cosp1da(k)))
!
                rctrm1bd3(ii) = - rctrm1b*rrctrm1b*(cos1b*cos1b3d(ii) + &
                                                    cos1b1d(i)*cos1b2d(jk) + &
                                                    cos1b1d(j)*cos1b2d(ik) + &
                                                    cos1b1d(k)*cos1b2d(ij))
                rctrm1bd3(ii) = rctrm1bd3(ii) - rctrm1b*(rrctrm1b**2)*cos1b*( &
                                   3.0_dp*cos1b1d(i)*cos1b1d(j)*cos1b1d(k) + &
                                   cos1b*(cos1b1d(i)*cos1b2d(jk) + &
                                          cos1b1d(j)*cos1b2d(ik) + &
                                          cos1b1d(k)*cos1b2d(ij) + &
                                   3.0_dp*rrctrm1b*cos1b1d(i)*cos1b1d(j)*cos1b1d(k)))
!
                rctrm2bd3(ii) = - rctrm2b*rrctrm2b*(cosphib*cosp3db(ii) + &
                                                    cosp1da(i)*cosp2db(jk) + &
                                                    cosp1db(j)*cosp2db(ik) + &
                                                    cosp1db(k)*cosp2db(ij))
                rctrm2bd3(ii) = rctrm2bd3(ii) - rctrm2b*(rrctrm1a**2)*cosphib*( &
                                   3.0_dp*cosp1db(i)*cosp1db(j)*cosp1db(k) + &
                                   cosphia*(cosp1db(i)*cosp2db(jk) + &
                                            cosp1db(j)*cosp2db(ik) + &
                                            cosp1db(k)*cosp2db(ij) + &
                                   3.0_dp*rrctrm1b*cosp1db(i)*cosp1db(j)*cosp1db(k)))
              enddo
            enddo
          enddo
!
          ...
          do i = 1,15
            do j = i,15
              ...
              do k = j,15
                ...
                ik = kb2(k,i)
                jk = kb2(k,j)
                oopd3a(ii) = r12*(rctrm1ad3(ii)*rctrm2a + rctrm1a*rctrm2ad3(ii) + &
                                  rctrm1ad1(i)*rctrm2ad2(jk) + &
                                  rctrm1ad1(j)*rctrm2ad2(ik) + &
                                  rctrm1ad1(k)*rctrm2ad2(ij) + &
                                  rctrm2ad1(i)*rctrm1ad2(jk) + &
                                  rctrm2ad1(j)*rctrm1ad2(ik) + &
                                  rctrm2ad1(k)*rctrm1ad2(ij))
                oopd3b(ii) = r12*(rctrm1bd3(ii)*rctrm2b + rctrm1b*rctrm2bd3(ii) + &
                                  rctrm1bd1(i)*rctrm2bd2(jk) + &
                                  rctrm1bd1(j)*rctrm2bd2(ik) + &
                                  rctrm1bd1(k)*rctrm2bd2(ij) + &
                                  rctrm2bd1(i)*rctrm1bd2(jk) + &
                                  rctrm2bd1(j)*rctrm1bd2(ik) + &
                                  rctrm2bd1(k)*rctrm1bd2(ij))
                if (i.eq.1) then
                  oopd3a(ii) = oopd3a(ii) + rr12*(rctrm1ad2(jk)*rctrm2a + rctrm1a*rctrm2ad2(jk) + &
                                                  rctrm1ad1(j)*rctrm2ad1(k) + rctrm1ad1(k)*rctrm2ad1(j))
                  oopd3b(ii) = oopd3b(ii) + rr12*(rctrm1bd2(jk)*rctrm2b + rctrm1b*rctrm2bd2(jk) + &
                                                  rctrm1bd1(j)*rctrm2bd1(k) + rctrm1bd1(k)*rctrm2bd1(j))
                  if (j.eq.1) then
                    oopd3a(ii) = oopd3a(ii) - rr122*rr12*(rctrm1ad1(k)*rctrm2a + rctrm1a*rctrm2ad1(k))
                    oopd3b(ii) = oopd3b(ii) - rr122*rr12*(rctrm1bd1(k)*rctrm2b + rctrm1b*rctrm2bd1(k))
                  endif
                  if (k.eq.1) then
                    oopd3a(ii) = oopd3a(ii) - rr122*rr12*(rctrm1ad1(j)*rctrm2a + rctrm1a*rctrm2ad1(j))
                    oopd3b(ii) = oopd3b(ii) - rr122*rr12*(rctrm1bd1(j)*rctrm2b + rctrm1b*rctrm2bd1(j))
                  endif
                endif
                if (j.eq.1) then
                  oopd3a(ii) = oopd3a(ii) + rr12*(rctrm1ad2(ik)*rctrm2a + rctrm1a*rctrm2ad2(ik) + &
                                                  rctrm1ad1(i)*rctrm2ad1(k) + rctrm1ad1(k)*rctrm2ad1(i))
                  oopd3b(ii) = oopd3b(ii) + rr12*(rctrm1bd2(ik)*rctrm2b + rctrm1b*rctrm2bd2(ik) + &
                                                  rctrm1bd1(i)*rctrm2bd1(k) + rctrm1bd1(k)*rctrm2bd1(i))
                  if (k.eq.1) then
                    oopd3a(ii) = oopd3a(ii) - rr122*rr12*(rctrm1ad1(i)*rctrm2a + rctrm1a*rctrm2ad1(i))
                    oopd3b(ii) = oopd3b(ii) - rr122*rr12*(rctrm1bd1(i)*rctrm2b + rctrm1b*rctrm2bd1(i))
                  endif
                endif
                if (k.eq.1) then
                  oopd3a(ii) = oopd3a(ii) + rr12*(rctrm1ad2(ij)*rctrm2a + rctrm1a*rctrm2ad2(ij) + &
                                                  rctrm1ad1(i)*rctrm2ad1(j) + rctrm1ad1(j)*rctrm2ad1(i))
                  oopd3b(ii) = oopd3b(ii) + rr12*(rctrm1bd2(ij)*rctrm2b + rctrm1b*rctrm2bd2(ij) + &
                                                  rctrm1bd1(i)*rctrm2bd1(j) + rctrm1bd1(j)*rctrm2bd1(i))
                endif
              enddo
            enddo
          enddo
          oopd3a(1) = oopd3a(1) + 3.0_dp*oopda*rr124*rr122
          oopd3b(1) = oopd3b(1) + 3.0_dp*oopdb*rr124*rr122
!
          ii = 0
          do i = 1,15
            do j = i,15

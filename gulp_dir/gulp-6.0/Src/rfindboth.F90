...
!
  ...
  elseif (ndim.eq.0) then
    call rfindmid0D(xik0,yik0,zik0,xjk0,yjk0,zjk0,rcut2,lincludeself,nvec0,nvec,vectorpair)
  endif
#ifdef TRACE
  call trace_out('rfindmid')
#endif
!
  return
  end
...
!  Rfindmid : Get any image common to two atoms  *
!*************************************************
  subroutine rfindmid0D(xik0,yik0,zik0,xjk0,yjk0,zjk0,rcut2,lincludeself,nvec0,nvec,vectorpair)
!
!  Subroutine that searches for valid images of an atom k that are 
!  within a cutoff of mid point of atom i and atom j.
!
...
!  zvector_pair2  = z component of valid j-k vector
!
!  10/18 Created based on rfindmid1D
!
!  Conditions of use:
!
!  GULP is available free of charge to academic institutions
...
!  Find mid point of i - j vector to k
!
  xij0 = 0.5_dp*(xjk0 + xik0)
  yij0 = 0.5_dp*(yjk0 + yik0)
  zij0 = 0.5_dp*(zjk0 + zik0)
!
!  Calculate squares of distances
!
  r2ij = xij0*xij0 + yij0*yij0 + zij0*zij0
...
!
  if (r2ij.le.rcut2) then
    if ((r2ik.gt.1.0d-12.and.r2jk.gt.1.0d-12).or.lincludeself) then
      ...
      vectorpair%distance_pair1(nvec0 + nvec) = r2ik
      vectorpair%xvector_pair1(nvec0 + nvec) = xik0
      vectorpair%yvector_pair1(nvec0 + nvec) = yik0
      vectorpair%zvector_pair1(nvec0 + nvec) = zik0
      vectorpair%distance_pair2(nvec0 + nvec) = r2jk
      vectorpair%xvector_pair2(nvec0 + nvec) = xjk0
      vectorpair%yvector_pair2(nvec0 + nvec) = yjk0
      vectorpair%zvector_pair2(nvec0 + nvec) = zjk0
    endif
  endif
#ifdef TRACE
  call trace_out('rfindmid0D')
#endif
!
  return
  end

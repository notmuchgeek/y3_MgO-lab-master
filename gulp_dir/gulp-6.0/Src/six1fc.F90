  subroutine six1fc(maxlhs,d1cell,matom,vmatom)
!
!  Subroutine to calculate six-body potential contribution to phonons.
!  Unphased version that uses numerical first derivatives
!
!  12/14 Created from sixfc
!   1/15 Debugged to working form
...
!  12/17 Goto statements replaced by loop labels
!   2/18 Trace added
!
!  Conditions of use:
!
...
!********************************
!
!  Find which atom is matom
!
                          mvec = 0
                          do ivec = 1,6
                            if (isatom(ivec).eq.matom) mvec = ivec
                          enddo
!
!  If no atom is matom then skip the rest
!
                          if (mvec.gt.0) then
!
!  Correct coordinates for matom displacements
!
                            sxyz(1,mvec) = sxyz(1,mvec) + (vmatom(1) - xclat(isatom(mvec)))
                            sxyz(2,mvec) = sxyz(2,mvec) + (vmatom(2) - yclat(isatom(mvec)))
                            sxyz(3,mvec) = sxyz(3,mvec) + (vmatom(3) - zclat(isatom(mvec)))
!
!  Calculate indices relative to matom
!
                            do ivec = 1,6
                              isvec(1,ivec) = icsxyz(1,ivec) - icsxyz(1,mvec)
                              isvec(2,ivec) = icsxyz(2,ivec) - icsxyz(2,mvec)
                              isvec(3,ivec) = icsxyz(3,ivec) - icsxyz(3,mvec)
                            enddo
!
...
!
                            ...
                            rko = rksix*ofct
                            call sixbody(np,nsixtype,sdist,eterm,e1d,e2d,e3d,rko,.true.,.false.,.false.)
!*************************
...
!  Find cell index
!
                              if (abs(isvec(1,ivec)).gt.nd2cell(1).or. &
                                  abs(isvec(2,ivec)).gt.nd2cell(2).or. &
                                  abs(isvec(3,ivec)).gt.nd2cell(3)) then
!
!  Find cell index - if outside user range then assign to central cell
!
                                ncind(ivec) = nd2central
                              else
!
!  Compute index
!
                                ncind(ivec) = nd2cellptr(nd2cell(1)+1+isvec(1,ivec), &
                                                         nd2cell(2)+1+isvec(2,ivec), &
                                                         nd2cell(3)+1+isvec(3,ivec))
                              endif
                            enddo
!
                            indvec = 0
                            do ivec = 1,5
                              do jvec = ivec+1,6
                                indvec = indvec + 1
                                if (ivec.ne.mvec) then
                                  d1cell(1,isatom(ivec),ncind(ivec)) = &
                                  d1cell(1,isatom(ivec),ncind(ivec)) + &
                                    svec(1,jvec,ivec)*e1d(indvec)
                                  d1cell(2,isatom(ivec),ncind(ivec)) = &
                                  d1cell(2,isatom(ivec),ncind(ivec)) + &
                                    svec(2,jvec,ivec)*e1d(indvec)
                                  d1cell(3,isatom(ivec),ncind(ivec)) = &
                                  d1cell(3,isatom(ivec),ncind(ivec)) + &
                                    svec(3,jvec,ivec)*e1d(indvec)
                                endif
!
                                if (jvec.ne.mvec) then
                                  d1cell(1,isatom(jvec),ncind(jvec)) = &
                                  d1cell(1,isatom(jvec),ncind(jvec)) + &
                                    svec(1,ivec,jvec)*e1d(indvec)
                                  d1cell(2,isatom(jvec),ncind(jvec)) = &
                                  d1cell(2,isatom(jvec),ncind(jvec)) + &
                                    svec(2,ivec,jvec)*e1d(indvec)
                                  d1cell(3,isatom(jvec),ncind(jvec)) = &
                                  d1cell(3,isatom(jvec),ncind(jvec)) + &
                            ...
                            enddo
!
!  End condition on one of the atoms being matom
!
                          endif
!

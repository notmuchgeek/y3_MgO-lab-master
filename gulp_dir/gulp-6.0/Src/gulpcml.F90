...
module gulp_cml
  ...
  use datatypes
  use gulp_lengths, only : maxwordlength
  ...
  character(len=maxwordlength), save  :: cmlfilename      ! CML file name 
#ifndef NOFOX
...
!============================================================================!
  ...
  call cmlStartParameterList(cmlfile, title='Configuration information', &
    ...
    ! shrink factors...
    if ((nks(1,cfgnumber)*nks(2,cfgnumber)*nks(3,cfgnumber)).gt.0) then
        is(1) = nks(1,cfgnumber)
        is(2) = nks(2,cfgnumber)
        is(3) = nks(3,cfgnumber)
    else
        is(1:3) = 1 
    endif
...
subroutine gulp_cml_print_epot_lattice(v)
  ...
  use current, only : nasym,  nrela2f, nat, nftype
#endif
...
use current, only : nasym, iatn, natype, nrela2f, &
                  & nrelf2a, xafrac, yafrac, zafrac, occua, qa, rv, ndim
use element, only : maxele

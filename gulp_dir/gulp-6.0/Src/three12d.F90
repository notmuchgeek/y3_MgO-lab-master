  subroutine three12d(ethb,lgrad1,lgrad2,imode)
!
!  Subroutine for defect three-body energy
!  Distributed memory parallel second derivatives
!
...
!
  ...
  if (status/=0) call outofmemory('three12d','xderv')
  allocate(yderv(nr1),stat=status)
  if (status/=0) call outofmemory('three12d','yderv')
  allocate(zderv(nr1),stat=status)
  if (status/=0) call outofmemory('three12d','zderv')
!
...
!*************************************************************************
    iloop: do iloc = imin,imax,nproci
!
!  Region 1 ion
!
      if (imode.eq.1) then
...
!
      ...
      enddo jloop
!
!  End of outer loop over atoms on node
!
    enddo iloop
!*****************************************
!  Outer loop over region 2 pivot atoms  *
!*****************************************
    iloop2: do i = procid+nr1+1,ntot,nprocs
!
!  Region 2 ion
!
      lin1 = .false.
...
! 
      if (ltdreiding(n)) then               
        if (.not.bonded2donor(npsi)) cycle iloop2
      endif
!*******************************
!  Inner loop over first site  *
!*******************************
...
!
        ...
        if (lmol.and.lneedmol) then
          ...
          if (lmolok.and.linter_only) cycle jloop2
          if (.not.lmolok.and.lintra_only) cycle jloop2
          if (lmolok.and.lbtyp) then
            if (imode.eq.1) then
              if (npsi.gt.0.and.npsj.gt.0) then
                call mindtoijk(indmj,jxx,jyy,jzz)
...
!
        r122 = x21*x21 + y21*y21 + z21*z21
        if (r122.lt.1.0d-12) cycle jloop2
        if ((r122.gt.ttr1.or.r122.lt.ttr1m).and.(.not.lbtyp.or..not.lbonded)) then
          if (ldiff23typ.or..not.ldiff23cut) cycle jloop2
          if (r122.lt.ttr2.and.r122.gt.ttr2m) then
            ttmp = ttr2m
            ttr2m = ttr1m
            ttr1m = ttmp
...
!  Check k is allowed for n, and not equivalent to j
!
          if (.not.lmatch(nk,ntypk,nto,ntypo,.true.)) cycle kloop2
!
!  Molecularity check
!
          lbonded = .false.
          if (lmol.and.lneedmol) then
            ...
            if (lmolok.and.linter_only) cycle kloop2
            if (.not.lmolok.and.lintra_only) cycle kloop2
            if (lmolok.and.lbtyp) then
              if (imode.eq.1) then
                if (npsi.gt.0.and.npsk.gt.0) then
                  call mindtoijk(indmk,kxx,kyy,kzz)
...
!
          r132 = x31*x31 + y31*y31 + z31*z31
          if (r132.lt.1.0d-12.and.(.not.lbtyp.or..not.lbonded)) cycle kloop2
!
!  Modification to handle case where species for 2 and 3 are the same
!  but cutoffs are different
!
          if (r132.gt.ttr2.or.r132.lt.ttr2m) then
            if (ldiff23typ.or..not.ldiff23cut) cycle kloop2
            if (r122.gt.ttr2.or.r132.gt.ttr1) cycle kloop2
            if (r122.lt.ttr2m.or.r132.lt.ttr1m) cycle kloop2
            if (lswaprho) then
              rro = rho1
              rho1 = rho2
              rho2 = rro
...
!  If bond types don't match, but atom types are symmetric then try other permutation
!                 
              if (.not.ldiff23typ.or..not.ldiff23bo) cycle kloop2
!  
!  Check i-j bond for correct order
!                 
              if (nbotyp21.gt.0.and.nbotyp21.ne.nbtypeij) cycle kloop2
              if (nbotyp22.gt.1.and.nbotyp22.ne.nbtypeij2) cycle kloop2
!               
!  Check i-k bond for correct order
!                 
              if (nbotyp11.gt.0.and.nbotyp11.ne.nbtypeik) cycle kloop2
              if (nbotyp12.gt.1.and.nbotyp12.ne.nbtypeik2) cycle kloop2
!  
!  If we make it to here then bond orders are the wrong way wrong and i-j/i-k terms should be swapped
!
              if (lswaprho) then
...
!
          ...
          z23 = z31 - z21
          r232 = x23**2 + y23**2 + z23**2
          if (r232.gt.tr3.and..not.lbtyp) cycle kloop2
          if (r232.lt.tr3m.or.r232.lt.1d-12) cycle kloop2
!
!  Valid three-body term  = > calculate potential
!
          r23v(1) = - x23
...
!  for symmetry adapted run otherwise gradients can be skipped.
!
          if (ldsl.and..not.lin1.and..not.lin2.and..not.lin3) cycle kloop2
!*************************
!  Internal derivatives  *
!*************************
          if (lgrad1) then
...
!  NB: This is only needed to complete the second derivatives           *
!************************************************************************
      iloop3: do i = 1,ntot
        if (i.le.nr1) then
!
...
!  Check i is allowed for n
!
        if (.not.lmatch(ni,ntypi,nt1,ntyp1,.true.)) cycle iloop3
!     
!  Check number of bonds if necessary
!  
        if (n3bondnono(1,n).gt.0) then
...
! 
        if (ltdreiding(n)) then               
          if (.not.bonded2donor(npsi)) cycle iloop3
        endif
!*******************************
!  Inner loop over first site  *
!*******************************
!
!  Loop only over local sites for second derivatives
!
        jloop3: do jloc = 1,nreg1onnode
          j = node2reg1(jloc)
!
!  Region 1 ion
!
...
!
          ...
          if (lmol.and.lneedmol) then
            ...
            if (lmolok.and.linter_only) cycle jloop3
            if (.not.lmolok.and.lintra_only) cycle jloop3
            if (lmolok.and.lbtyp) then
              if (imode.eq.1) then
                if (lregion1i) then
                  lbonded = .false.
              ...
              if (.not.lbonded) cycle jloop3
            endif
          else
            lmolok = .false.
          endif
          if (lbtyp.and..not.lmolok) cycle jloop3
!
!  Check r12 is OK
!
          r122 = x21*x21 + y21*y21 + z21*z21
          if (r122.lt.1.0d-12) cycle jloop3
          if ((r122.gt.ttr1.or.r122.lt.ttr1m).and.(.not.lbtyp.or..not.lbonded)) then
            if (ldiff23typ.or..not.ldiff23cut) cycle jloop3
            if (r122.lt.ttr2.and.r122.gt.ttr2m) then
              ttmp = ttr2m
              ttr2m = ttr1m
              ttr1m = ttmp
...
!  Check k is allowed for n, and not equivalent to j
!
            if (.not.lmatch(nk,ntypk,nto,ntypo,.true.)) cycle kloop3
!
!  Molecularity check
!
            lbonded = .false.
            if (lmol.and.lneedmol) then
              ...
              if (lmolok.and.linter_only) cycle kloop3
              if (.not.lmolok.and.lintra_only) cycle kloop3
              if (lmolok.and.lbtyp) then
                if (imode.eq.1) then
...
!
            ...
            if (lbtyp.and..not.lmolok) cycle kloop3
!
!  Check r13 is OK
!
            r132 = x31*x31 + y31*y31 + z31*z31
            if (r132.lt.1.0d-12.and.(.not.lbtyp.or..not.lbonded)) cycle kloop3
!
!  Modification to handle case where species for 2 and 3 are the same
!  but cutoffs are different
!
            if (r132.gt.ttr2.or.r132.lt.ttr2m) then
              if (ldiff23typ.or..not.ldiff23cut) cycle kloop3
              if (r122.gt.ttr2.or.r132.gt.ttr1) cycle kloop3
              if (r122.lt.ttr2m.or.r132.lt.ttr1m) cycle kloop3
              if (lswaprho) then
                rro = rho1
                rho1 = rho2
                rho2 = rro
...
!  If bond types don't match, but atom types are symmetric then try other permutation
!                 
                if (.not.ldiff23typ.or..not.ldiff23bo) cycle kloop3
!  
!  Check i-j bond for correct order
!                 
                if (nbotyp21.gt.0.and.nbotyp21.ne.nbtypeij) cycle kloop3
                if (nbotyp22.gt.1.and.nbotyp22.ne.nbtypeij2) cycle kloop3
!               
!  Check i-k bond for correct order
!                 
                if (nbotyp11.gt.0.and.nbotyp11.ne.nbtypeik) cycle kloop3
                if (nbotyp12.gt.1.and.nbotyp12.ne.nbtypeik2) cycle kloop3
!  
!  If we make it to here then bond orders are the wrong way wrong and i-j/i-k terms should be swapped
!
                if (lswaprho) then
...
!
            ...
            z23 = z31 - z21
            r232 = x23**2 + y23**2 + z23**2
            if (r232.gt.tr3.and..not.lbtyp) cycle kloop3
            if (r232.lt.tr3m.or.r232.lt.1d-12) cycle kloop3
!
!  Valid three-body term  = > calculate potential
!
            r23v(1) = - x23
...
!  for symmetry adapted run otherwise gradients can be skipped.
!
            if (ldsl.and..not.lin1.and..not.lin2.and..not.lin3) cycle kloop3
            if (lregion1i) then
              n1x = 3*(i-1) + 1
              if (lin1) then
                n1xr = 3*(ir-1) + 1
...
!
        ...
        enddo jloop3
!
!  End of outer loop over sites searching for j on node
!
      enddo iloop3
    endif
  enddo potl
...
!  Globalise energy
!
  call sumone(ethb,ethb1,"three12d","ethb")
  ethb = ethb1
!
!  Free local memory
!
  deallocate(zderv,stat=status)
  if (status/=0) call deallocate_error('three12d','zderv')
  deallocate(yderv,stat=status)
  if (status/=0) call deallocate_error('three12d','yderv')
  deallocate(xderv,stat=status)
  if (status/=0) call deallocate_error('three12d','xderv')
!
!  Timing
!
  time2 = g_cpu_time()
